object Frame_Parent: TFrame_Parent
  Left = 0
  Top = 0
  Width = 1145
  Height = 594
  TabOrder = 0
  object Panel_Parent: TPanel
    Left = 0
    Top = 82
    Width = 1145
    Height = 512
    Align = alClient
    TabOrder = 2
  end
  object Panel_list: TPanel
    Left = 0
    Top = 82
    Width = 1145
    Height = 512
    Align = alClient
    Anchors = [akTop, akRight, akBottom]
    Color = clWhite
    ParentBackground = False
    TabOrder = 3
    object StatusBar1: TStatusBar
      Left = 1
      Top = 482
      Width = 1143
      Height = 29
      Panels = <
        item
          Width = 150
        end
        item
          Width = 150
        end
        item
          Width = 150
        end>
    end
    object Panel3: TPanel
      Left = 1
      Top = 1
      Width = 1143
      Height = 100
      Align = alTop
      TabOrder = 0
      DesignSize = (
        1143
        100)
      object Label2: TLabel
        Left = 11
        Top = 5
        Width = 115
        Height = 21
        Alignment = taCenter
        AutoSize = False
        Caption = 'Patient Search'
        Color = 2922059
        Font.Charset = ANSI_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        Transparent = False
        Layout = tlCenter
      end
      object Label110: TLabel
        Left = 560
        Top = 24
        Width = 106
        Height = 13
        Caption = '  - > Today'#39's Followup'
        Visible = False
      end
      object Label116: TLabel
        Left = 869
        Top = 5
        Width = 71
        Height = 13
        Caption = '- > Free In-Pt.'
        Visible = False
      end
      object SPBRegisteredPateintFromFrontDesk: TSpeedButton
        Left = 1094
        Top = 43
        Width = 33
        Height = 28
        Caption = 'Registered Patient From Front Desk'
        Enabled = False
        Visible = False
        OnClick = SPBRegisteredPateintFromFrontDeskClick
      end
      object SpeedButton5: TSpeedButton
        Left = 1104
        Top = 1
        Width = 38
        Height = 98
        Align = alRight
        Flat = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
        OnClick = SpeedButton5Click
        ExplicitLeft = 1102
        ExplicitTop = -2
      end
      object Label117: TLabel
        Left = 560
        Top = 7
        Width = 74
        Height = 13
        Caption = '  - > Normal Pt.'
        Visible = False
      end
      object Label105: TLabel
        Left = 560
        Top = 40
        Width = 71
        Height = 13
        Caption = '  - > In Patient'
        Visible = False
      end
      object Label1: TLabel
        Left = 560
        Top = 57
        Width = 80
        Height = 13
        Caption = '  - > P. Admitted'
        Visible = False
      end
      object Edit_Search: TEdit
        Left = 11
        Top = 30
        Width = 193
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 4
        OnKeyPress = Edit_SearchKeyPress
      end
      object GroupBox1: TGroupBox
        Left = 207
        Top = 2
        Width = 163
        Height = 73
        TabOrder = 1
        object Label4: TLabel
          Left = 3
          Top = 32
          Width = 31
          Height = 13
          Caption = 'From :'
        end
        object SPB_From: TSpeedButton
          Left = 101
          Top = 28
          Width = 22
          Height = 22
          Caption = 'AD'
          OnClick = SPB_FromClick
        end
        object Label5: TLabel
          Left = 18
          Top = 54
          Width = 16
          Height = 13
          Alignment = taRightJustify
          Caption = 'To:'
        end
        object SPB_To: TSpeedButton
          Left = 101
          Top = 50
          Width = 22
          Height = 22
          Caption = 'AD'
          OnClick = SPB_ToClick
        end
        object DEX_From: TDateEditX
          Left = 36
          Top = 29
          Width = 65
          Height = 21
          TabOrder = 2
          ControlData = {
            545046300A5444617465456469745200044C656674022403546F70021D0B4461
            746541442E5965617203DB070C4461746541442E4D6F6E746802090A44617465
            41442E44617902160B4461746556532E596561720314080C4461746556532E4D
            6F6E746802060A4461746556532E44617902050000}
        end
        object DEX_To: TDateEditX
          Left = 36
          Top = 51
          Width = 65
          Height = 21
          TabOrder = 3
          ControlData = {
            545046300A5444617465456469745200044C656674022403546F7002330B4461
            746541442E5965617203DB070C4461746541442E4D6F6E746802090A44617465
            41442E44617902160B4461746556532E596561720314080C4461746556532E4D
            6F6E746802060A4461746556532E44617902050000}
        end
        object CB_All: TCheckBox
          Left = 5
          Top = 4
          Width = 28
          Height = 17
          Caption = 'All'
          Checked = True
          Enabled = False
          State = cbChecked
          TabOrder = 1
          OnClick = CB_AllClick
        end
        object DBLC_Dep: TDBLookupComboBox
          Left = 35
          Top = 2
          Width = 118
          Height = 21
          Enabled = False
          KeyField = 'DEPT_DEPCODE'
          ListField = 'DEPT_DEPNAME'
          ListSource = DS_Dep
          TabOrder = 0
        end
      end
      object GroupBox2: TGroupBox
        Left = 373
        Top = 2
        Width = 156
        Height = 72
        TabOrder = 2
        object Label6: TLabel
          Left = 2
          Top = 20
          Width = 65
          Height = 26
          Alignment = taCenter
          Caption = 'Include more than'
          WordWrap = True
        end
        object Label7: TLabel
          Left = 130
          Top = 22
          Width = 24
          Height = 13
          Caption = 'Days'
        end
        object BB_Refresh: TSpeedButton
          Left = 56
          Top = 41
          Width = 97
          Height = 28
          Caption = 'Refresh [F5]'
          OnClick = BB_RefreshClick
        end
        object se_days: TSpinEdit
          Left = 71
          Top = 18
          Width = 56
          Height = 22
          MaxValue = 0
          MinValue = 0
          TabOrder = 1
          Value = 0
          OnChange = se_daysChange
          OnKeyPress = se_daysKeyPress
        end
        object CB_DateRange: TCheckBox
          Left = 3
          Top = 3
          Width = 98
          Height = 13
          Caption = 'By Date Range'
          Checked = True
          State = cbChecked
          TabOrder = 0
          OnClick = CB_DateRangeClick
        end
      end
      object Edit3: TEdit
        Left = 544
        Top = 24
        Width = 12
        Height = 12
        TabStop = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Color = 4227327
        TabOrder = 5
        Visible = False
      end
      object Edit12: TEdit
        Left = 859
        Top = 7
        Width = 8
        Height = 8
        TabStop = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Color = 12718764
        TabOrder = 3
        Visible = False
      end
      object CB_WithRepeatVisit: TCheckBox
        Left = 159
        Top = 54
        Width = 25
        Height = 17
        Caption = 'Patient With Repeat Visit'
        Enabled = False
        TabOrder = 7
        Visible = False
      end
      object GB_PatientAdmission: TGroupBox
        Left = 1081
        Top = 77
        Width = 15
        Height = 24
        Anchors = [akLeft, akTop, akRight]
        TabOrder = 21
        Visible = False
        object RB_AllRegPatient: TRadioButton
          Left = 8
          Top = 5
          Width = 129
          Height = 17
          Caption = 'All Registered Patient'
          Checked = True
          TabOrder = 0
          TabStop = True
        end
        object RB_OutStdPatient: TRadioButton
          Left = 8
          Top = 25
          Width = 175
          Height = 17
          Caption = 'Admitted(OutStd.) Patient Only'
          TabOrder = 1
        end
      end
      object Edit_PatientID: TEdit
        Left = 0
        Top = 77
        Width = 86
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 8
        OnChange = Edit_PatientIDChange
      end
      object Edit_CareOf: TEdit
        Left = 734
        Top = 77
        Width = 89
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 18
        OnChange = Edit_CareOfChange
      end
      object Panel5: TPanel
        Left = 898
        Top = 1
        Width = 206
        Height = 98
        Align = alRight
        TabOrder = 0
        DesignSize = (
          206
          98)
        object BitBtnOutPt: TBitBtn
          Left = -4
          Top = 1
          Width = 112
          Height = 25
          Hint = 'Out-Patient Analysis'
          Anchors = [akTop, akRight]
          Caption = 'Out-Patient (F3)'
          DoubleBuffered = True
          Enabled = False
          Glyph.Data = {
            
            3333777777333333333333333333333333333333333333333333}
          NumGlyphs = 2
          ParentDoubleBuffered = False
          ParentShowHint = False
          ShowHint = True
          TabOrder = 0
          Visible = False
          OnClick = BitBtnOutPtClick
        end
        object BitBtn8: TBitBtn
          Left = 110
          Top = 1
          Width = 94
          Height = 25
          Hint = 'Patient Analysis'
          Anchors = [akTop, akRight]
          Caption = '&Analysis           '
          DoubleBuffered = True
          Glyph.Data = {
            
            3333777777333333333333333333333333333333333333333333}
          NumGlyphs = 2
          ParentDoubleBuffered = False
          ParentShowHint = False
          ShowHint = True
          TabOrder = 1
          Visible = False
        end
        object BitBtnInpatient: TBitBtn
          Left = -1
          Top = 26
          Width = 108
          Height = 25
          Hint = 'In-Patient Analysis'
          Anchors = [akTop, akRight]
          BiDiMode = bdLeftToRight
          Caption = 'In-Patient   (F4)'
          DoubleBuffered = True
          Glyph.Data = {
            
            3333777777333333333333333333333333333333333333333333}
          NumGlyphs = 2
          ParentBiDiMode = False
          ParentDoubleBuffered = False
          ParentShowHint = False
          ShowHint = True
          TabOrder = 2
          Visible = False
          OnClick = BitBtnInpatientClick
        end
        object BitBtnMLC: TBitBtn
          Left = 110
          Top = 27
          Width = 94
          Height = 25
          Hint = 'MLC'
          Anchors = [akTop, akRight]
          Caption = 'MLC        (F10)'
          DoubleBuffered = True
          Glyph.Data = {
            
            3333777777333333333333333333333333333333333333333333}
          NumGlyphs = 2
          ParentDoubleBuffered = False
          ParentShowHint = False
          ShowHint = True
          TabOrder = 3
          Visible = False
        end
      end
      object CB_GlobalSearch: TCheckBox
        Left = 11
        Top = 53
        Width = 48
        Height = 20
        Caption = '&Global'
        TabOrder = 6
        OnClick = CB_GlobalSearchClick
      end
      object Edit_FileNo: TEdit
        Left = 89
        Top = 77
        Width = 72
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 9
        OnChange = Edit_FileNoChange
      end
      object CB_ByCRNo: TCheckBox
        Left = 69
        Top = 53
        Width = 72
        Height = 20
        Caption = 'By CR No.'
        Enabled = False
        TabOrder = 22
        Visible = False
        OnClick = CB_GlobalSearchClick
      end
      object Edit1: TEdit
        Tag = 12
        Left = 544
        Top = 7
        Width = 12
        Height = 12
        TabStop = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Color = -1
        TabOrder = 23
        Visible = False
      end
      object Edit11: TEdit
        Left = 544
        Top = 41
        Width = 12
        Height = 12
        TabStop = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Color = clRed
        TabOrder = 24
        Visible = False
      end
      object Edit5: TEdit
        Left = 544
        Top = 58
        Width = 12
        Height = 12
        TabStop = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Color = 12718764
        TabOrder = 25
        Visible = False
      end
      object Edit_FirstName: TEdit
        Left = 89
        Top = 77
        Width = 103
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 10
        OnChange = Edit_FirstNameChange
      end
      object Edit_LastName: TEdit
        Left = 193
        Top = 77
        Width = 93
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 11
        OnChange = Edit_LastNameChange
      end
      object Edit_AgeSex: TEdit
        Left = 288
        Top = 77
        Width = 63
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 12
        OnChange = Edit_AgeSexChange
      end
      object Edit_DepCode: TEdit
        Left = 352
        Top = 77
        Width = 53
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 13
        OnChange = Edit_DepCodeChange
      end
      object Edit_LastVisitDate: TEdit
        Left = 406
        Top = 77
        Width = 59
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 14
        OnChange = Edit_LastVisitDateChange
      end
      object Edit_VisitTime: TEdit
        Left = 467
        Top = 77
        Width = 47
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 15
        OnChange = Edit_VisitTimeChange
      end
      object Edit_Address: TEdit
        Left = 515
        Top = 77
        Width = 150
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 16
        OnChange = Edit_AddressChange
      end
      object Edit_District: TEdit
        Left = 667
        Top = 77
        Width = 66
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 17
        OnChange = Edit_DistrictChange
      end
      object Edit_MobileNo: TEdit
        Left = 825
        Top = 77
        Width = 90
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 19
        OnChange = Edit_MobileNoChange
      end
      object Edit_PhoneNo: TEdit
        Left = 918
        Top = 77
        Width = 75
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 20
        OnChange = Edit_PhoneNoChange
      end
      object BTN_CARD: TBitBtn
        Left = 712
        Top = 24
        Width = 105
        Height = 33
        Caption = 'Card Print BACK'
        DoubleBuffered = True
        ParentDoubleBuffered = False
        TabOrder = 26
        OnClick = BTN_CARDClick
      end
    end
    object Panel4: TPanel
      Left = 1
      Top = 101
      Width = 1143
      Height = 381
      Align = alClient
      Anchors = [akTop, akRight, akBottom]
      Caption = 'Panel4'
      TabOrder = 1
      object DBGrid1: TDBGrid
        Left = 1
        Top = 1
        Width = 1141
        Height = 379
        Align = alClient
        Anchors = [akTop, akRight, akBottom]
        Color = clWhite
        DataSource = DS_List
        DrawingStyle = gdsGradient
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnCellClick = DBGrid1CellClick
        OnDrawColumnCell = DBGrid1DrawColumnCell
        OnDblClick = DBGrid1DblClick
        Columns = <
          item
            Expanded = False
            Visible = False
          end
          item
            Expanded = False
            FieldName = 'sn'
            Title.Alignment = taCenter
            Title.Caption = 'S.N'
            Width = 33
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'PATIENTID'
            Title.Alignment = taCenter
            Title.Caption = 'Hosp. No'
            Width = 73
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FILENO'
            Title.Alignment = taCenter
            Title.Caption = 'CR No.'
            Visible = False
          end
          item
            Expanded = False
            FieldName = 'ERPATIENTCATEGORY'
            Title.Caption = 'Category'
            Visible = False
          end
          item
            Expanded = False
            FieldName = 'FNAME'
            Title.Alignment = taCenter
            Title.Caption = 'First Name'
            Width = 103
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'LNAME'
            Title.Alignment = taCenter
            Title.Caption = 'Last Name'
            Width = 93
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CURRENTAGEGENDER'
            Title.Alignment = taCenter
            Title.Caption = 'Age/Sex'
            Width = 65
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DEPCODE'
            Title.Alignment = taCenter
            Title.Caption = 'L.V.Dep.'
            Width = 52
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VISITDATE'
            Title.Alignment = taCenter
            Title.Caption = 'L.Visit Date'
            Width = 59
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VISITTIME2ND'
            Title.Alignment = taCenter
            Title.Caption = 'V. Time'
            Width = 48
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FullAddress'
            Title.Alignment = taCenter
            Title.Caption = 'Address'
            Width = 151
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DISTRICTNAME'
            Title.Caption = 'District'
            Width = 65
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NEXTTOKIN'
            Title.Alignment = taCenter
            Title.Caption = 'Care of '
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'MOBILENO'
            Title.Alignment = taCenter
            Title.Caption = 'Mobile No'
            Width = 93
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'PHONENO'
            Title.Alignment = taCenter
            Title.Caption = 'Phone No'
            Width = 75
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'REGDATE'
            Title.Caption = 'Reg. Date'
            Width = 65
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VISITCOUNTTOTAL'
            Title.Alignment = taCenter
            Title.Caption = 'V. Count'
            Width = 47
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'COUNTRYNAME'
            Title.Alignment = taCenter
            Title.Caption = 'Country'
            Width = 65
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DOBAD'
            Title.Alignment = taCenter
            Title.Caption = 'DOB(AD)'
            Width = 65
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'EMAIL'
            Title.Alignment = taCenter
            Title.Caption = 'Email'
            Width = 99
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'EMERGENCYNO'
            Title.Caption = 'Emr. No'
            Visible = False
          end
          item
            Expanded = False
            FieldName = 'DischargeType'
            Title.Caption = 'Discharge Type'
            Visible = False
          end>
      end
      object DBGrid2: TDBGrid
        Left = 99
        Top = 200
        Width = 992
        Height = 120
        DataSource = ds_excel
        TabOrder = 1
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Visible = False
        Columns = <
          item
            Expanded = False
            FieldName = 'SN'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'HospitalNo'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FNAME'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'LNAME'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'AGEGENDER'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VISITCOUNT'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ADDRESS'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DISTRICTNAME'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'MOBILENO'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'PHONENO'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'REGDATE'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VISITCOUNTTOTAL'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'COUNTRYNAME'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DOBAD'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'EMAIL'
            Visible = True
          end>
      end
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1145
    Height = 41
    Align = alTop
    Color = 16771304
    ParentBackground = False
    TabOrder = 0
    object Sb_Queuelist: TSpeedButton
      AlignWithMargins = True
      Left = 504
      Top = 4
      Width = 81
      Height = 30
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = 'Queue List'
      Enabled = False
      Visible = False
      OnClick = Sb_QueuelistClick
      ExplicitLeft = 248
      ExplicitTop = 0
    end
    object SPBPatientInOut: TSpeedButton
      AlignWithMargins = True
      Left = 845
      Top = 4
      Width = 99
      Height = 30
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = 'Patient In/Out'
      Enabled = False
      Visible = False
      OnClick = SPBPatientInOutClick
      ExplicitLeft = 688
    end
    object SB_CancelReg: TSpeedButton
      AlignWithMargins = True
      Left = 423
      Top = 4
      Width = 81
      Height = 30
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = 'Cancel Reg.'
      Enabled = False
      Visible = False
      OnClick = SB_CancelRegClick
      ExplicitLeft = 248
      ExplicitTop = 6
    end
    object SPB_OPDCount: TSpeedButton
      AlignWithMargins = True
      Left = 585
      Top = 4
      Width = 74
      Height = 30
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = 'OPD Count'
      Enabled = False
      Visible = False
      OnClick = SPB_OPDCountClick
      ExplicitLeft = 449
    end
    object SPBServiceBillDetail: TSpeedButton
      AlignWithMargins = True
      Left = 944
      Top = 4
      Width = 89
      Height = 30
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = 'Service Detail'
      Enabled = False
      Visible = False
      OnClick = SPBServiceBillDetailClick
      ExplicitLeft = 808
    end
    object SPB_ServiceAcknowledge: TSpeedButton
      AlignWithMargins = True
      Left = 1033
      Top = 4
      Width = 111
      Height = 30
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = 'Service Acknowledge'
      Enabled = False
      Visible = False
      OnClick = SPB_ServiceAcknowledgeClick
      ExplicitLeft = 898
      ExplicitTop = 13
      ExplicitHeight = 28
    end
    object SPB_Investigation: TSpeedButton
      AlignWithMargins = True
      Left = 297
      Top = 4
      Width = 126
      Height = 30
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = 'Investigation Reports'
      Enabled = False
      Visible = False
      OnClick = SPB_InvestigationClick
      ExplicitLeft = 50
    end
    object BtnDoctorVisit: TBitBtn
      AlignWithMargins = True
      Left = 659
      Top = 4
      Width = 85
      Height = 30
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = 'Doctor Visit'
      DoubleBuffered = True
      Enabled = False
      ParentDoubleBuffered = False
      TabOrder = 0
      Visible = False
      OnClick = BtnDoctorVisitClick
    end
    object BtnServiceComplete: TBitBtn
      AlignWithMargins = True
      Left = 744
      Top = 4
      Width = 101
      Height = 30
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = 'Service Complete'
      DoubleBuffered = True
      Enabled = False
      ParentDoubleBuffered = False
      TabOrder = 1
      Visible = False
      OnClick = BtnServiceCompleteClick
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 41
    Width = 1145
    Height = 41
    Align = alTop
    Color = 16771304
    ParentBackground = False
    TabOrder = 1
    object SPBUploadPrescription: TSpeedButton
      Left = 1
      Top = 1
      Width = 99
      Height = 39
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alLeft
      Caption = '&Image Upload'
      Glyph.Data = {
        
        3333777777333333333333333333333333333333333333333333}
      NumGlyphs = 2
      Visible = False
      OnClick = SPBUploadPrescriptionClick
    end
    object SPBUndoDischarge: TSpeedButton
      AlignWithMargins = True
      Left = 847
      Top = 4
      Width = 126
      Height = 30
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = 'Undo Discharge [ER]'
      Enabled = False
      Visible = False
      OnClick = SPBUndoDischargeClick
      ExplicitLeft = 884
    end
    object SPB_EmrDutyDocConsultant: TSpeedButton
      AlignWithMargins = True
      Left = 681
      Top = 4
      Width = 166
      Height = 30
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = 'EMR.Duty Doctor/ Consultant'
      Enabled = False
      Visible = False
      OnClick = SPB_EmrDutyDocConsultantClick
      ExplicitLeft = 664
    end
    object SB_HospitalVisit: TSpeedButton
      Left = 973
      Top = 1
      Width = 171
      Height = 39
      Align = alRight
      Caption = 'Follow-Up Visit  (F2)'
      Enabled = False
      Glyph.Data = {
        
        EE33337FFFFFFFFFFF7F33EEEEEEEEEEEE333377777777777773}
      NumGlyphs = 2
      Visible = False
      OnClick = SB_HospitalVisitClick
      ExplicitLeft = 969
      ExplicitTop = 2
    end
    object SB_ChangeDepartment: TSpeedButton
      Left = 236
      Top = 1
      Width = 137
      Height = 39
      Align = alLeft
      Caption = 'Change Doc. &&  Dept. (F9)'
      Enabled = False
      Visible = False
      OnClick = SB_ChangeDepartmentClick
    end
    object SPB_PatientAdmn: TSpeedButton
      Left = 373
      Top = 1
      Width = 113
      Height = 39
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alLeft
      Caption = 'Patient Admission'
      Enabled = False
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      NumGlyphs = 2
      Visible = False
      OnClick = SPB_PatientAdmnClick
    end
    object SPB_DayCareReg: TSpeedButton
      Left = 758
      Top = 1
      Width = 111
      Height = 39
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alLeft
      Caption = 'Day Care Reg.'
      Glyph.Data = {
        
        5B5557555557F55557555555555B555555555555555755555555}
      NumGlyphs = 2
      Visible = False
      OnClick = SPB_DayCareRegClick
      ExplicitLeft = 647
    end
    object SPB_VisitDetail: TSpeedButton
      Left = 486
      Top = 1
      Width = 161
      Height = 39
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alLeft
      Caption = 'Set Scheme/Visit Detail (F10)'
      Enabled = False
      Glyph.Data = {
        
        00337777FFFF77FF7733EEEE0000000003337777777777777333}
      NumGlyphs = 2
      Visible = False
      OnClick = SPB_VisitDetailClick
    end
    object SPB_NewCRNo: TSpeedButton
      Left = 647
      Top = 1
      Width = 111
      Height = 39
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alLeft
      Caption = 'Get CR No. (F11)'
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      NumGlyphs = 2
      Visible = False
      OnClick = SPB_NewCRNoClick
      ExplicitLeft = 655
    end
    object BitBtnRefund: TBitBtn
      Left = 100
      Top = 1
      Width = 136
      Height = 39
      Hint = 'Registration Refund'
      Align = alLeft
      Caption = 'Refund/Re-Print    (F8)'
      DoubleBuffered = True
      Enabled = False
      Glyph.Data = {
        
        3333777777333333333333333333333333333333333333333333}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      Visible = False
      OnClick = BitBtnRefundClick
    end
    object BitBtn_DocVisit: TBitBtn
      Left = 869
      Top = 1
      Width = 104
      Height = 39
      Margins.Left = 0
      Margins.Right = 0
      Margins.Bottom = 6
      Caption = 'Doctor Visit'
      DoubleBuffered = True
      Enabled = False
      ParentDoubleBuffered = False
      TabOrder = 1
      Visible = False
      OnClick = BitBtn_DocVisitClick
    end
  end
  object QueryBlank: TOraQuery
    Left = 232
    Top = 232
  end
  object DS_List: TDataSource
    DataSet = QueryList
    Left = 584
    Top = 232
  end
  object QueryList: TOraQuery
    SQL.Strings = (
      'select rownum as sn, PA.* from VW_HS_PATIENTMAIN_ALL PA'
      'WHERE 99=99'
      'AND 99=99'
      'and VisitDate='#39'2015/03/26'#39
      'ORDER BY REGDATE DESC,REGTIME DESC')
    FetchAll = True
    AfterOpen = QueryListAfterOpen
    AfterScroll = QueryListAfterScroll
    Left = 712
    Top = 248
  end
  object Timer_CheckBillingClosed: TTimer
    Enabled = False
    Interval = 1
    Left = 528
    Top = 312
  end
  object Timer_ChkNewPatientClosed: TTimer
    Enabled = False
    Interval = 1
    Left = 488
    Top = 264
  end
  object Twain: TDelphiTwain
    TransferMode = ttmMemory
    SourceCount = 0
    Info.MajorVersion = 1
    Info.MinorVersion = 0
    Info.Language = tlUserLocale
    Info.CountryCode = 1
    Info.Groups = [tgControl, tgImage]
    Info.VersionInfo = 'A'
    Info.Manufacturer = 'A'
    Info.ProductFamily = 'A'
    Info.ProductName = 'A'
    LibraryLoaded = False
    SourceManagerLoaded = False
    Left = 800
    Top = 304
  end
  object Query_Dep: TOraQuery
    SQL.Strings = (
      'Select DEPT_DEPCODE,DEPT_DEPNAME FROM HS_DEPT_Department'
      'where 1=1')
    Left = 384
    Top = 248
  end
  object DS_Dep: TDataSource
    DataSet = Query_Dep
    Left = 352
    Top = 312
  end
  object SavePictureDialog1: TSavePictureDialog
    Left = 640
    Top = 272
  end
  object OpenPictureDialog1: TOpenPictureDialog
    Left = 800
    Top = 296
  end
  object Queryexcel: TOraQuery
    Session = DM_Hospital.DB
    FetchAll = True
    Left = 920
    Top = 112
  end
  object ds_excel: TDataSource
    DataSet = Queryexcel
    Left = 872
    Top = 216
  end
  object Query_patientmain: TOraQuery
    Session = DM_Hospital.DB
    Left = 840
    Top = 32
  end
end
