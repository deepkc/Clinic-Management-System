object Form_UserRectification: TForm_UserRectification
  Left = 312
  Top = 174
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = 'Patient Rectification'
  ClientHeight = 298
  ClientWidth = 751
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poMainFormCenter
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 751
    Height = 39
    Align = alTop
    TabOrder = 0
    object BB_Save: TBitBtn
      AlignWithMargins = True
      Left = 553
      Top = 4
      Width = 103
      Height = 31
      Margins.Left = 0
      Margins.Right = 0
      Align = alRight
      Caption = 'Save (F12)'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 0
      OnClick = BB_SaveClick
    end
    object BB_Close: TBitBtn
      AlignWithMargins = True
      Left = 656
      Top = 4
      Width = 94
      Height = 31
      Margins.Left = 0
      Margins.Right = 0
      Align = alRight
      Caption = 'Close (Esc.)'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 1
      OnClick = BB_CloseClick
    end
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 279
    Width = 751
    Height = 19
    Panels = <
      item
        Width = 50
      end
      item
        Text = 'F12 -> Save'
        Width = 50
      end>
  end
  object Panel2: TPanel
    Left = 0
    Top = 41
    Width = 751
    Height = 242
    Align = alCustom
    BevelOuter = bvLowered
    TabOrder = 2
    object Label1: TLabel
      Left = 44
      Top = 127
      Width = 66
      Height = 13
      Alignment = taRightJustify
      Caption = 'Prev. Doctor :'
    end
    object Label3: TLabel
      Left = 74
      Top = 78
      Width = 36
      Height = 13
      Alignment = taRightJustify
      Caption = 'Bill No :'
    end
    object Label4: TLabel
      Left = 62
      Top = 207
      Width = 48
      Height = 13
      Alignment = taRightJustify
      Caption = 'Remarks :'
    end
    object Label5: TLabel
      Left = 39
      Top = 180
      Width = 71
      Height = 13
      Alignment = taRightJustify
      Caption = 'Actual Doctor :'
    end
    object Label6: TLabel
      Left = 41
      Top = 104
      Width = 69
      Height = 13
      Alignment = taRightJustify
      Caption = 'Prev. Hospital:'
    end
    object Label7: TLabel
      Left = 32
      Top = 157
      Width = 77
      Height = 13
      Alignment = taRightJustify
      Caption = 'Actual Hospital :'
    end
    object Label8: TLabel
      Left = 45
      Top = 44
      Width = 64
      Height = 13
      Alignment = taRightJustify
      Caption = 'Hosptial No. :'
    end
    object Label_PatientName: TLabel
      Left = 248
      Top = 45
      Width = 77
      Height = 13
      Caption = 'Patient Name'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object DBLCB_PrevDoctor: TDBLookupComboBox
      Left = 114
      Top = 123
      Width = 347
      Height = 21
      KeyField = 'Docid'
      ListField = 'DOCNAME'
      ListSource = DS_PrevDoctor
      TabOrder = 2
      OnKeyPress = DBLCB_PrevDoctorKeyPress
    end
    object EditRemarks: TEdit
      Left = 114
      Top = 203
      Width = 347
      Height = 21
      TabOrder = 5
    end
    object DBLCB_ActualDoc: TDBLookupComboBox
      Left = 114
      Top = 177
      Width = 347
      Height = 21
      KeyField = 'docid'
      ListField = 'DOCNAME'
      ListSource = Ds_ActualDoc
      TabOrder = 4
      OnKeyPress = DBLCB_ActualDocKeyPress
    end
    object DBLCB_PrevHos: TDBLookupComboBox
      Left = 114
      Top = 100
      Width = 347
      Height = 21
      KeyField = 'HOSE_HOSPITALCODE'
      ListField = 'HOSE_HOSPITALNAME'
      ListSource = DS_Hospital
      TabOrder = 1
      OnKeyPress = DBLCB_PrevHosKeyPress
    end
    object DBLCB_ActualHos: TDBLookupComboBox
      Left = 114
      Top = 153
      Width = 347
      Height = 21
      KeyField = 'Hose_HOSPITALCODE'
      ListField = 'hose_HOSPITALNAME'
      ListSource = Ds_ActualHos
      TabOrder = 3
      OnClick = DBLCB_ActualHosClick
      OnKeyPress = DBLCB_ActualHosKeyPress
    end
    object Edit_PatientId: TEdit
      Left = 114
      Top = 41
      Width = 121
      Height = 21
      NumbersOnly = True
      TabOrder = 0
      OnKeyPress = Edit_PatientIdKeyPress
    end
    object EditBillNo: TEdit
      Left = 114
      Top = 72
      Width = 144
      Height = 21
      TabOrder = 6
      OnKeyPress = EditBillNoKeyPress
    end
  end
  object DS_PrevDoctor: TDataSource
    DataSet = OraQuery_PrevDoctor
    Left = 264
    Top = 8
  end
  object Query_ActDoctor: TQuery
    DatabaseName = 'Hospital'
    SQL.Strings = (
      'Select * from HS_DOCT_Doctor where DOCT_ISActive='#39'Y'#39'  '
      '     and 0=0'
      '     Order By DOCT_DocName')
    Left = 492
    Top = 392
  end
  object DS_ActulaDoctor: TDataSource
    DataSet = Query_ActDoctor
    Left = 544
    Top = 408
  end
  object Query_Process: TQuery
    DatabaseName = 'Hospital'
    Left = 312
    Top = 333
  end
  object OraQuery_Void: TOraQuery
    Left = 136
  end
  object OraQuery_PrevDoctor: TOraQuery
    SQL.Strings = (
      
        'select REDO_DOCID as DocId,REDO_DOCNAME as DOCNAME from hs_redo_' +
        'referaldoctor'
      'order by REDO_DOCNAME')
    Left = 352
    Top = 48
  end
  object OraQuery_Process: TOraQuery
    Left = 552
    Top = 128
  end
  object OraQuery_Hospital: TOraQuery
    SQL.Strings = (
      'select * from hs_hose_hospitalsetup'
      'order by HOSE_HOSPITALNAME')
    Left = 424
    Top = 56
  end
  object DS_Hospital: TDataSource
    DataSet = OraQuery_Hospital
    Left = 480
  end
  object OraQuery_ActualHos: TOraQuery
    SQL.Strings = (
      'select * from hs_hose_hospitalsetup'
      'order by HOSE_HOSPITALNAME')
    Left = 520
    Top = 184
  end
  object OraQuery_ActualDoctor: TOraQuery
    SQL.Strings = (
      
        'select REDO_DOCID as DocId,REDO_DOCNAME as DOCNAME from hs_redo_' +
        'referaldoctor'
      'order by REDO_DOCNAME')
    Left = 664
    Top = 104
  end
  object Ds_ActualDoc: TDataSource
    DataSet = OraQuery_ActualDoctor
    Left = 688
    Top = 216
  end
  object Ds_ActualHos: TDataSource
    DataSet = OraQuery_ActualHos
    Left = 608
    Top = 184
  end
  object OraQuery_SaveLog: TOraQuery
    Left = 552
    Top = 56
  end
end
