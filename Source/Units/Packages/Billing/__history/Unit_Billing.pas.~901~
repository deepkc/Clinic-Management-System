...
uses
  ...
  Fxn, dm, serverdate, Unit_Master_Hos, PatientClass, Unit_Deposits,
   Unit_DischargeBilling,
  ...
  Unit_otherpatient, OleCtrls, DateEditXControl_TLB, ComCtrls;
...
type
  TForm_Billing = class(TForm)
    ...
    CB_DelAll: TCheckBox;
    Btn_KeepPending: TSpeedButton;
    Query_TempProcess: TQuery;
    Table_Billing: TTable;
    Table_Fraction: TTable;
    Table_TempBilling: TTable;
    Table_MKTFraction: TTable;
    Table_PendingBills: TTable;
    Table_PendingFraction: TTable;
    ...
    Query_CheckBilldate: TOraQuery;
    lbl1: TLabel;
    lbl_memtype: TLabel;
    Query_patientmain: TOraQuery;
    tmr1: TTimer;
    ...
    procedure DBLCB_HospitalKeyPress(Sender: TObject; var Key: Char);
    ...
    procedure CB_DelAllClick(Sender: TObject);
    procedure Btn_KeepPendingClick(Sender: TObject);
    procedure Btn_BringPendingClick(Sender: TObject);
    procedure DBLCB_HospitalClick(Sender: TObject);
    procedure DBLCB_RefDocCodeExit(Sender: TObject);
    procedure DBLCB_HospitalExit(Sender: TObject);
  ...
  private
    ...
    ps_isCopaymentitem, ps_CopayItemsCodeList, ps_SampleNo, ps_DocCode, ps_RefundType,
    ...
    pb_IsFinalSelectedTestName, pb_IsTestListGridDblClick: Boolean;
    ...
    Procedure SaveInvAdvanceDeposit(DrAmount, CrAmount: Double;
      DepositRemarks,IsServiceCancel: String);
    Procedure PrintDepositSlip;
    Procedure DepartmentWiseBillPrint;
  public
    ProgressStatus, Pi_ClinicalDepId, Pi_DaycareDocId,pi_patientid,pi_refdocid: Integer;
    Pb_IsPathoDep, pb_PharmacyRefundOnly,IsCreditParty: Boolean;
    pf_DiscountPer:Double;
    ps_IsDependent,ps_MemberNo,ps_refhoscode,Ps_SampleType,ps_RefundBillNo:String;

    pb_IsOPUserDoTPBillAfterCreditAppForInpt : Boolean;
    { Public declarations }
    Procedure LoadBill;
    ...
    Procedure CreateTempPendingBill;
    ...
    Function IsErAndCreditMemoRegPatient : Boolean;
...
uses Unit_BillingParent, Unit_QrBill, Unit_QRDischargeBill, Unit_TestList,
  Unit_CustomerMonitor, Unit_BringPendingBill,
  Unit_BillDetail, Unit_DrInvolveInOperation, Unit_QrBill_Manipal,Unit_StaffMemberSearch,
  Unit_QrDeptwiseBillPrint, Unit_QrBill_PPL,Unit_QrBill_GMC, Unit_QrBill_nozomi;
{$R *.dfm}
{ TForm_Billing }
...
var
  ...
  ls_FinalBillNo, ls_RefBillNo, ls_DepositBillNo, ls_BillType,ls_refhoscode: String;
  Key: Char;
  MyTable: TOraTable;
  MytableName: String;
  Ps_RefDocCode: String;
  Qry: TOraQuery;
var Oraqeury1:TOraQuery;
  //Qry1:ToraQuery;
begin
     // gi_billcase
     ...
     begin
          if  (gs_BillType <> 'SERVICE REFUNDBILL')  then
          begin
             {  IF DBLCB_Hospital.KeyValue=null  then
               begin
                    ShowMessage('Hospital Name Cannot be Null');
                    DBLCB_Hospital.setfocus;
                    exit;
               end;    }
                IF (DBLCB_RefDocCode.KeyValue=null) or (DBLCB_RefDocCode.KeyValue=0)  then
               begin
                    ShowMessage('Referral Doctor Cannot be Null');
     ...
     Begin
          ...
          pi_CommunityID := QueryScheme.FieldByName('SCHE_CommunityId').AsInteger;
     End;

     if Trim(DBLCB_Hospital.Text)='' then
     begin
          ps_refhoscode:='';
     end
     else
     begin
          ps_refhoscode:=DBLCB_Hospital.KeyValue;
     end;
     ...
     begin
          pi_refdocid:=DBLCB_RefDocCode.KeyValue;
     end;

     //
    { Oraqeury1:=TOraQuery.Create(nil);
     with Oraqeury1 do
     begin
          close;
          Session:=DM_Hospital.DB;
          sql.Clear;
          sql.Add('insert into patienttest (refdocid,refhoscode) values('+intTOStr(li_refdocid)+','+QuotedStr(ls_refhoscode)+')');
    ...
    //
     ...
     if (RB_ERBill.Checked=True) then
     Begin
          if DBLCB_Scheme.KeyValue<=0 then
          begin
               MessageDlg('Please Provide Credit Memo Facility For ( '+IntToStr(gi_PatientId)+' ) .',mtWarning,[mbok],0);
               Exit;
          end;
          if IsErAndCreditMemoRegPatient=False then
          Begin
               MessageDlg(' Sorry ! This Is Not a Emergency Or Credit Memo Registered Patient.',mtWarning,[mbok],0);
               Exit;
          End;
     End;
     ...
     End
     Else if (gs_BillType = 'OPBILL') or (gs_BillType = 'TPBILL') then
     begin
          ...
          if (gi_compileValue in [14,21,22,23,24,25,26,27,28]) AND (gs_BillType = 'OPBILL') AND (gs_BillType<>'TPBILL') then     //
          begin
               if (Trim(DBLCB_Scheme.Text)='') AND (CB_PayType.ItemIndex=1) then
               begin
...
//                    DBLCB_Scheme.setfocus;
          ...
          if Query_TempProcess.FieldByName('DOcId').AsInteger > 1 then
          begin
               Pb_IsDayCare := True;
               Pi_DaycareDocId := Query_TempProcess.FieldByName('DOcId').AsInteger;
          end
          else
          begin
               Pb_IsDayCare := False;
     ...
     Else if (gs_BillType = 'REFUNDBILL') or (gs_BillType='SERVICE REFUNDBILL') then
     begin
           if (CB_PayType.ItemIndex = 1) AND (gs_BillType<>'SERVICE REFUNDBILL') then
          begin
               if Trim(DBLCB_Scheme.Text)='' then
               begin
                    MessageDlg('For Credit You Must Have to Mention Credit Scheme(Party)',mtWarning,[mbOK],0);
     ...
     Gb_IsReprint:=False;
     if (gs_BillType = 'REFUNDBILL') or (gs_BillType='SERVICE REFUNDBILL') then
     begin
          ...
          if (Query_RefundBillList.FieldByName('BILLTYPE').AsString = 'FINALBILL' ) AND (gs_BillType<>'SERVICE REFUNDBILL') then
          Begin
                              ...
                              begin
                                   ...
                                   begin
                                        try
                                             ...
                                             Table_Billing.FieldByName('Discount').AsFloat,Le_Remarks.Text, 'N',ps_TodaysDate,ps_TodaysTime);
                                        except
                                             try
                                                  SaveRefundDetail(Table_Billing.FieldByName('BillDetailID').AsInteger, Table_Billing.FieldByName('Qty').AsFloat,
                                                  Table_Billing.FieldByName('TestPrice').AsFloat,Table_Billing.FieldByName('SvrTax').AsFloat,
                                   ...
                                   Begin
                                        ...
                                        try
                                             ...
                                             Le_Remarks.Text, 'Y',ps_TodaysDate,ps_TodaysTime);
                                        except
                                             try
                                                  SaveRefundDetail(Table_Billing.FieldByName('BillDetailID')
                                                  .AsInteger, Table_Billing.FieldByName('Qty').AsFloat,
                                   ...
                                   begin
                                        ...
                                        begin
                                             SaveDeposit(0, gi_InPatientID, gi_PatientID, 0, Gi_UserId, 0,gf_TotalRefundAmount, 0, 'DEPOSIT', 'OP DEPOSIT REFUND',
                                                  Gs_BillNo, '', Table_Billing.FieldByName('TestNameCode').AsString, Ps_TodaysDate, Ps_TodaysTime, 'N',ps_PatientCategory,'N','','N','N');//GetPatientCagetory(gi_PatientID)
          ...
          Begin
               ...
               try
                    ...
                    begin
                         ...
                         begin
                              CancelServiceTest(Table_Billing.FieldByName('ServiceBillDetailId')
                                        .AsInteger, 0,Table_Billing.FieldByName('Discount').AsFloat, Le_Remarks.Text);
                              if Table_Billing.FieldByName('ServiceBillDetailId').asString<>'' then
                              begin
                                   if gs_servicebilldetailid = '' then
                                   gs_servicebilldetailid:=Table_Billing.FieldByName('ServiceBillDetailId').asString
                                   else
                                   gs_servicebilldetailid:= gs_servicebilldetailid + ',' +
                                   Table_Billing.FieldByName('ServiceBillDetailId').asString;
                              end;
                         end;
                    ...
                    Begin
                         Gs_BillNo := Edit_PreveBillNo.Text;
                         SaveInvAdvanceDeposit(0, pf_AdvancRefund, Le_Remarks.Text,'Y');
               ...
               begin
                    ...
                    Begin
                         ...
                         Try
                             // gs_BillType := 'IPBILL';
                              gs_BillType := 'SERVICE REFUNDBILL';
                              PrintBill(CB_Preview, 1);
               ...
               Begin
                    ...
                    gi_BillCase := 2;
                    gi_noOfPrint := 0;
                    gs_billtype:='SERVICE REFUNDBILL FINAL';
                    PrintBill(CB_Preview, 1);
                    Gs_BillNo := ls_RefBillNo;
               End;
     ...
     begin
          ...
          begin
               if (IsRemarksCompulsory(DBLCB_Scheme.KeyValue)=True) and (Trim(Le_Remarks.Text)='') then
               begin
                    MessageDlg(' Please provide remarks', mtWarning, [MbOk], 0);
                    Le_Remarks.SetFocus;
                    Exit;
               end;
          end;
         ...
         { if CB_DayCare.Checked = True then
          begin
               ...
               begin
                    ...
                    Session:=DM_Hospital.DB;
                    sql.Clear;
                    sql.Add('select HORU_ISADVANCESYSTEMFORIPBILL from HS_HORU_HOSPITALRULES');
                    open;
               end;
               if Trim(Qry1.FieldByName('HORU_ISADVANCESYSTEMFORIPBILL').AsString)='Y' then
               begin
                    if pf_Deposit > 0 then
                         SaveDeposit(0, gi_InPatientID, gi_PatientID, 0, Gi_UserId, 0, 0,pf_grandtotal, 'ADVANCE RECEIPT',
                              'ADJ. IN OP', Gs_BillNo, '','', Ps_TodaysDate, Ps_TodaysTime, 'N',ps_PatientCategory,'N','','N');//GetPatientCagetory(gi_PatientID)
               end;
          end; }
          //
     ...
     begin
          ...
          if CB_DayCare.Checked = True then
          begin
               if (pf_Deposit < pf_grandtotal) and (gs_CalledFrom<>'FinalBilling') then //and (pf_Deposit < pf_grandtotal) then
               begin
          ...
          Try
               ...
               gi_InPatientID := 0;
               if (gi_BillCase = 3) and (gs_CalledFrom<>'FinalBilling')  then
               begin
               ...
               begin
                    if pf_Deposit > 0 then
                         SaveDeposit(0, gi_InPatientID, gi_PatientID, 0, Gi_UserId, 0, 0,pf_Deposit, 'ADVANCE RECEIPT',
                              'ADJ. IN OP', Gs_BillNo, '','', Ps_TodaysDate, Ps_TodaysTime, 'N',ps_PatientCategory,'N','','N','N');//GetPatientCagetory(gi_PatientID)
               end;
     ...
     if pb_IsOPUserDoTPBillAfterCreditAppForInpt=True then
     Begin
          RB_OPBILL.Checked:=True;
          RB_OPBILL.Visible:=True;
          gs_BillType:= 'OPBILL';
          Label_BBImageCap.Visible:=False;
          pb_IsOPUserDoTPBillAfterCreditAppForInpt:=False;
     End;
     ...
     if (gs_BillType='IPBILL') or (gs_BillType='TPBILL')or (gs_BillType='ERBILL') then
     gi_BillCase:=0;
...
begin
  ...
  begin
    ...
    Le_Payment.Enabled := False;
     Label_PayTypeFlash.Caption:='CREDIT';
    if RB_OPBILL.Checked = True then
  ...
  begin
    ...
    Le_Payment.Enabled := True;
      Label_PayTypeFlash.Caption:='CASH';
  ...
  Begin
    ...
    begin
          ...
          Begin
               ...
               SQL.Add(' (Select UserName From lab_UserMain Where UserId=BIDE_BillBy) UserName');
              ...
              // SQL.SaveToFile('C:\abc445.txt');
               Open;
          End;

    end
    Else if (gs_BillType = 'IPBILL') or (gs_BillType = 'TPBILL') or (gs_BillType='ERBILL') then
    begin
      With Query_BillList do
      Begin
        Close;
        ...
        SQL.Add(' (Select UserName From lab_UserMain Where UserId=SEBD_BillBy) UserName');
        SQL.Add(' From HS_SEBD_ServiceBillDetail');
        if gs_BillType<>'ERBILL' then
        begin
             if (gs_BillType = 'TPBILL') then
             begin
                    SQL.Add(' where SEBD_PatientId=' + Le_HosNo.Text +' and SEBD_BillType In (''B'')');
             end
             else
    ...
    Begin
      ...
      Begin
        ...
        SQL.Add(' (Select UserName From lab_UserMain Where UserId=REDE_RefundBy) UserName,');
...
begin
     if gs_IsTestNameFilterDepWise='Y' then
     Begin
          With Query_MKTReferral Do
          begin
               Close;
               SQL.Clear;
               SQL.Add(' Select DEPT_DepID ID, Dept_DepName NAME From HS_DEPT_Department');
          ...
          Label_MKTGRefCap.Caption := 'Ref. Hos:';
     End
     Else
     Begin
          With Query_MKTReferral Do
...
begin
     ...
     Begin
           ...
           Query_ReferingDocDept.Session:=DM_hospital.DB;
           Query_ReferingDocDept.Open;


          DBLCB_RefDocCode.KeyField := 'DocID';
          DBLCB_RefDocCode.ListField := 'DocName';
...
begin
     ...
     BEGIN
          ...
          begin
               ...
               FieldDefs.Add('ReportSNO', ftInteger);
               ...
               FieldDefs.Add('PatientCategory', ftString, 5);
               CreateTable;
          end;
     END;
end;
...
Begin
  if FileExists(gs_temppath + '\ServiceWiseFraction.db') then
  begin
...
begin
  if FileExists(gs_temppath + '\ServiceWiseMKTFraction.db') then
  begin
...
procedure TForm_Billing.CreateTempPendingBill;
begin
     if FileExists(gs_temppath + '\PendingBill.db') then
     DeleteFile(gs_temppath + '\PendingBill.db');
     with Table_PendingBills do
     begin
          Close;
          DatabaseName := gs_temppath;
          TableName := 'PendingBill.db';
          TableType := ttDefault;
          FieldDefs.Clear;
          FieldDefs.Add('SNO', ftInteger);
          FieldDefs.Add('DepID', ftInteger);
          FieldDefs.Add('PatientID', ftInteger);
          FieldDefs.Add('EmrNo',ftString,10);
          FieldDefs.Add('ServiceBillDetailID', ftInteger);
          FieldDefs.Add('BillDetailID', ftInteger);
          FieldDefs.Add('BillNo', ftString, 15);
          FieldDefs.Add('InPatientID', ftInteger);
     ...
     IF Table_PendingFraction.Active Then
     Table_PendingFraction.Active := False;
     // Begin
     With Table_PendingFraction do
     begin
          Close;
          DatabaseName := gs_temppath;
          TableName := 'PendingFraction.db';
          TableType := ttDefault;
          FieldDefs.Clear;
          FieldDefs.Add('Sno', ftInteger);
          FieldDefs.Add('PatientId', ftInteger);
...
begin
  Try
    ...
    begin
      ...
      (Table_Billing.FieldByName('Status').AsString <> 'R')) Or ((Key = VK_DELETE) and (gs_BillType = 'SERVICE REFUNDBILL') and
...
begin
     ...
     Begin
          if (Table_Billing.FieldByName('TestNameCode').AsString = 'ONE001') and
               (gi_compileValue = 562) then // Specail Case for Manipal Blood Refund
          begin
               IF (DBGrid_Billing.SelectedIndex = 4) or (DBGrid_Billing.SelectedIndex = 5) then
               Begin
                    if Key = #13 then
                    Begin
                         IF Table_Billing.FieldByName('Qty').AsFloat > Table_Billing.FieldByName('PrevQty').AsFloat then
                         begin
                              MessageDlg('Sorry You Are Trying to Refund " ' + FloatToStr(Table_Billing.FieldByName('Qty').AsFloat)+ ' " More Than Billing Qty ' + ' " '
                                   + FloatToStr(Table_Billing.FieldByName('PrevQty').AsFloat) + ' "',mtWarning, [MbOk], 0);
                              Table_Billing.FieldByName('Qty').AsFloat :=Table_Billing.FieldByName('PrevQty').AsFloat;
                         end;
                         ReLoadRefundChange;
          ...
          begin
               ...
               IF (DBGrid_Billing.SelectedIndex = 5) and (Table_Billing.FieldByName('PrevQty').AsFloat > 1) then // Qty
               Begin
                    if Key = #13 then
                    Begin
                         IF Table_Billing.FieldByName('Qty').AsFloat > Table_Billing.FieldByName('PrevQty').AsFloat then
...
begin
     ...
     pb_IsTestListGridDblClick:=True;
     ...
     pb_IsTestListGridDblClick:=False;
...
procedure TForm_Billing.DBLCB_HospitalClick(Sender: TObject);
begin
     with Query_ReferingDocDept do
     begin
          Close;
          Session:=DM_Hospital.DB;
          sql.clear;
          sql.Add('select REDO_DOCID as docid,REDO_DOCCODE as docode,REDO_DOCNAME as DOCNAME ');
...
procedure TForm_Billing.DBLCB_HospitalExit(Sender: TObject);
begin
     DBLCB_RefDocCode.SetFocus;
end;

procedure TForm_Billing.DBLCB_HospitalKeyDown
...
procedure TForm_Billing.DBLCB_HospitalKeyPress
...
procedure TForm_Billing.DBLCB_RefDocCodeExit(Sender: TObject);
begin
     if (gs_CalledFrom<>'FinalBilling') AND (gs_CalledFrom<>'REFUNDBILL') then
    begin
//          DBLCB_Scheme.setfocus;
    end;
end;
...
procedure TForm_Billing.DBLCB_RefDocCodeKeyPress(Sender: TObject; var Key: Char);
begin
     if Key = #13 then
     Begin
...
begin
     begin
          ...
          Begin
               ...
               End
               else if (gs_BillType='TPBILL') then
               begin
                     IF CB_PayType.ItemIndex = 0 THEN
                     BEGIN
                      Label_SchemeCap.Font.Color := clRed;
...
begin
     IF pb_IsTestListGridDblClick=True Then Exit;
     ...
     begin
          ...
          begin
               ...
               if pb_IsFinalSelectedTestName = True then
                    SQL[1] := 'Where IsActive=''Y'' and Trim(TestNameCode)=' + #39 + Trim(Edit_TestNameCode.Text) + #39

               Else
               Begin
                    if cb_extended.Checked = True then
                         SQL[1] :='Where IsActive=''Y'' and Trim(TestNameCode) like' + #39 + '%' +Trim(Edit_TestNameCode.Text) + '%' + #39
                    else
                         SQL[1] :='Where IsActive=''Y'' and Trim(TestNameCode) Like ' + #39 + Trim(Edit_TestNameCode.Text) + '%' + #39;
               End;
               ...
               Begin
                    if Trim(DBLCB_MKTGReferral.Text)<>'' then
                    SQL[4] := ' And DepId='+IntToStr(DBLCB_MKTGReferral.KeyValue)
                    Else
                    SQL[4] := ' ';
               End
               Else
...
begin
     ...
     begin
          ...
          BEGIN
              if DBLCB_Scheme.KEYVALUE>0 then
                BEGIN
                  Edit_Disper.Text:=floattostr(QueryScheme.FieldByName('SCHE_OPBILLDISPER').AsFloat);
          ...
          BEGIN
             if DBLCB_Scheme.KEYVALUE>0 then
               BEGIN
                  Edit_Disper.Text:=floattostr(QueryScheme.FieldByName('SCHE_IPBILLDISPER').AsFloat);
               END
             ELSE
               BEGIN
                  Edit_Disper.Text:='0';
          ...
          begin
               ...
               begin
                    ...
                    Begin
                         ...
                         SQL.Clear;
                         SQL.Add(
                              ' Select sebd_BillNo BillNo,sebd_BillNo FinalBillNo,sebd_BillDate BillDate,sebd_BillTime BillTime,sebd_BillBy BillBy,');
                         SQL.Add(
                              ' (Select UserName From lab_UserMain where UserId=sebd_BillBy) as UserName,');
                         SQL.Add(
                              ' Sum((sebd_Amount*sebd_Qty)+sebd_VatAmt-(sebd_Amount*sebd_Qty*sebd_DisPer/100)) as NetTotal,''FINALBILL'' as BillType,');
                         SQL.Add(
                              ' NVL(Case when SubStr(sebd_BillNo,1,2)=''ME'' Then (Select Sum(ReceivedAmount) as Total From CreditPayment where CrBillNo=sebd_BillNo) Else 0 End,0) as DepositAmt');
                         SQL.Add(' From HS_sebd_serviceBillDetail BD where sebd_PatientId=' + IntToStr
                                   (gi_PatientID) + ' and sebd_BillType=''B'' and SEBD_BILLDETAILID=0');
                         SQL.Add(
                              ' Group By sebd_BillNo,sebd_BillDate,sebd_BillTime,sebd_BillBy');
                         SQL.Add(' Order by sebd_BillDate Desc,sebd_BillTime Desc');
//                         SQL.Savetofile('C:\abc66.txt');
               ...
               Begin
                    ...
                    Begin
                         ...
                         SQL.Add(
                              ' (Select UserName From lab_UserMain where UserId=BIDE_BillBy) as UserName,');
                         SQL.Add(
...
//                         SQL.SaveToFile('C:\abc.txt');
               ...
               Begin
                    ...
                    Begin
                         ...
                         SQL.Add(' (Select UserName From lab_UserMain where UserId=BIDE_BillBy) as UserName,');
                         SQL.Add(' Sum((BIDE_Amount*BIDE_Qty)+BIDE_VatAmt-(BIDE_Amount*BIDE_Qty*BIDE_DisPer/100)) as NetTotal,0 as DepositAmt,0 as CopayAmount,''FINALBILL'' as BillType');
                         ...
                         SQL.Add(' (Select UserName From lab_UserMain where UserId=SEBD_BillBy) as UserName,');
                         SQL.Add(' Sum((SEBD_Amount*SEBD_Qty)+SEBD_VatAmt-(SEBD_Amount*SEBD_Qty*SEBD_DisPer/100)) as NetTotal,');
                         SQL.Add(' NVL((Select Sum(DEPO_DrAmount-DEPO_CrAmount) From HS_DEPO_Deposit where DEPO_ReferenceNo=SEBD_BillNo),0) as DepositAmt,');
     ...
     begin
          ...
          begin
               ...
               ps_RefundBillNo := trim(Edit_PreveBillNo.Text);
               ...
               begin
                    ...
                    begin
                        ...
                        // SQL.Add(' Select BIDE_BillDetailId From HS_BIDE_BillDetail');
                         if (RB_IPBILL.Checked = True) and(Copy(Query_RefundBillList.FieldByName('BillNo').AsString, 1, 2)= 'TP') or (gs_BillType='SERVICE REFUNDBILL')  then
                         SQL.Add(' where sebd_BillNo=' + #39 +Query_RefundBillList.FieldByName('FinalBillNo').AsString + #39 + ' and NVL(SEBD_DisSchemeId,0) > 0')
                         Else
                         SQL.Add(' where sebd_BillNo=' + #39 + Edit_PreveBillNo.Text + #39 +' and NVL(SEBD_DisSchemeId,0) > 0');
                    ...
                    else if (gs_BillType<>'SERVICE REFUNDBILL') AND (RB_OPBILL.Checked = True) or (gs_BillType<>'SERVICE REFUNDBILL')AND (Query_RefundBillList.FieldByName('BillType').AsString = 'FINALBILL') then
                    Begin
                         SQL.Add(' Select Distinct SEBD_DisSchemeId as SchemeId,(Select Distinct SEBD_RefDocID From HS_SEBD_ServiceBillDetail');
                         SQL.Add(' where SEBD_ServiceBillDetailId=SBD.SEBD_ServiceBillDetailId and RowNum=1) as RefDocID,');
                         SQL.Add(' (Select Distinct SEBD_RefDepID From HS_SEBD_ServiceBillDetail');
                    ...
                    sql.SaveToFile('C:\RefundBill.txt');
                    Open;
               end;
               ...
               begin
                    ...
                    begin
                         ...
                         Session := DM_Hospital.DB;
                         SQL.Clear;
                         SQL.Add('Select Distinct BIDE_DISSCHEMEID as SchemeId,BIDE_REFDOCID RefDocID,BIDE_REFDEPID RefDepID');
                         sql.add('from hs_bide_billdetail');
                         if (RB_IPBILL.Checked = True) and(Copy(Query_RefundBillList.FieldByName('BillNo').AsString, 1, 2)= 'TP') then
                         SQL.Add(' where BIDE_BillNo=' + #39 +Query_RefundBillList.FieldByName('FinalBillNo').AsString + #39 + ') and NVL(BIDE_DISSCHEMEID,0) > 0')
                         Else
                         SQL.Add(' where BIDE_BillNo=' + #39 + Edit_PreveBillNo.Text + #39 +' and NVL(BIDE_DISSCHEMEID,0) > 0');
                         sql.SaveToFile('C:\RefundBill.txt');
                         Open;
                    end;
               end;
...
Begin
     ...
     Begin
          ...
          Session := DM_Hospital.DB;
          SQL.Clear;
          if  (gs_BillType<>'SERVICE REFUNDBILL') and (RB_OPBILL.Checked = True) or (gs_BillType<>'SERVICE REFUNDBILL') AND (Query_RefundBillList.FieldByName('BillType').AsString = 'FINALBILL') then
          Begin
          ...
          Begin
               ...
               SQL.Add(' NVL((Select NVL(CancelStatus,''N'') From PatientTest PT where PatientID=SEBD_PatientID and ServiceBillDetailId=SEBD_ServiceBillDetailId and RowNum=1),''Y'') CancelStatus,');
               SQL.Add(' NVL((Select NVL(SampleTakenBy,0) SampleReceivedBy From SampleCollection where PatientTestId In (Select PatientTestId');
...
begin
     ...
     begin
          ...
          begin
               Close;
               if pb_IsFinalSelectedTestName = True then
               SQL[1] :=' Where IsActive=''Y'' and Trim(TestName)=' + #39 + StringReplace(Trim(Edit_TestName.Text), '''', '''''', [rfReplaceAll]) + #39
               Else
               Begin
                    if cb_extended.Checked = True then
                    SQL[1] :=' Where IsActive=''Y'' and Trim(TestName) like' + #39 +'%' + StringReplace(Trim(Edit_TestName.Text), '''', '''''',[rfReplaceAll]) + '%' + #39
                    else
                    SQL[1] :=' Where IsActive=''Y'' and Trim(TestName) Like ' + #39 +StringReplace(Trim(Edit_TestName.Text), '''', '''''',[rfReplaceAll]) + '%' + #39;
               End;
               ...
               if Trim(Edit_TestNameCode.Text) <> '' then
               ls_Str:=' And TestNameCode=' + #39 + Edit_TestNameCode.Text + #39
               Else
               ls_Str:=' ';
               ...
               Begin
                    if Trim(DBLCB_MKTGReferral.Text)<>'' then
                    SQL[4] := ls_Str+' And DepId='+IntToStr(DBLCB_MKTGReferral.KeyValue)
                    Else
                    SQL[4] := ls_Str;
               End
               Else
     ...
     begin
          ...
          begin
               ...
               begin
                    ...
                    FieldByName('Disper').AsFloat := StrToFloat(FormatFloat('#0.00', DISPER));
                    Qty := FieldByName('Qty').AsInteger;
                    Discount := (DISPER / 100) * FieldByName('TestPrice').AsFloat * Qty;
                    FieldByName('Discount').AsFloat := StrToFloat(FormatFloat('#0.00', Discount));
                    //TestPriceWithDis := (Table_Billing.FieldByName('TestPrice').AsFloat *Qty) -(((DISPER / 100) * (Table_Billing.FieldByName('TestPrice').AsFloat*Qty));
                    TestPriceWithDis := ((Table_Billing.FieldByName('TestPrice').AsFloat *Qty)-(Table_Billing.FieldByName('TestPrice').AsFloat *Qty*DisPer/100));// * gf_TaxPercent/100;
                    SvrTaxWithDis := (gf_TaxPercent / 100) * TestPriceWithDis;
...
begin
  (* Key := #13;
   ...
   { if (gs_CalledFrom<>'FinalBilling') AND (gs_CalledFrom<>'REFUNDBILL') then
    ...
    if (gs_CalledFrom='REFUNDBILL') or (gs_CalledFrom='RegisteredPatientList') or (gs_BillType='SERVICE REFUNDBILL') then
    begin
          Edit_PreveBillNo.SetFocus;
    end;
...
begin
     ...
     begin
          ...
          if (gs_CalledFrom='NormalOPDBilling') and (gs_BillType<>'TPBILL') then
          Begin
               if (gs_BillType<>'REFUNDBILL')  then
               begin
                    if (gs_BillType<>'SERVICE REFUNDBILL') then
          ...
          begin
               ...
               Label_PatientName.Caption :=Gs_PatientName;       //Gs_Title + ' ' + Gs_PatientName;
               Label_AgeSex.Caption := Gs_CurrentAgeGender;
               CB_Patientcategory.ItemIndex :=0;

               with Query_patientmain do
               begin
                    close;
                    sql.clear;
                    sql.add('select pama.*,(select SCHE_SCHEME from HS_SCHE_Scheme where sche_schemeid=pama_schemeid)scheme from hs_pama_patientmain pama where pama_patientid = ' +Le_HosNo.Text);
                    SQL.SaveToFile('C:\acb.txt');
                    open;
               end;
               DBLCB_scheme.keyvalue := query_patientmain.fieldbyname('PAMA_SCHEMEID').AsInteger;
               lbl_memtype.caption := query_patientmain.fieldbyname('scheme').asstring;
               DBLCB_SchemeClick(sender);
               if lbl_memtype.Caption= '' then
               begin
                    lbl_memtype.Visible:=False;
               end
               else
               begin
                    lbl_memtype.Visible:=True;
//                    lbl_memtype.Font.Color := clHighlight;
                    lbl_memtype.Font.Name :='Times New Roman';
                    lbl_memtype.Font.Size := 16;
               end;

               pb_IsOPUserDoTPBillAfterCreditAppForInpt:=False;
               if (gs_BillType = 'OPBILL') or (gs_BillType='TPBILL') then
               Begin
               ...
               Begin
                    ...
                    begin
                         ...
                         Session := DM_Hospital.DB;
                         SQL.Clear;
                         SQL.Add('Select PAvi_Schemeid From HS_PAVI_PatientVisit Where PAVI_EMERGENCYNO='+QuotedStr(gs_EMRNoForMedPT) +'Order By Pavi_Schemeid Desc');
                         Open;
                    end;
                   // DBLCB_Scheme.KeyValue := Query_Process.FieldByName('Pavi_Schemeid').AsInteger;
                    DBLCB_Scheme.Enabled := False;
               End;
               ...
               Begin
                    ...
                    SQL.Add(' (Select SCHE_OPBILLDISPER From HS_SCHE_Scheme where SCHE_SchemeID=Memb_schemeid) as OPDisPer,');
                    SQL.Add(' (Select SCHE_IPBILLDISPER From HS_SCHE_Scheme where SCHE_SchemeID=Memb_schemeid) as IPDisPer,');
                    ...
                    Begin
                         ...
                         if gs_BillType = 'OPBILL' then
                         Edit_Disper.Text:=FloatToStr(Query_Process.FieldByName('OPDisPer').AsFloat)
                         Else
                         Edit_Disper.Text:=FloatToStr(Query_Process.FieldByName('IPDisPer').AsFloat);

                         pb_IsMedicarePatient := True;
                         //DBLCB_Scheme.KeyValue := Pi_SchemeID;

                         //CB_PayType.ItemIndex := 1;
                         if gb_IsOPCreditBillBlock=False then
                         Begin
                              CB_PayType.ItemIndex := 1;
                              Label_SchemeCap.Font.Color := clRed;
                              Timer_PayTypeFlash.Enabled := True;
               ...
               Begin
                    ...
                    begin
                         ...
                         Begin
                              ...
                              gi_SchemeId := Qry.FieldByName('SchemeId').AsInteger;
                              gi_CommunityId := Qry.FieldByName('CommunityId').AsInteger;

                              Pi_CommunityID := Qry.FieldByName('CommunityId').AsInteger;
                              Pi_SchemeID := Qry.FieldByName('SchemeId').AsInteger;
...
//                              DBLCB_Scheme.KeyValue := Pi_SchemeID;
          ...
          begin
               if (gi_BillCase = 3) or (gs_BillType='TPBILL') then
               begin
                    if gs_BillType <> 'REFUNDBILL' then
                    Begin
                         {Try
               ...
               else if (gi_BillCase=0) and (gs_BillType='ERBILL') then
               begin
                   { Try
                         Form_otherPatient := TForm_otherPatient.Create(Nil);
                         With Form_otherPatient do
          ...
          if (gi_PatientID > 0) and (gs_CalledFrom<>'FinalBilling') then
          Begin
               if (gs_BillType = 'REFUNDBILL') or (gs_BillType='SERVICE REFUNDBILL') then
                  Edit_PreveBillNo.SetFocus
               Else
               begin
                   // Edit_TestNameCode.visible:=true;
                   // Edit_TestNameCode.SetFocus;
                    //Edit_TestName.setfocus;
                    //DBLCB_Hospital.setfocus;
                    DBLCB_RefDocCode.SetFocus;
          ...
          End;
          //Edit_TestName.setfocus;
          if (gs_CalledFrom='FinalBilling') and (gi_compileValue in [1,14,21,22,23,24,25,26,27,28]) then
          BEGIN
               //gi_BillCase:=1;
               Form_Billing.LoadBill;
               RB_OPBILLClick(Sender);
               Panel_ItemSearch.Enabled:=False;
              // DBLCB_RefDocCode.Enabled:=False;
               SPB_Memberno.Enabled:=False;
               Le_MemberNo.Enabled:=False;
               CB_Patientcategory.Enabled:=False;
               CB_BILLTYPE.Enabled:=False;
               DBLCB_Scheme.Enabled:=False;
               SpeedButton2.Enabled:=False;
               CB_ViewOldBill.Enabled:=False;
               CB_ViewOldTest.Enabled:=False;
               //SPB_FractionAddition.Enabled:=false;
               SPB_FractionAddition.Enabled:=True;
               Btn_BringPending.Enabled:=False;
               Btn_KeepPending.Enabled:=false;
               SPB_AddItem.enabled:=false;
...
begin
  ...
  begin
    ...
    begin
      ...
      //SQL.Add(',(SELECT TESTNAMEID FROM PATIENTTEST WHERE PATIENTTESTID=SBD.SEBD_PATIENTTESTID)TESTNAMEID,(select tena_ispackagetest from hs_tena_testname where TENA_TestNameCode=SBD.SEBD_Service)ISPACKAGETEST');
      SQL.Add(',(SELECT TENA_TESTNAMEID FROM HS_TENA_TESTNAME WHERE TENA_TESTNAMECODE=SBD.SEBD_SERVICE)TESTNAMEID,(select tena_ispackagetest from hs_tena_testname where TENA_TestNameCode=SBD.SEBD_Service)ISPACKAGETEST');
      SQL.Add(',(SELECT TENA_TNCategoryCode FROM HS_TENA_TestName WHERE TENA_TestNameCode=SBD.SEBD_Service) TNCategoryCode,SEBD_PatientCategory PatientCategory');
      ...
      SQL.Add(' ,(SELECT UserName From lab_USERMAIN Where UserID=SBD.SEBD_BillBy)UserName');
      ...
      //SQL.Add('AND SEBD_BillDetailId<>-1111 AND SBD.SEBD_SERVICEBILLDETAILID NOT IN (SELECT SEBD_SERVICEBILLDETAILID FROM HS_TECA_TESTCANCEL WHERE TECA_SERVICEBILLDETAILID=SBD.SEBD_SERVICEBILLDETAILID)');
       SQL.Add('AND SEBD_BillDetailId=0 AND SBD.SEBD_SERVICEBILLDETAILID NOT IN (SELECT SEBD_SERVICEBILLDETAILID FROM HS_TECA_TESTCANCEL WHERE TECA_SERVICEBILLDETAILID=SBD.SEBD_SERVICEBILLDETAILID)');
    end
    ...
    begin
      ...
      SQL.Add(',BD.BIDE_ISFRACTIONABLETEST ISFRACTIONABLETEST,(SELECT TENA_TESTNAMEID FROM HS_TENA_TESTNAME WHERE TENA_TESTNAMECODE=BD.BIDE_SERVICE)TESTNAMEID');
      SQL.Add(',(SELECT TENA_TNCATEGORYCODE FROM HS_TENA_TESTNAME WHERE TENA_TESTNAMECODE=BD.BIDE_Service) TNCategoryCode,Bide_PatientCategory PatientCategory');
      SQL.Add(' ,(SELECT BIMA_TOBESENTTOWEB FROM HS_BIMA_BILLMASTER WHERE BIMA_BILLNO=BD.BIDE_BILLNO)TOBE_SENTTOWEB');
      SQL.Add(' ,(SELECT BIMA_TOBESENTTOSMS FROM HS_BIMA_BILLMASTER WHERE BIMA_BILLNO=BD.BIDE_BILLNO)TOBE_SENTTOSMS');
      SQL.Add(' ,(SELECT BIMA_TOBESENTTOEMAIL FROM HS_BIMA_BILLMASTER WHERE BIMA_BILLNO=BD.BIDE_BILLNO)TOBE_SENTTOEMAIL');
        ...
        1, 5:
          begin
            ...
            begin
              ...
              FieldByName('RefDocid').AsString := Qry.FieldByName('RefDocid').AsString;
              ...
              FieldByName('ISPACKAGETEST').AsString := Qry.FieldByName('ISPACKAGETEST').AsString;
        ...
        2, 4, 6, 7:
          begin
            ...
            begin
              ...
              FieldByName('TestNameCode').AsString := Qry.FieldByName('Service').AsString;
              FieldByName('TNCategoryCode').AsString := Qry.FieldByName('TNCategoryCode').AsString;
              FieldByName('PatientCategory').AsString := Qry.FieldByName('PatientCategory').AsString;
              FieldByName('TestNameId').AsInteger := Qry.FieldByName('TestNameId').AsInteger;
              FieldByName('TestName').AsString := Qry.FieldByName('ServiceType').AsString;
              FieldByName('TestPrice').AsFloat := StrToFloat(FormatFloat('#0.00', pf_TestPrice));
           ...
           if gi_compileValue<>23 then
           begin
                if (Qry.FieldByName('ISFRACTIONABLETEST').AsString='Y') and (gi_BillCase=1) then
                LoadServiceWiseFraction(Qry.FieldByName('Service').AsString, Qry.FieldByName('TNCategoryCode').AsString,
                         pf_TestPrice, pf_Qty, Qry.FieldByName('DOCID').AsInteger, Qry.FieldByName('DepID')
                              .AsInteger, Qry.FieldByName('TestNameId').AsInteger);
                    Display_Frct_Involve_Person(Qry.FieldByName
                              ('ISFRACTIONABLETEST').AsString, Query_TestName.FieldByName
                              ('TestNameID').AsInteger);
           end
           else
             LoadServiceWiseFraction(Qry.FieldByName('Service').AsString, Qry.FieldByName('TNCategoryCode').AsString,
...
begin
     ...
     Begin
          if gi_compileValue = 24 then
          begin
               try
                    Form_QRBill_PPL := TForm_QRBill_PPL.Create(nil);
                    (*if Gb_IsReprint then
                    Form_QRBill_Manipal.lbl_reprint.Enabled := True;*)
                    // if Gb_IsPreview then
                    // begin
                    with Form_QRBill_PPL do
                    begin
                         QrBill.Prepare;
                    ...
                    Gb_IsReprint := False;
                    Form_QRBill_PPL.Free;
               end;
          end
          else if gi_compileValue = 25 then
          begin
               try
                    Form_QRBill_GMC := TForm_QRBill_GMC.Create(nil);
                    (*if Gb_IsReprint then
                    Form_QRBill_Manipal.lbl_reprint.Enabled := True;*)
                    // if Gb_IsPreview then
                    // begin
                    with Form_QRBill_GMC do
                    begin
                         QrBill.Prepare;
                         //lbl_totpage.Caption := IntToStr(QrBill.QRPrinter.PageCount);
                    ...
                    Gb_IsReprint := False;
                    Form_QRBill_GMC.Free;
               end;
          end
          else if gi_compileValue=26 then
          begin
               try
                    Form_QRBill_nozomi := TForm_QRBill_nozomi.Create(nil);
                    (*if Gb_IsReprint then
                    Form_QRBill_Manipal.lbl_reprint.Enabled := True;*)
                    // if Gb_IsPreview then
                    // begin
                    with Form_QRBill_nozomi do
                    begin
                         QrBill.Prepare;
                         //lbl_totpage.Caption := IntToStr(QrBill.QRPrinter.PageCount);
...
begin
  li_DepositHeadId := 1; // INVESTIGATION CHARGE
  GB_isDepositBill := True;
  SaveDeposit(li_DepositHeadId, gi_InPatientID, gi_PatientID, 0, Gi_UserId, 0,DrAmount, CrAmount, 'ADVANCE RECEIPT', 'ADVANCE DEPOSIT', Gs_BillNo,
     '', DepositRemarks, Ps_TodaysDate, Ps_TodaysTime, 'N',ps_PatientCategory,'N','','N',IsServiceCancel);//GetPatientCagetory(gi_PatientID)
end;
...
begin
  ...
  begin
    ...
    SaveDeposit(li_DepositHeadId, gi_InPatientID, gi_PatientID, 0, Gi_UserId,0, lf_CrAmount, lf_DrAmount, '', '', '', '', '', Ps_TodaysDate,
          Ps_TodaysTime, 'N',ps_PatientCategory,'N','','N','N');//GetPatientCagetory(gi_PatientID)
  end;
...
begin
  ...
  SaveDeposit(0, gi_InPatientID, gi_PatientID, 0, Gi_UserId, 0, DrAmount,
    ...
    Ps_TodaysDate, Ps_TodaysTime, 'N',ps_PatientCategory,'N','','N','N');//GetPatientCagetory(gi_PatientID)
  (* Try
...
Var
  ls_sendtoweb, ls_sendtosms, ls_sendtoemail, INITIAL, ls_IsVatable,ls_WorkingSatus: String;
  ls_FinalBillNo, Ls_NewSampleNo,ls_fname,ls_lname,ls_age,ls_lastbilldate,ls_userpassword: String;
  Qty, i, J, li_Community, li_Scheme, pi_DocFraDepId, li_TestNameId, li_Qty,
    li_RefDepID, li_RefDocId, li_MKTGREFID,CountRandomPathotest: Integer;
...
begin
        if gi_compileValue=23 then
        begin
              CountRandomPathotest :=0;
               with Query_CheckBilldate do
               begin
                    Close;
                    session := DM_Hospital.DB;
                    sql.Clear;
                    sql.Add('select Max(bide_billdate) Billdate from hs_bide_billdetail where bide_patientid='+IntToStr(gi_PatientID));
                    Open;
               end;
               ls_lastbilldate := Query_CheckBillDate.FieldByName('Billdate').AsString;
        end;
      ...
      begin
        ...
        Try
          SaveBillDetail(gi_ServiceBillDetailid, gi_PatientID, gi_InPatientID,gi_PatientTestID, Gi_UserId, li_Community, li_Scheme, li_Community,
            ...
            FieldByName('IsFractionableTest').AsString, FieldByName('ISPACKAGETEST').AsString, '', FieldByName('OrgBillCategory').AsString, FieldByName('HOSPARTPER').AsString,
            FieldByName('ISOT').AsString,FieldByName('PatientCategory').AsString);//ps_PatientCategory);
        except
          SaveBillDetail(gi_ServiceBillDetailid, gi_PatientID, gi_InPatientID,
            ...
            ls_RateType, ls_RateType, gs_memberNo, ls_IsVatable, '', FieldByName
              ('IsFractionableTest').AsString, FieldByName('ISPACKAGETEST').AsString, '', FieldByName
        ...
        End;
          ...
          begin
             ...
             begin
                  ...
                  SQL.Add('SEBD_BILLDETAILID='+inttostr(gi_BillDetailId));
                  sql.Add('where SEBD_SERVICEBILLDETAILID='+inttostr(gi_ServiceBillDetailid)) ;
                  ExecSQL;
             end;

             updatePatienttestForFinalBill(gi_ServiceBillDetailid,gi_billdetailid,Gs_BillNo);

          end;


          if  gi_BillCase = 2 then   // DIrect Billing
          begin
          // if pf_Deposit> pf_grandtotal then
                    ...
                    begin
                      ...
                      begin
                        SavePatientTest(0, gi_BillDetailId, 0, gi_PatientID,gi_InPatientID, Pi_ClinicalDepId, FieldByName('DepID').AsInteger,
                            ...
                            0,Gi_UserId,gi_Recommendation, gi_DoNotSync,gi_BillingInvCtrl,pi_refdocid, FieldByName('TestPrice').AsFloat,
                            Gs_BillNo, FieldByName('Doccode').AsString, FieldByName('TestNameCode').AsString, Ps_TodaysDate, Ps_TodaysTime,
                          CB_PayType.Text, Le_Remarks.Text, 'RE', Ps_TodaysDate,Ps_TodaysTime,ps_refhoscode);
                      end;
                    end
                    else
                      SavePatientTest(0, gi_BillDetailId, 0, gi_PatientID,
                        ...
                        gi_BillingInvCtrl,pi_refdocid, FieldByName('TestPrice').AsFloat,Gs_BillNo,
                        FieldByName('Doccode').AsString, FieldByName('TestNameCode')
                          .AsString, Ps_TodaysTime, TodaysTime, CB_PayType.Text,
                        Le_Remarks.Text, 'RE', Ps_TodaysDate, TodaysTime,ps_refhoscode);
          ...
          //for userpassword
           ...
           begin
                 if IsPathoRadioDep(FieldByName('DepID').AsInteger) then
                 begin
                      if CountRandomPathotest = 0 then
                      begin
                           with Query_CheckBilldate do
                           begin
                                Close;
                                session := DM_Hospital.DB;
                                sql.Clear;
                                sql.add('select pama_userpassword,pama_fname,pama_lname,pama_age from hs_pama_patientmain where pama_patientid='+inttostr(gi_patientid));
                                open;
                           end;

                           ls_fname:=Query_CheckBilldate.fieldbyname('pama_fname').asstring;
                           ls_lname:=Query_CheckBilldate.fieldbyname('pama_lname').asstring;
                           ls_age:=Query_CheckBilldate.fieldbyname('pama_age').asstring;
                           if (ls_lastbilldate < Ps_TodaysDate) or (Query_CheckBilldate.FieldByName('pama_userpassword').asstring='') then
                           begin
                                     ls_userpassword:=MakeRNDString(trim(StringReplace(ls_fname, ' ', '', [rfReplaceAll]))+trim(StringReplace(ls_lname, ' ', '', [rfReplaceAll]))+trim(StringReplace(ls_age, ' ', '', [rfReplaceAll]))+'KYMERT',8);
                                     with Query_CheckBilldate do
                                     begin
                                          Close;
                                          session := DM_Hospital.DB;
                                          sql.Clear;
                                          sql.add('update hs_pama_patientmain set pama_userpassword='+quotedstr(ls_userpassword)+',PAMA_SYNCSTATUS = 1 where pama_patientid='+inttostr(gi_patientid));
                                          ExecSQL;
                                     end;
                           end;
                           CountRandomPathotest := CountRandomPathotest+1;
                      end;
                 end;
           end;
      ...
      begin
        ...
        SaveBillDetail(gi_ServiceBillDetailid, gi_PatientID, gi_InPatientID,
          ...
          gs_memberNo, ls_IsVatable, '', '', '', '', FieldByName
            ('OrgBillCategory').AsString, FieldByName('HOSPARTPER').AsString,
          'N',FieldByName('PatientCategory').AsString);//ps_PatientCategory);
          ...
          begin
            ...
            begin
              ...
              begin
                SavePatientTest(0, gi_BillDetailId, 0, gi_PatientID,
                  gi_InPatientID, Pi_ClinicalDepId, FieldByName('DepID')
                    ...
                    ('TestNameID').AsInteger, FieldByName('TestPackageID')
                    .AsInteger, 0, Gi_UserId, gi_Recommendation, gi_DoNotSync,
                  gi_BillingInvCtrl,pi_refdocid, FieldByName('TestPrice').AsFloat,
              ...
              SavePatientTest(0, gi_BillDetailId, 0, gi_PatientID,
                ...
                Le_Remarks.Text, 'RE', Ps_TodaysDate, Ps_TodaysTime,ps_refhoscode);
...
begin
     ...
     try
          ...
          ps_DepositBillNo := '';
          {if Trim(Le_GrandTotal.Text) = '' then
          Le_GrandTotal.Text := '0';}

          if Trim(Le_Payment.text)='' then
          Le_Payment.text:='0';

          if (StrToFloat(Le_Payment.Text) > 0) then  //and (Pi_SchemeID <> 22)
          Begin
               SaveInvAdvanceDeposit(StrToFloat(Le_Payment.Text), 0, Le_Remarks.Text,'N');
               Label_DepositNo.Caption := Gs_BillNo;
               Label_DepositNo.Visible := True;
               Label11.Visible := True;
               ps_DepositBillNo := Gs_BillNo;
          ...
          begin
               ...
               Try
                    SaveServiceBillDetail(gi_PatientID, gi_InPatientID, 0, Gi_UserId,
                         li_Community, li_Scheme, li_Community, li_Scheme, FieldByName('DepID').AsInteger, Pi_ClinicalDepId, li_RefDepID,
                          ...
                          ls_RateType, gs_memberNo, '', '', FieldByName('isfractionabletest').AsString, FieldByName('ISPACKAGETEST').AsString, '', FieldByName('ORGBILLCATEGORY').AsString,
                          FieldByName('ISOT').AsString,gs_EMRNoForMedPT,FieldByName('PatientCategory').AsString);//ps_PatientCategory);
               Except
                    SaveServiceBillDetail(gi_PatientID, gi_InPatientID, 0, Gi_UserId,
                         li_Community, li_Scheme, li_Community, li_Scheme, FieldByName('DepID').AsInteger, Pi_ClinicalDepId, li_RefDepID,
                          FieldByName('DocId').AsInteger, li_RefDocId, li_MKTGREFID, 0, FieldByName('AccountID').AsInteger,
               ...
               Begin
                    ...
                    Begin
                         Close;
                         SQL.Clear;
                         SQL.Add(' Select TPD.TEPD_TestNameId TestNameId,TN.TENA_TESTNAMECODE TESTNAMECODE,TN.TENA_DepId DEPID,TN.TENA_TestName TestName From HS_TEPD_TestPackageDetail TPD,HS_TENA_TestName TN where');
                         SQL.Add(' TPD.TEPD_TestNameId=TN.TEna_TestNameId and');
                         SQL.Add(' TPD.TEPD_TestPackageId In (Select Tena_TestNameID From Hs_Tena_TestName where Tena_TestNameCode=' + #39 + QrySave.FieldByName('TestNameCode').AsString + #39 + ')');
                         Open;
                         First;
                         while Not eof do
                         Begin
                              ...
                              Try
                                   SavePatientTest(0, 0, gi_ServiceBillDetailid, gi_PatientID,
                                        ...
                                        Query_PackageTest.FieldByName('TestNameID').AsInteger, QrySave.FieldByName('testnameid').AsInteger, 0,
                                        Gi_UserId, gi_Billed, gi_DoNotSync, gi_BillingInvCtrl,pi_refdocid,QrySave.FieldByName('TestPrice').AsFloat,
                                        ...
                                        ls_WorkingSatus, Ps_TodaysDate, Ps_TodaysTime,ps_refhoscode);
                              Except
                                   SavePatientTest(0, 0, gi_ServiceBillDetailid, gi_PatientID,
                                        gi_InPatientID, Pi_ClinicalDepId, Query_PackageTest.FieldByName
                                             ('DepID').AsInteger, QrySave.FieldByName('DOCID').AsInteger,
               ...
               if FieldByName('ISPACKAGETEST').AsString = 'N' then
               begin
                         ...
                         begin
                              ...
                              Try
                                   SavePatientTest(0, 0, gi_ServiceBillDetailid, gi_PatientID,
                                        ...
                                        0, FieldByName('TestNameID').AsInteger,
                                        0, 0,Gi_UserId, gi_Recommendation, gi_DoNotSync,
                                        gi_BillingInvCtrl,pi_refdocid, FieldByName('TestPrice').AsFloat, Gs_BillNo,
                                        FieldByName('Doccode').AsString, FieldByName('TestNameCode')
                                             .AsString, Ps_TodaysDate, Ps_TodaysTime, CB_PayType.Text,
                                        Le_Remarks.Text, ls_WorkingSatus, Ps_TodaysDate, Ps_TodaysTime,ps_refhoscode);
                              Except
...
Var
  ...
  begin
     ...
     //COMMENTED TO ALLOW THE PACKAGE TEST AND THE OTHER TEST

    { if Gb_IsPackageTest=False then
     begin
          if ((Table_Billing.RecordCount>0) and (Query_TestName.FieldByName('ISPACKAGETEST').AsString = 'Y')) then
          begin
               MessageDlg('Sorry ! You cannot add Package Test with Other Test ' , mtWarning, [MbOk], 0);
               Exit;
          end
          else if Query_TestName.FieldByName('ISPACKAGETEST').AsString = 'Y' then
               Gb_IsPackageTest := True;
     end
     else if Gb_IsPackageTest=True then
     begin
          if ((Table_Billing.RecordCount>0)) then
          begin
               MessageDlg('Sorry ! You cannot add Other Test with Package Test ' , mtWarning, [MbOk], 0);
               Exit;
          end
          else
    ...
    { else
     ...
     begin
          ...
          FieldByName('ReportSno').AsInteger := GetMax_TempSno;
          ...
          if gi_compileValue=23 then
            FieldByName('ISFRACTIONABLETEST').AsString := 'Y'
            else
          ...
          FieldByName('PatientCategory').AsString:=gs_PatientCategory;
...
begin
     ...
     Begin
          MessageDlg('Sorry ! This is not Fractional Item.', mtInformation, [MbOk],0);
          Exit;
     End;
...
procedure TForm_Billing.DepartmentWiseBillPrint;
Var
     j,li_depid,li_depidpast,li_testnameid,li_testnameidpast: Integer;
     ls_service,ls_servicepast : string;
     Qry,QRY3: TOraQuery;
     LB_SINGLETEST:BOOLEAN;
    COUNT:INTEGER;
begin
     //li_depid:=0;
     ls_servicepast:='';
     Qry3 := TOraQuery.Create(nil);
      ...
      begin
          ...
          begin
               ...
               Session:=DM_Hospital.DB;
               SQL.Clear;
               SQL.Add('select distinct bide_depid,nvl((SELECT TEPA_TESTPACKAGEID FROM hs_tepa_TESTPACKAGE WHERE TEPA_TESTPACKAGENAME=Hs_Bide_BILLDETAIL.Bide_SERVICETYPE),0)PACKAGEID  from Hs_Bide_billDETAIL where bide_billno='+ #39 + Gs_BillNo + #39);
               Open;
          end;
      end
      else
      begin
          ...
          begin
               ...
               Session:=DM_Hospital.DB;
               SQL.Clear;
               SQL.Add('select distinct sebd_depid,nvl((SELECT TEPA_TESTPACKAGEID FROM hs_tepa_TESTPACKAGE WHERE TEPA_TESTPACKAGENAME=Hs_sebd_servicebillDETAIL.sebd_SERVICETYPE),0)PACKAGEID  from Hs_sebd_servicebillDETAIL where sebd_billno='+ #39 + Gs_BillNo + #39);
               Open;
          end;
      end;
     ...
     try
         // while  not QRY3.Eof do
          begin
               if (gs_BillType <> 'TPBILL') then
               li_depidpast:=Qry3.FieldByName('bide_depid').AsInteger
               else
               li_depidpast:=Qry3.FieldByName('sebd_depid').AsInteger;

               //ls_servicepast:=Qry3.FieldByName('service').AsString;
               gi_packid:=0;
               if Qry3.FieldByName('packageid').AsInteger=0 then
               BEGIN
                    //qry3.next;
                    ...
                    begin
                         ...
                         begin
                              ...
                              Session:=DM_Hospital.DB;
                              SQL.Clear;
                              {SQL.Add('select distinct depid ,rownum as SN,(select depname from department where depid=billdetail.depid)depname,');
                              SQL.ADD('(select TESTPACKAGEID from testPACKAGE where testPACKAGEname=billdetail.serviceTYPE)TESTPACKAGEID,');
                              SQL.ADD('SERVICETYPE,service,BILLDETAILID,''N'' as packagetest from billdetail');
                              SQL.Add('where billNo=' + #39 + Gs_BillNo + #39);
                              SQL.Add('and depid not in(1010,999,998)');
                              sql.Add('and depid='+inttostr(li_depidpast));
                              SQL.Add('union');
                              SQL.Add('select distinct TN.DEPID,rownum as SN,(select depname from department where depid=tn.depid)depname,0 AS TESTPACKAGEID,');
                              SQL.ADD(''''' AS SERVICETYPE,'''' as service,0 AS BILLDETAILID,''Y'' as Packagetest from testpackagedetail td,testname tn');
                              SQL.Add('where TN.TESTNAMEID=TD.TESTNAMEID');
                              SQL.Add('and testpackageid in(');
                              SQL.Add('select (select testnameid from testname where testnamecode=billdetail.service and billNo='+ #39+Gs_BillNo+ #39+'and depid=999)testNameid from billdetail');
                              SQL.Add('where depid=999 and billNo='+ #39 + Gs_BillNo + #39+')');
                              sql.Add('order by SN' ); }

                              SQL.Add('select distinct bide_depid as depid ,(select dept_depname from hs_dept_department where dept_depid=hs_bide_billdetail.bide_depid)depname,');
                              SQL.ADD('(select TEPA_TESTPACKAGEID from HS_TEPA_TESTPACKAGE where TEPA_TESTPACKAGENAME=hs_bide_billdetail.bide_serviceTYPE)TESTPACKAGEID,');
                              SQL.ADD('''N'' as packagetest from hs_bide_billdetail');
                              SQL.Add('where bide_billNo=' + #39 + Gs_BillNo + #39);
                              SQL.Add('and bide_depid not in(1010,999,998)');
                              sql.Add('and bide_depid='+inttostr(li_depidpast));
                              SQL.Add('union');
                              SQL.Add('select distinct TN.tena_DEPID,(select dept_depname from Hs_dept_department where dept_depid=tn.tena_depid)depname,0 AS TESTPACKAGEID,');
                              SQL.ADD('''Y'' as Packagetest from hs_tepd_testpackagedetail td,hs_tena_testname tn');
                              SQL.Add('where TN.tena_TESTNAMEID=TD.tepd_TESTNAMEID');
                              SQL.Add('and tepd_testpackageid in(');
                              SQL.Add('select (select tena_testnameid from hs_tena_testname where tena_testnamecode=hs_bide_billdetail.bide_service and bide_billNo='+ #39+Gs_BillNo+ #39+'and bide_depid=999)testNameid from hs_bide_billdetail');
                              SQL.Add('where bide_depid=999 and bide_billNo='+ #39 + Gs_BillNo + #39+')');
                              //sql.Add('order by SN' );

                              //sql.savetofile('c:\packagetestnew.txt');
                              Open;
                    ...
                    begin
                         ...
                         begin
                              ...
                              SQL.Add('select distinct sebd_depid as depid ,(select dept_depname from hs_dept_department where dept_depid=hs_sebd_servicebilldetail.sebd_depid)depname,');
                              SQL.ADD('(select TEPA_TESTPACKAGEID from HS_TEPA_TESTPACKAGE where TEPA_TESTPACKAGENAME=hs_sebd_servicebilldetail.sebd_serviceTYPE)TESTPACKAGEID,');
                              SQL.ADD('''N'' as packagetest from hs_sebd_servicebilldetail');
                              SQL.Add('where sebd_billNo=' + #39 + Gs_BillNo + #39);
                              SQL.Add('and sebd_depid not in(1010,999,998)');
                              sql.Add('and sebd_depid='+inttostr(li_depidpast));
                              SQL.Add('union');
                              SQL.Add('select distinct TN.tena_DEPID,(select dept_depname from Hs_dept_department where dept_depid=tn.tena_depid)depname,0 AS TESTPACKAGEID,');
                              SQL.ADD('''Y'' as Packagetest from hs_tepd_testpackagedetail td,hs_tena_testname tn');
                              SQL.Add('where TN.tena_TESTNAMEID=TD.tepd_TESTNAMEID');
                              SQL.Add('and tepd_testpackageid in(');
                              SQL.Add('select (select tena_testnameid from hs_tena_testname where tena_testnamecode=hs_sebd_servicebilldetail.sebd_service and sebd_billNo='+ #39+Gs_BillNo+ #39+'and sebd_depid=999)testNameid from hs_sebd_servicebilldetail');
                              SQL.Add('where sebd_depid=999 and sebd_billNo='+ #39 + Gs_BillNo + #39+')');
                              //sql.Add('order by SN' );
               ...
               begin
                    Gb_IsPackageTest:=TRUE;
                    gi_packid:=Qry3.fieldbyname('PACKAGEID').AsInteger;
                    Qry := TOraQuery.Create(nil);
                    with Qry do
                    begin
                         Close;
                         Session:=DM_Hospital.DB;
                         SQL.Clear;
                         SQL.Add('Select distinct tena_DEPID as depid,''PACK'' as service,(SELECT dept_DEPNAME FROM hs_dept_DEPARTMENT WHERE dept_DEPID=tena_DEPID)DEPNAME,');
                         SQL.Add('(select TEPA_TESTPACKAGENAME from HS_TEPA_TESTPACKAGE where TEPA_TESTPACKAGEid in(Select TEPD_TESTPACKAGEID from HS_TEPD_TESTPACKAGEDETAIL');
                         SQL.Add( 'where tepd_TestPackageID='+IntToStr(gi_packid)+')');
                         sql.add(') packagename');
                         SQL.add(' from hs_tena_TestName Where tena_TestNameID In');
                         SQL.Add('(Select tepd_TestNameID from HS_TEPD_TESTPACKAGEDETAIL');
                         SQL.Add('Where Tepd_TestPackageID=' + IntToStr(gi_packid)+')');
                         sql.add('order by depname');
                         //sql.savetofile('c:\packagetest.txt');
                         Open;
                    end;
              ...
              // while not Qry.eof do
               begin
                    if li_depidpast=li_depid then
                    li_depid:=0;
                    if li_depidpast<>li_depid then
                    begin
                          Gi_DepID:=Qry.FieldByName('depid').AsInteger;
                          gs_DepartmentName:=Qry.FieldByName('depname').AsString;
                          {if Qry.FieldByName('packagetest').AsString='Y' then
                          Gb_IsPackageTest:=true
                          else
                          Gb_IsPackageTest:=false; }

                          //if Qry3.FieldByName('SERVICE').AsString='PACK' then
                          if Qry3.FieldByName('packageid').AsInteger>0 then
                          gs_packagename:=Qry.FieldByName('packagename').AsString;
                         Form_QrDepWiseBill := TForm_QrDepWiseBill.Create(nil);
                         with Form_QrDepWiseBill do
                         begin
                              try
          //                         Query_TestName.close;
          //                         Query_TestName.sql[2] := 'and billno=' + #39 + Gs_BillNo + #39;
          //                         Query_TestName.Open;
                                   // Query_TestName.Open;
                                   // Query_TestName.ParamByName('depid').AsInteger:=Qry.FieldByName('depid').AsInteger;
                                   // Query_TestName.Close;

                                   //lbl_sampletype.Caption := Ps_SampleType;
                                   if CB_Preview.Checked = true then
                                   begin
                                        with Form_QrDepWiseBill do
                                        begin
                                   ...
                                   begin
                                        Form_QrDepWiseBill.QrBill.Prepare;
                                        // for j:= 0 to strToInt(Se_NoofPrint.Text) - 1 do
                                        Form_QrDepWiseBill.QrBill.Print;
                                   end;
                                   // Form_QrDepWiseBill.QrBill.Preview;
                              finally
                                   Form_QrDepWiseBill.Free;

                              end;
                         end;
                         //if  Qry3.FieldByName('SERVICE').AsString='PACK' then
                         //Qry.Next;
                         //li_depid:=Qry.FieldByName('depid').AsInteger;
                    end;
                    {if gi_packid=0 then
                    BEGIN
                         li_depid:=0;
                    ...
                    Qry.Next;
                    li_depid:=Qry.FieldByName('depid').AsInteger;
                    //ls_service:=Qry.FieldByName('service').Asstring;
                    END;
...
procedure TForm_Billing.Btn_BringPendingClick(Sender: TObject);
var
     li_PatientId,li_Inpatientid:Integer;
     Key:Char;
begin
     try
          Form_BringonPendingBill := TForm_BringonPendingBill.Create(Nil);
          Form_BringonPendingBill.ShowModal;
     finally
          li_PatientId:=Form_BringonPendingBill.pi_Patientid;
          li_InPatientId:=Form_BringonPendingBill.pi_Inpatientid;
          Form_BringonPendingBill.Free;
     end;
     Key:=#13;
     //if (Trim(Le_HosNo.Text) = '') and (Trim(Le_HosNo.Text) <= '0') then Exit;

     Le_HosNo.Text:=IntToStr(li_PatientId);
     Le_HosNoKeyPress(Sender,Key);
     ...
     Begin
          ...
          Begin
               Clear;
               Add('Select Distinct PatientID From PendingBill.db Where PatientID='+IntToStr(li_PatientId));
          End;
          Open;
          If FieldByName('PatientID').AsInteger=0 Then
          Begin
               MessageDlg('This Patient Has No Bill Pending.',mtConfirmation,[mbok],0);
               Exit;
          End;
     End;  }
     with Table_Billing do
     ...
     begin
          ...
          Begin
               Clear;
               Add('Select * From PendingBill.db Where PatientID='+IntToStr(li_PatientId));
               if li_Inpatientid<>0 then
               SQL.Add('and Inpatientid='+IntToStr(li_Inpatientid))
               else if gs_EMRNoForMedPT<>'' then
               SQL.Add('and emrno='+QuotedStr(gs_EMRNoForMedPT));
          ...
          While Not Eof Do
          Begin
               Table_Billing.Append;
               Table_Billing.Fieldbyname('SNO').AsInteger :=FieldByName('SNO').AsInteger;
               Table_Billing.Fieldbyname('DepID').AsInteger := Fieldbyname('DepID').AsInteger;
               Table_Billing.Fieldbyname('PatientID').AsInteger:=Fieldbyname('PatientID').AsInteger;
               Table_Billing.Fieldbyname('ServiceBillDetailID').AsInteger :=Fieldbyname('ServiceBillDetailID').AsInteger;
               Table_Billing.Fieldbyname('BillDetailID').AsInteger := Fieldbyname('BillDetailID').AsInteger;
               Table_Billing.Fieldbyname('BillNo').AsString :=Fieldbyname('BillNo').AsString;
               Table_Billing.Fieldbyname('InPatientID').AsInteger := Fieldbyname('InPatientID').AsInteger;
               Table_Billing.Fieldbyname('PatientTestID').AsInteger :=Fieldbyname('PatientTestID').AsInteger;
               Table_Billing.Fieldbyname('DocCode').AsString := Fieldbyname('DocCode').AsString;
               Table_Billing.Fieldbyname('DocID').AsInteger :=Fieldbyname('DocID').AsInteger;
               Table_Billing.Fieldbyname('AccountId').AsInteger :=Fieldbyname('AccountId').AsInteger;
               Table_Billing.Fieldbyname('TestNameCode').AsString := Fieldbyname('TestNameCode').AsString;
               Table_Billing.Fieldbyname('TNCategoryCode').AsString :=Fieldbyname('TNCategoryCode').AsString;
               Table_Billing.Fieldbyname('TestNameId').AsInteger :=Fieldbyname('TestNameId').AsInteger;
               Table_Billing.Fieldbyname('TestPackageId').AsInteger := Fieldbyname('TestPackageId').AsInteger;
               Table_Billing.Fieldbyname('TestName').AsString :=Fieldbyname('TestName').AsString;
               Table_Billing.Fieldbyname('TestPrice').AsFloat := Fieldbyname('TestPrice').AsFloat;
               Table_Billing.Fieldbyname('PrevTestPrice').AsFloat := Fieldbyname('PrevTestPrice').AsFloat;
               Table_Billing.Fieldbyname('TestPriceWithTax').AsFloat :=Fieldbyname('TestPriceWithTax').AsFloat;
               Table_Billing.Fieldbyname('TestPriceAfterDiscount').AsFloat := Fieldbyname('TestPriceAfterDiscount').AsFloat;
               Table_Billing.Fieldbyname('Qty').AsFloat :=Fieldbyname('Qty').AsFloat;
               Table_Billing.Fieldbyname('PrevQty').AsFloat :=Fieldbyname('PrevQty').AsFloat;
               Table_Billing.Fieldbyname('RefundQty').AsFloat := Fieldbyname('RefundQty').AsFloat;
               Table_Billing.Fieldbyname('TotalPrice').AsFloat := Fieldbyname('TotalPrice').AsFloat;
               Table_Billing.Fieldbyname('SvrTax').AsFloat := Fieldbyname('SvrTax').AsFloat;
               Table_Billing.Fieldbyname('DisPer').AsFloat := Fieldbyname('DisPer').AsFloat;
               Table_Billing.Fieldbyname('Discount').AsFloat := Fieldbyname('Discount').AsFloat;
               Table_Billing.Fieldbyname('NetTotal').AsFloat :=Fieldbyname('NetTotal').AsFloat;
               Table_Billing.Fieldbyname('RefundTotal').AsFloat := Fieldbyname('RefundTotal').AsFloat;
               Table_Billing.Fieldbyname('PharmacyCost').AsFloat := Fieldbyname('PharmacyCost').AsFloat;
               Table_Billing.Fieldbyname('CommunityID').AsInteger := Fieldbyname('CommunityID').AsInteger;
               Table_Billing.Fieldbyname('SchemeID').AsInteger := Fieldbyname('SchemeID').AsInteger;
               Table_Billing.Fieldbyname('RefDocCode').AsString := Fieldbyname('RefDocCode').AsString;
               Table_Billing.Fieldbyname('RefDocID').AsInteger := Fieldbyname('RefDocID').AsInteger;
               Table_Billing.Fieldbyname('Status').AsString := Fieldbyname('Status').AsString;
               Table_Billing.Fieldbyname('RefundBillNo').AsString := Fieldbyname('RefundBillNo').AsString;
               Table_Billing.Fieldbyname('IsDiscountable').AsString :=Fieldbyname('IsDiscountable').AsString;
               Table_Billing.Fieldbyname('IsVatable').AsString := Fieldbyname('IsVatable').AsString;
               Table_Billing.Fieldbyname('ISPACKAGETEST').AsString := Fieldbyname('ISPACKAGETEST').AsString;
               Table_Billing.Fieldbyname('ISFractionableTest').AsString := Fieldbyname('ISFractionableTest').AsString;
               Table_Billing.Fieldbyname('User').AsString := Fieldbyname('User').AsString;
               Table_Billing.Fieldbyname('ChargeType').AsString := Fieldbyname('ChargeType').AsString;
               Table_Billing.Fieldbyname('TestProgressStatus').AsInteger := Fieldbyname('TestProgressStatus').AsInteger;
               Table_Billing.Fieldbyname('BillDate').AsString := Fieldbyname('BillDate').AsString;
               Table_Billing.Fieldbyname('BillTime').AsString := Fieldbyname('BillTime').AsString;
               Table_Billing.Fieldbyname('DisplayOrder').AsString := Fieldbyname('DisplayOrder').AsString;
               Table_Billing.Fieldbyname('DepType').AsString := Fieldbyname('DepType').AsString;
               Table_Billing.Fieldbyname('LabDepID').AsInteger := Fieldbyname('LabDepID').AsInteger;
               Table_Billing.Fieldbyname('RateType').AsString := Fieldbyname('RateType').AsString;
               Table_Billing.Fieldbyname('VerifiedBy').AsInteger := Fieldbyname('VerifiedBy').AsInteger;
               Table_Billing.Fieldbyname('OrgBillCategory').AsString := Fieldbyname('OrgBillCategory').AsString;
               Table_Billing.Fieldbyname('HOSPARTPER').AsString := Fieldbyname('HOSPARTPER').AsString;
               Table_Billing.Fieldbyname('IsCoPaymentItem').AsString := Fieldbyname('IsCoPaymentItem').AsString;
               Table_Billing.Fieldbyname('CoPaymentItemPercent').AsFloat := Fieldbyname('CoPaymentItemPercent').AsFloat;
               Table_Billing.Fieldbyname('IsRecentBilled').AsString := Fieldbyname('IsRecentBilled').AsString;
               Table_Billing.Fieldbyname('PatientTypeCode').AsString := Fieldbyname('PatientTypeCode').AsString;
               Table_Billing.Fieldbyname('IsValidForRefund').AsString := Fieldbyname('IsValidForRefund').AsString;

               Table_Billing.Fieldbyname('REFFRACTIONPER').AsFloat := Fieldbyname('REFFRACTIONPER').AsFloat;
               Table_Billing.Fieldbyname('REFFRACTIONAMOUNT').AsFloat := Fieldbyname('REFFRACTIONAMOUNT').AsFloat;
               Table_Billing.Fieldbyname('REFERRALFRACTIONID').AsInteger := Fieldbyname('REFERRALFRACTIONID').AsInteger;

               Table_Billing.Fieldbyname('WardId').AsInteger := Fieldbyname('WardId').AsInteger;

               Table_Billing.Fieldbyname('DOCREFFRACTIONPER').AsFloat := Fieldbyname('DOCREFFRACTIONPER').AsFloat;
               Table_Billing.Fieldbyname('DOCREFFRACTIONAMOUNT').AsFloat := Fieldbyname('DOCREFFRACTIONAMOUNT').AsFloat;
               Table_Billing.Fieldbyname('DOCREFERRALFRACTIONID').AsInteger := Fieldbyname('DOCREFERRALFRACTIONID').AsInteger;
               Table_Billing.Fieldbyname('Isot').AsString := Fieldbyname('Isot').AsString;
               Table_Billing.Fieldbyname('PatientCategory').AsString := Fieldbyname('PatientCategory').AsString;
               Table_Billing.Post;
               Next;
          End;
     end;
     ...
     begin
          ...
          Begin
               Clear;
               Add('Select * From PendingFraction.db Where PatientID='+IntToStr(li_PatientId));
               if li_Inpatientid<>0 then
               SQL.Add('and Inpatientid='+IntToStr(li_Inpatientid))
               else if gs_EMRNoForMedPT<>'' then
               SQL.Add('and emrno='+QuotedStr(gs_EMRNoForMedPT));
          ...
          While Not Eof Do
          Begin
               Table_Fraction.Append;
               Table_Fraction.FieldByName('Sno').AsInteger := FieldByName('Sno').AsInteger;
               Table_Fraction.FieldByName('InpatientId').AsInteger := FieldByName('InpatientId').AsInteger;
               Table_Fraction.FieldByName('BillDetailId').AsInteger := FieldByName('BillDetailId').AsInteger;
               Table_Fraction.FieldByName('DepId').AsInteger := FieldByName('DepId').AsInteger;
               Table_Fraction.FieldByName('TestNameId').AsInteger := FieldByName('TestNameId').AsInteger;
               Table_Fraction.FieldByName('TestNameCode').AsString := FieldByName('TestNameCode').AsString;
               Table_Fraction.FieldByName('DocId').AsInteger := FieldByName('DocId').AsInteger;
               Table_Fraction.FieldByName('DocName').AsString := FieldByName('DocName').AsString;
               Table_Fraction.FieldByName('Description').AsString := FieldByName('Description').AsString;

               Table_Fraction.FieldByName('FractionId').AsInteger := FieldByName('FractionId').AsInteger;
               Table_Fraction.FieldByName('PositionWiseFractionId').AsInteger := FieldByName('PositionWiseFractionId').AsInteger;

               Table_Fraction.FieldByName('MergePercent').AsFloat := FieldByName('MergePercent').AsFloat;
               Table_Fraction.FieldByName('MergeAmount').AsFloat := FieldByName('MergeAmount').AsFloat;

               Table_Fraction.FieldByName('FractionRate').AsFloat := FieldByName('FractionRate').AsFloat;
               Table_Fraction.FieldByName('FractionAmount').AsFloat := FieldByName('FractionAmount').AsFloat;
               Table_Fraction.FieldByName('FractionAmountOrg').AsFloat := FieldByName('FractionAmountOrg').AsFloat;
               Table_Fraction.FieldByName('UnitTestCost').AsFloat := FieldByName('UnitTestCost').AsFloat;
               Table_Fraction.FieldByName('DisPer').AsFloat := FieldByName('DisPer').AsFloat;
               Table_Fraction.FieldByName('Qty').AsFloat := FieldByName('Qty').AsFloat;

               Table_Fraction.FieldByName('IsFixedPersonForFraction').AsString := FieldByName('IsFixedPersonForFraction').AsString;
               Table_Fraction.FieldByName('IsFractionPropWithDisPer').AsString := FieldByName('IsFractionPropWithDisPer').AsString;
               //Table_Fraction.FieldByName('IsDocCompAtBillingForSingFrct').AsString := FieldByName('IsDocCompAtBillingForSingFrct').AsString;
               Table_Fraction.FieldByName('IsFrctPostAtReportingTime').AsString := FieldByName('IsFrctPostAtReportingTime').AsString;

               Table_Fraction.FieldByName('ISCOMPFORFRACTION').AsString := FieldByName('ISCOMPFORFRACTION').AsString;
               Table_Fraction.FieldByName('IsOperation').AsString := FieldByName('IsOperation').AsString;
               Table_Fraction.FieldByName('IsHospitalPart').AsString := FieldByName('IsHospitalPart').AsString;
               Table_Fraction.FieldByName('IsRevenueSeparatehead').AsString := FieldByName('IsRevenueSeparatehead').AsString;
               Table_Fraction.Post;
               Next;
          End;
     end;
...
procedure TForm_Billing.Btn_KeepPendingClick(Sender: TObject);
var
     li_PatientId:Integer;
begin
     CreateTempPendingBill;
     if (Trim(Le_HosNo.Text) = '') and (Trim(Le_HosNo.Text) <= '0') then Exit;

     li_PatientId := StrToInt(Trim(Le_HosNo.Text));
     ...
     begin
          ...
          while not eof do
          begin
               Table_PendingBills.Append;
               Table_PendingBills.Fieldbyname('SNO').AsInteger :=FieldByName('SNO').AsInteger;
               Table_PendingBills.Fieldbyname('DepID').AsInteger := Fieldbyname('DepID').AsInteger;
               Table_PendingBills.Fieldbyname('PatientID').AsInteger:=li_PatientId;
               Table_PendingBills.FieldByName('EmrNo').AsString :=gs_EMRNoForMedPT;
               Table_PendingBills.Fieldbyname('ServiceBillDetailID').AsInteger :=Fieldbyname('ServiceBillDetailID').AsInteger;
               Table_PendingBills.Fieldbyname('BillDetailID').AsInteger := Fieldbyname('BillDetailID').AsInteger;
               Table_PendingBills.Fieldbyname('BillNo').AsString :=Fieldbyname('BillNo').AsString;
               Table_PendingBills.Fieldbyname('InPatientID').AsInteger := gi_InPatientID;
               Table_PendingBills.Fieldbyname('PatientTestID').AsInteger :=Fieldbyname('PatientTestID').AsInteger;
               Table_PendingBills.Fieldbyname('DocCode').AsString := Fieldbyname('DocCode').AsString;
               Table_PendingBills.Fieldbyname('DocID').AsInteger :=Fieldbyname('DocID').AsInteger;
               Table_PendingBills.Fieldbyname('AccountId').AsInteger :=Fieldbyname('AccountId').AsInteger;
               Table_PendingBills.Fieldbyname('TestNameCode').AsString := Fieldbyname('TestNameCode').AsString;
               Table_PendingBills.Fieldbyname('TNCategoryCode').AsString :=Fieldbyname('TNCategoryCode').AsString;
               Table_PendingBills.Fieldbyname('TestNameId').AsInteger :=Fieldbyname('TestNameId').AsInteger;
               Table_PendingBills.Fieldbyname('TestPackageId').AsInteger := Fieldbyname('TestPackageId').AsInteger;
               Table_PendingBills.Fieldbyname('TestName').AsString :=Fieldbyname('TestName').AsString;
               Table_PendingBills.Fieldbyname('TestPrice').AsFloat := Fieldbyname('TestPrice').AsFloat;
               Table_PendingBills.Fieldbyname('PrevTestPrice').AsFloat := Fieldbyname('PrevTestPrice').AsFloat;
               Table_PendingBills.Fieldbyname('TestPriceWithTax').AsFloat :=Fieldbyname('TestPriceWithTax').AsFloat;
               Table_PendingBills.Fieldbyname('TestPriceAfterDiscount').AsFloat := Fieldbyname('TestPriceAfterDiscount').AsFloat;
               Table_PendingBills.Fieldbyname('Qty').AsFloat :=Fieldbyname('Qty').AsFloat;
               Table_PendingBills.Fieldbyname('PrevQty').AsFloat :=Fieldbyname('PrevQty').AsFloat;
               Table_PendingBills.Fieldbyname('RefundQty').AsFloat := Fieldbyname('RefundQty').AsFloat;
               Table_PendingBills.Fieldbyname('TotalPrice').AsFloat := Fieldbyname('TotalPrice').AsFloat;
               Table_PendingBills.Fieldbyname('SvrTax').AsFloat := Fieldbyname('SvrTax').AsFloat;
               Table_PendingBills.Fieldbyname('DisPer').AsFloat := Fieldbyname('DisPer').AsFloat;
               Table_PendingBills.Fieldbyname('Discount').AsFloat := Fieldbyname('Discount').AsFloat;
               Table_PendingBills.Fieldbyname('NetTotal').AsFloat :=Fieldbyname('NetTotal').AsFloat;
               Table_PendingBills.Fieldbyname('RefundTotal').AsFloat := Fieldbyname('RefundTotal').AsFloat;
               Table_PendingBills.Fieldbyname('PharmacyCost').AsFloat := Fieldbyname('PharmacyCost').AsFloat;
               Table_PendingBills.Fieldbyname('CommunityID').AsInteger := Fieldbyname('CommunityID').AsInteger;
               Table_PendingBills.Fieldbyname('SchemeID').AsInteger := Fieldbyname('SchemeID').AsInteger;
               Table_PendingBills.Fieldbyname('RefDocCode').AsString := Fieldbyname('RefDocCode').AsString;
               Table_PendingBills.Fieldbyname('RefDocID').AsInteger := Fieldbyname('RefDocID').AsInteger;
               Table_PendingBills.Fieldbyname('Status').AsString := Fieldbyname('Status').AsString;
               Table_PendingBills.Fieldbyname('RefundBillNo').AsString := Fieldbyname('RefundBillNo').AsString;
               Table_PendingBills.Fieldbyname('IsDiscountable').AsString :=Fieldbyname('IsDiscountable').AsString;
               Table_PendingBills.Fieldbyname('IsVatable').AsString := Fieldbyname('IsVatable').AsString;
               Table_PendingBills.Fieldbyname('ISPACKAGETEST').AsString := Fieldbyname('ISPACKAGETEST').AsString;
               Table_PendingBills.Fieldbyname('ISFractionableTest').AsString := Fieldbyname('ISFractionableTest').AsString;
               Table_PendingBills.Fieldbyname('User').AsString := Fieldbyname('User').AsString;
               Table_PendingBills.Fieldbyname('ChargeType').AsString := Fieldbyname('ChargeType').AsString;
               Table_PendingBills.Fieldbyname('TestProgressStatus').AsInteger := Fieldbyname('TestProgressStatus').AsInteger;
               Table_PendingBills.Fieldbyname('BillDate').AsString := Fieldbyname('BillDate').AsString;
               Table_PendingBills.Fieldbyname('BillTime').AsString := Fieldbyname('BillTime').AsString;
               Table_PendingBills.Fieldbyname('DisplayOrder').AsString := Fieldbyname('DisplayOrder').AsString;
               Table_PendingBills.Fieldbyname('DepType').AsString := Fieldbyname('DepType').AsString;
               Table_PendingBills.Fieldbyname('LabDepID').AsInteger := Fieldbyname('LabDepID').AsInteger;
               Table_PendingBills.Fieldbyname('RateType').AsString := Fieldbyname('RateType').AsString;
               Table_PendingBills.Fieldbyname('VerifiedBy').AsInteger := Fieldbyname('VerifiedBy').AsInteger;
               Table_PendingBills.Fieldbyname('OrgBillCategory').AsString := Fieldbyname('OrgBillCategory').AsString;
               Table_PendingBills.Fieldbyname('HOSPARTPER').AsString := Fieldbyname('HOSPARTPER').AsString;
               Table_PendingBills.Fieldbyname('IsCoPaymentItem').AsString := Fieldbyname('IsCoPaymentItem').AsString;
               Table_PendingBills.Fieldbyname('CoPaymentItemPercent').AsFloat := Fieldbyname('CoPaymentItemPercent').AsFloat;
               Table_PendingBills.Fieldbyname('IsRecentBilled').AsString := Fieldbyname('IsRecentBilled').AsString;
               Table_PendingBills.Fieldbyname('PatientTypeCode').AsString := Fieldbyname('PatientTypeCode').AsString;
               Table_PendingBills.Fieldbyname('IsValidForRefund').AsString := Fieldbyname('IsValidForRefund').AsString;

               Table_PendingBills.Fieldbyname('REFFRACTIONPER').AsFloat := Fieldbyname('REFFRACTIONPER').AsFloat;
               Table_PendingBills.Fieldbyname('REFFRACTIONAMOUNT').AsFloat := Fieldbyname('REFFRACTIONAMOUNT').AsFloat;
               Table_PendingBills.Fieldbyname('REFERRALFRACTIONID').AsInteger := Fieldbyname('REFERRALFRACTIONID').AsInteger;

               Table_PendingBills.Fieldbyname('WardId').AsInteger := Fieldbyname('WardId').AsInteger;

               Table_PendingBills.Fieldbyname('DOCREFFRACTIONPER').AsFloat := Fieldbyname('DOCREFFRACTIONPER').AsFloat;
               Table_PendingBills.Fieldbyname('DOCREFFRACTIONAMOUNT').AsFloat := Fieldbyname('DOCREFFRACTIONAMOUNT').AsFloat;
               Table_PendingBills.Fieldbyname('DOCREFERRALFRACTIONID').AsInteger := Fieldbyname('DOCREFERRALFRACTIONID').AsInteger;
               Table_PendingBills.Fieldbyname('Isot').AsString := Fieldbyname('Isot').AsString;
               Table_PendingBills.Fieldbyname('PatientCategory').AsString := Fieldbyname('PatientCategory').AsString;
               Table_PendingBills.Post;
               Next;
          end;
     end;
     with Table_Fraction do
     begin
          ...
          while not eof do
          begin
               Table_PendingFraction.Append;
               Table_PendingFraction.FieldByName('Sno').AsInteger := FieldByName('Sno').AsInteger;
               Table_PendingFraction.FieldByName('PatientId').AsInteger := li_PatientId;
               Table_PendingFraction.FieldByName('EmrNo').AsString :=gs_EMRNoForMedPT;
               Table_PendingFraction.FieldByName('InpatientId').AsInteger := gi_InPatientID;
               Table_PendingFraction.FieldByName('BillDetailId').AsInteger := FieldByName('BillDetailId').AsInteger;
               Table_PendingFraction.FieldByName('DepId').AsInteger := FieldByName('DepId').AsInteger;
               Table_PendingFraction.FieldByName('TestNameId').AsInteger := FieldByName('TestNameId').AsInteger;
               Table_PendingFraction.FieldByName('TestNameCode').AsString := FieldByName('TestNameCode').AsString;
               Table_PendingFraction.FieldByName('DocId').AsInteger := FieldByName('DocId').AsInteger;
               Table_PendingFraction.FieldByName('DocName').AsString := FieldByName('DocName').AsString;
               Table_PendingFraction.FieldByName('Description').AsString := FieldByName('Description').AsString;

               Table_PendingFraction.FieldByName('FractionId').AsInteger := FieldByName('FractionId').AsInteger;
               Table_PendingFraction.FieldByName('PositionWiseFractionId').AsInteger := FieldByName('PositionWiseFractionId').AsInteger;

               Table_PendingFraction.FieldByName('MergePercent').AsFloat := FieldByName('MergePercent').AsFloat;
               Table_PendingFraction.FieldByName('MergeAmount').AsFloat := FieldByName('MergeAmount').AsFloat;

               Table_PendingFraction.FieldByName('FractionRate').AsFloat := FieldByName('FractionRate').AsFloat;
               Table_PendingFraction.FieldByName('FractionAmount').AsFloat := FieldByName('FractionAmount').AsFloat;
               Table_PendingFraction.FieldByName('FractionAmountOrg').AsFloat := FieldByName('FractionAmountOrg').AsFloat;
               Table_PendingFraction.FieldByName('UnitTestCost').AsFloat := FieldByName('UnitTestCost').AsFloat;
               Table_PendingFraction.FieldByName('DisPer').AsFloat := FieldByName('DisPer').AsFloat;
               Table_PendingFraction.FieldByName('Qty').AsFloat := FieldByName('Qty').AsFloat;

               Table_PendingFraction.FieldByName('IsFixedPersonForFraction').AsString := FieldByName('IsFixedPersonForFraction').AsString;
               Table_PendingFraction.FieldByName('IsFractionPropWithDisPer').AsString := FieldByName('IsFractionPropWithDisPer').AsString;
               //Table_PendingFraction.FieldByName('IsDocCompAtBillingForSingFrct').AsString := FieldByName('IsDocCompAtBillingForSingFrct').AsString;
               Table_PendingFraction.FieldByName('IsFrctPostAtReportingTime').AsString := FieldByName('IsFrctPostAtReportingTime').AsString;

               Table_PendingFraction.FieldByName('ISCOMPFORFRACTION').AsString := FieldByName('ISCOMPFORFRACTION').AsString;
               Table_PendingFraction.FieldByName('IsOperation').AsString := FieldByName('IsOperation').AsString;
               Table_PendingFraction.FieldByName('IsHospitalPart').AsString := FieldByName('IsHospitalPart').AsString;
               Table_PendingFraction.FieldByName('IsRevenueSeparatehead').AsString := FieldByName('IsRevenueSeparatehead').AsString;
               Table_PendingFraction.Post;
               Next;
          end;
     end;
     ShowDoneMessage;
...
begin
     ...
     begin
          ...
          if CB_DayCare.Checked = True then
          begin
          pf_Deposit := GetTotalDepositRefNo(ps_RefundBillNo);
          pf_AdvancRefund := pf_Deposit;
          end;
          ...
          Begin
               ...
               Begin
                    ...
                    Else if (Copy(Edit_PreveBillNo.Text, 1, 2) = 'CS')  then
                         Label_CoPaymentAmt.Caption := 'Cash Refund Amt. :' + FloatToStr(pf_grandtotal)
                    else if (gs_BillType='SERVICE REFUNDBILL') then
                         Label_CoPaymentAmt.Caption := 'Service Refund Amt. :' + FloatToStr(pf_grandtotal)
                    else
                    Begin
          ...
          Le_GrandTotal.Text := FormatFloat('#0,0.00', pf_grandtotal);
          Le_NetBalance.Text := FormatFloat('#0,0.00', pf_balance);
          le_TotalDeposit.Text := FormatFloat('#0,0.00', pf_Deposit);      //'0.00';
...
begin
     ...
     begin
          ...
          if (gi_BillCase = 1) and ((gs_CalledFrom<>'FinalBilling')) then
          Begin
               Close;
               DatabaseName := gs_temppath;
               SQL.Clear;
...
begin
     ...
     pf_Return := Abs(pf_Payment +pf_deposit- pf_grandtotal);
...
procedure TForm_Billing.CB_DelAllClick(Sender: TObject);
Var
     Query:TQuery;
begin
     Query := TQuery.Create(nil);
     if CB_DelAll.Checked=True then
     begin
          if (gs_BillType = 'REFUNDBILL') or (gs_BillType='SERVICE REFUNDBILL') and
          ( Table_Billing.FieldByName('Status').AsString <> 'R' ) then
          begin
               if not MsgBox(1002, 1, '', '', '') then
                  Exit;
          ...
          begin
               ...
               begin
                  ...
                  SQL.Add(' Where ServiceBillDetailID=' + IntToStr
                      (Table_Billing.FieldByName('ServiceBillDetailID').AsInteger));
                  //li_TestNameId := Query_TempBilling.FieldByName('TestNameID').AsInteger;
                  //li_ServiceBillDetailID := Table_Billing.FieldByName('ServiceBillDetailID').AsInteger;
                  ExecSQL;
               end;
               Table_Billing.Next;
          end;
          ...
          Table_Billing.Close;
          Table_Billing.Open;
          Table_Billing.Locate('ServiceBillDetailID', Table_Billing.FieldByName('ServiceBillDetailID').AsInteger, []);
          end;
     end
     else if CB_DelAll.Checked=False then
     begin
          if DBLCB_Scheme.KeyValue = '22' then // 22 - Medicare Scheme
          Begin
          if (Table_Billing.FieldByName('TestNameID').AsInteger <> 0) then
          ...
          begin
               ...
               begin
                  ...
                  DatabaseName := gs_temppath;
                  SQL.Clear;
                  SQL.Add(' Update Billing Set Status=''''');
                  SQL.Add(' ,IsCoPaymentItem=''''' +
                      ',CoPaymentItemPercent=' + FloatToStr(pf_PercentageAmt));
                  SQL.Add(' Where ServiceBillDetailID=' + IntToStr
                      (Table_Billing.FieldByName('ServiceBillDetailID').AsInteger));
                  //li_TestNameId := Query_TempBilling.FieldByName('TestNameID').AsInteger;
...
procedure TForm_Billing.tmr1Timer(Sender: TObject);
begin
     If lbl_memtype.Visible then
     begin
     lbl_memtype.Visible:=False;
     end
     Else
...
Begin
  ...
  Begin
    ...
    Open;
  End;
  Result := Query_TempProcess.FieldByName('MaxNo').AsInteger + 1;
End;
...
Var
  li_RfDocId,li_DocId, li_Sno, li_TotNoForFract: Integer;
...
Begin
     ...
     Begin
          IF Trim(DBLCB_RefDocCode.Text) <> '' Then
          Begin
              // li_DocId := Query_ReferingDocDept.FieldByName('DOCID').AsInteger;
               //ls_DocCode := Query_ReferingDocDept.FieldByName('docode').AsString;
               li_DocId := Query_Doctor.FieldByName('DOCID').AsInteger;
               ls_DocCode := Query_Doctor.FieldByName('doccode').AsString;
               li_RfDocId:=Query_Doctor.FieldByName('DOCID').AsInteger;
          End
          Else
          Begin
          ...
          begin
               ...
               Begin
                    ...
                    Begin
                         ...
                         Begin
                              Query_ReferingDocDept.close;
                              Query_ReferingDocDept.sql.savetofile('c:/ss.txt');
                              Query_ReferingDocDept.open;
          ...
          else // From Fraction Setup Specail
          begin
               with Query_Process do
               begin
                    Close;
                    Session := DM_Hospital.DB;
                    SQL.Clear;
                    SQL.Add(' Select * From hs_frsp_FractionSpecial Where frsp_IsFractionActive=''Y''');
                    SQL.Add(' and frsp_DocCode='+#39+ls_DocCode+#39);
                    Open;
               end;


               if Query_Process.FieldByName('frsp_FrctSPLId').AsInteger > 0 then
               Begin
                    With Query_TempProcess do
                    Begin
                         Close;
                    ...
                    Begin
                         ...
                         li_Sno:=li_Sno+1;
                         FieldbyName('Sno') .AsInteger:=li_Sno;
                         FieldbyName('DocId') .AsInteger:=li_RfDocId;
                         FieldbyName('PositionWiseFractionId') .AsInteger:=0;
                         FieldbyName('IsFixedPersonForFraction') .AsString:='Y';
                         FieldbyName('IsFractionPropWithDisPer') .AsString:='Y';
                         FieldbyName('IsDocCompAtBillingForSing') .AsString:='Y';
                         FieldbyName('IsFrctPostAtReportingTime') .AsString:='N';
                         FieldbyName('IsHospitalPart') .AsString:='N';
                         FieldbyName('TestNameCode') .AsString:=TestNameCode;
                         FieldbyName('TestNameId') .AsInteger:=TestNameId;
                         FieldbyName('DepId') .AsInteger:=DepId;
                         FieldbyName('UnitTestCost') .AsFloat:=UnitTestCost;
                         FieldbyName('Description') .AsString:='SPECIAL SETUP';
                         FieldbyName('ISCOMPFORFRACTION') .AsString:='Y';
                         FieldbyName('Qty') .Asfloat:=Qty;
                         FieldbyName('InpatientId') .AsInteger:=pi_InpatientId;
                         ...
                         if TNCategoryCode='GEN' then
                         lf_FractionRate:=Query_Process.FieldByName('FRSP_FractionRateGen').AsFloat
                         else
                         lf_FractionRate:=Query_Process.FieldByName('FRSP_FractionRateSPL').AsFloat;
...
var
  Qry_CopaymentAmt: TQuery;
begin
  // for coPayment
  Qry_CopaymentAmt := TQuery.Create(Nil);
...
Begin
  ...
  begin
    if RB_IPBILL.Checked = True then
      gi_BillCase := 0
    Else if (RB_OPBILL.Checked) and (gs_BillType='OPBILL') = True then
      gi_BillCase := 3
...
Var
  ...
  i: Integer;
begin
     //lbl_memtype.caption := DBLCB_Scheme.keyvalue;
     Query_RateType.Close;
     Query_RateType.Session := DM_Hospital.DB;
     Query_RateType.Open;
     //   BB_Save.Visible := False;
     //BB_Close.Visible := False;
...
begin
     ...
     if (Key = VK_F1) and (RB_OPBILL.Enabled=True) then
     Begin
          if gs_BillType <> 'REFUNDBILL' then
               gi_BillCase := 3;
          RB_OPBILL.Checked := True;
     ...
     if (Key = VK_F2) and (RB_IPBILL.Enabled=True) then
     Begin
          if gs_BillType <> 'REFUNDBILL' then
               gi_BillCase := 0;
...
begin
     ...
     begin
          ...
          Edit_TestNameCode.left := 18;
          Edit_TestNameCode.Width := 100;

          Edit_TestName.left := 120;
          Edit_TestName.Width := 343;

          Edit_UnitPrice.left := 464;

          Edit_Qty.left := 538;
          Edit_TotalPrice.left := 582;
          Edit_SvrTaxEntry.left := 648;


          Edit_Disper.left := 713;
          Edit_DisAmount.left := 774;
          SPB_AddItem.left := 841;
...
begin
    ...
    // GetReferralDocDepartment;
     ...
     begin
         ...
         // Btn_BringPending.Visible:=True;
         // Btn_KeepPending.Visible:=True;
     end;
     ...
     Begin
          ...
          Label_BillingStatus.Caption := 'Direct Billing';
          Label_BillingStatus.left := 78;

          CB_DayCare.Caption := 'Adjust Deposit';
          Label_InptNoCap.Visible := False;
          Label_IPNO.Visible := False;

          if gb_IsOPCreditBillBlock=True then
          begin
               CB_PayType.ItemIndex := 0;
               CB_PayType.Enabled:=False;
          end;
     ...
     begin
          ...
          RB_ERBill.Enabled:=False;
          RB_ERBill.Visible:=False;
          Label_BillingStatus.Caption := 'Service Billing';
          Label_BillingStatus.left := 78;
     ...
     Begin
          ...
          Begin
               ...
               CB_DayCare.Caption := 'Adjust Deposit At Refund';
               ...
               if gs_RefundType='IPREFUND' then
               begin
                    RB_IPBILL.Checked:=True;
     ...
     Begin
          ...
          if gs_BillType='SERVICE REFUNDBILL' then
          begin
               Label_BillingStatus.Caption := 'Service Refund Billing';
               Label_BillingStatus.left := 6;
               Label_BillingStatus.Top := 11;
          end
          else
          begin
               Label_BillingStatus.Caption := 'Refund Billing';
     ...
     Begin
          ...
          Le_HosNo.SetFocus;
     End
     else if (gi_BillCase=1) and (gs_BillType='OPBILL') then
     begin
          Le_HosNoKeyPress(Sender,key);
     end;
...
Function TForm_Billing.IsErAndCreditMemoRegPatient : Boolean;
Begin
     With Query_Process do
     Begin
          Close;
          Session := DM_Hospital.DB;
          SQL.Clear;
          SQL.Add(' Select * From VW_HS_MEDICAREEMERGENCYLIST where PatientID='+IntToStr(gi_PatientId));
          SQL.Add(' and ISEMRSERVICECOMPLETED=''N''');
          Open;
     End;

     if Query_Process.FieldByName('PatientID').AsInteger > 0 then
     Result:=True
     Else
     Result:=False;
End;
