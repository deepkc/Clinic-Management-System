unit Unit_QrDischargeBill_BNB;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, pngimage, QRCtrls, QuickRpt, ExtCtrls, DB, DBTables, DBAccess, Ora, OraSmart, MemDS, OraError, qrBarcode;

type
  TForm_QrDisChargeBill_BNB = class(TForm)
    ...
    procedure Qr_DischargeBillBeforePrint(Sender: TCustomQuickRep;
...
var
  Form_QrDisChargeBill_BNB: TForm_QrDisChargeBill_BNB;
...
procedure TForm_QrDisChargeBill_BNB.BillHeaderInfo;
begin
...
procedure TForm_QrDisChargeBill_BNB.lbl_headNamePrint(sender: TObject;
  var Value: string);
begin
     pi_Sno:=0;
end;

procedure TForm_QrDisChargeBill_BNB.Lbl_HosNoPrint(sender: TObject;
  var Value: string);
begin
     pi_Sno:=0;
end;

procedure TForm_QrDisChargeBill_BNB.lbl_SchemePrint(sender: TObject;
  var Value: string);
begin
...
procedure TForm_QrDisChargeBill_BNB.LoadPatientInfo;
Var
     Qry: TOraQuery;
     ls_RefBillNo : String;
...
//          lbl_headName.Caption := 'TAX INVOICE';
     ...
     Begin
          ...
          sql.add('Where PaymentCategory is not null Group by PaymentCategory Order by Sn desc,PaymentCategory');
          Open;
     End;
...
procedure TForm_QrDisChargeBill_BNB.PageHeaderBand1AfterPrint(
...
procedure TForm_QrDisChargeBill_BNB.QRDBText3Print(sender: TObject;
  var Value: string);
begin
     pi_Sno := pi_Sno + 1;
     if (Query_DischargeBill.FieldByName('Sn').AsInteger>101) and (Query_DischargeBill.FieldByName('Sn').AsInteger<500) then
     begin
     pi_Sno:=pi_Sno-1;
     Value := '';
     end
...
procedure TForm_QrDisChargeBill_BNB.QRDBText4Print(sender: TObject;
  var Value: string);
begin
     Itemscount := Itemscount + 1;
end;

procedure TForm_QrDisChargeBill_BNB.QRDBText5Print(sender: TObject;
  var Value: string);
begin
     if (Query_DischargeBill.FieldByName('PaymentCategory').AsString='Bed Charges')
     and (Query_DischargeBill.FieldByName('Sn').AsString='501') then
     Value:='';
end;

procedure TForm_QrDisChargeBill_BNB.QRLabel70Print(sender: TObject;
  var Value: string);
begin
     Value:= gs_HospitalName;
end;

procedure TForm_QrDisChargeBill_BNB.QRLabel74Print(sender: TObject;
  var Value: string);
begin
...
procedure TForm_QrDisChargeBill_BNB.QRLabel_HosAddressPrint(sender: TObject;
  var Value: string);
begin
     Value := gs_HospitalAddress;
end;

procedure TForm_QrDisChargeBill_BNB.QRSubDetail1AfterPrint(
  Sender: TQRCustomBand; BandPrinted: Boolean);
begin
      if Itemscount > 28 then
     begin
//          lbl_contd.Enabled:=true;
          Qr_DischargeBill.NewPage;
          Itemscount:=0;
...
procedure TForm_QrDisChargeBill_BNB.Qr_DischargeBillAfterPrint(Sender: TObject);
begin
...
procedure TForm_QrDisChargeBill_BNB.Qr_DischargeBillBeforePrint(Sender: TCustomQuickRep; var PrintReport: Boolean);
Var
  lf_TopMargin:Double;
begin
     lf_TopMargin := Qr_DischargeBill.Page.TopMargin;
     Qr_DischargeBill.Page.TopMargin:=lf_TopMargin+GetReportHeaderHeight(gs_MacID,'Discharge Bill');
end;
