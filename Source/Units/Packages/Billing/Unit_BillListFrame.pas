...
uses
     ...
     fxn, dm, ServerDate,PatientClass,
     DateEditXControl_TLB, DB, DBTables, DBAccess, Ora, OraSmart, MemDS, OraError, Unit_QrDepositSlip_Manipal;
...
type
     TFrame_BillList = class(TFrame)
          ...
          Ds_List: TDataSource;
          DBGrid2: TDBGrid;
          QueryListR: TOraQuery;
          Ds_listR: TDataSource;
          BB_Refresh: TBitBtn;
          Label1: TLabel;
          Edit_Search: TEdit;
          BB_PrintDepositSlip: TBitBtn;
          ...
          procedure CB_BillTypeChange(Sender: TObject);
    procedure BB_PrintDepositSlipClick(Sender: TObject);
     private
...
procedure TFrame_BillList.BB_PrintDepositSlipClick(Sender: TObject);
begin
...
begin
     ...
     begin
          ...
          begin
               ...
               SQL[4] := 'AND DEPOSITTYPE=''ADVANCE''';
          end
          else if CB_BillType.ItemIndex = 3 then
          begin
               SQL[0] := 'SELECT DISTINCT PATIENTID,PATIENTNAME FROM VW_DEPOSITBILL ';
               SQL[4] := 'AND DEPOSITTYPE=''DEPOSIT''';
          end
          else if CB_BillType.ItemIndex = 4 then
          begin
               SQL[0] := 'SELECT DISTINCT PATIENTID,PATIENTNAME FROM VW_REFUNDBILL_LIST';
               SQL[4] := 'AND 99=99';
          end
          else if CB_BillType.ItemIndex = 5 then
          begin
               SQL[0] := 'SELECT DISTINCT PATIENTID,PATIENTNAME FROM VW_DEPOSITBILL';
               SQL[4] := 'AND DEPOSITTYPE=''DEPOSIT REFUND''';
          end;
          if StrToInt(Se_Days.text) > 0 then
               SQL[1] := 'Where BillDate> TO_CHAR(SYSDATE-' + Se_Days.text + ',''YYYY/MM/DD'')'
          Else
     ...
     begin
          ...
          begin
               ...
               with DBGrid1 do
               begin
                    Columns[1].Title.Caption := 'Bill No.';
                    Columns[3].Title.Caption := 'Bill Date';
                    Columns[4].Title.Caption := 'Bill Time';
          ...
          begin
               SQL[0] := 'SELECT * FROM VW_REFUNDBILL_LIST';
               SQL[4] := 'AND 99=99';
               with DBGrid1 do
               begin
...
procedure TFrame_BillList.CB_BillTypeChange(Sender: TObject);
begin
     Form_BillingParent:=TForm_BillingParent.create(nil);
     Form_BillingParent.lbl_patientname.Caption := '';
     Form_BillingParent.lbl_Hosno.Caption := '';
     lbl_Billno.Caption := '';
     ClearVariable;
     gs_CalledFrom:='FinalBilling';
     BB_RefreshClick(Sender);

     if CB_BillType.ItemIndex = 1 then
     begin
     ...
     if CB_BillType.ItemIndex in [3, 5] then
     begin
          Form_BillingParent.BB_FinalBilling.Enabled := False;
          BB_PrintDepositSlip.Visible := True;
     end
     else
     begin
          Form_BillingParent.BB_FinalBilling.Enabled := True;
          BB_PrintDepositSlip.Visible := False;
     end;
...
begin
     ...
     begin
          ...
          begin
               if Length(Edit_Search.text) = 15 then
               begin
                    Edit_HosNo.text := IntToStr(GetPatientIdFromBillNo(Edit_Search.text));
                    Edit_HosNoChange(Sender);
...
procedure TFrame_BillList.PrintDepositSlip;
Var
     i: Integer;
begin
     try
