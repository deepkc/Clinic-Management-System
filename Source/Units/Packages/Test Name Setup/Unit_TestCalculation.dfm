object Form_TestCalculation: TForm_TestCalculation
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = 'Test Calculation Category'
  ClientHeight = 366
  ClientWidth = 646
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = -41
    Top = 0
    Width = 690
    Height = 33
    TabOrder = 0
    object BitBtn1: TBitBtn
      Left = 603
      Top = 1
      Width = 84
      Height = 30
      Caption = 'Close'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Verdana'
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentDoubleBuffered = False
      ParentFont = False
      TabOrder = 0
      OnClick = BitBtn1Click
    end
    object BitBtn2: TBitBtn
      Left = 520
      Top = 1
      Width = 84
      Height = 30
      Caption = 'Save'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Verdana'
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      TabOrder = 1
      OnClick = BitBtn2Click
    end
    object BitBtn3: TBitBtn
      Left = 437
      Top = 1
      Width = 84
      Height = 30
      Caption = 'New'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Verdana'
      Font.Style = []
      ParentDoubleBuffered = False
      ParentFont = False
      TabOrder = 2
      OnClick = BitBtn3Click
    end
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 32
    Width = 646
    Height = 334
    ActivePage = View
    TabOrder = 1
    OnChange = PageControl1Change
    object TabSheet1: TTabSheet
      Caption = 'List'
      object DBGrid1: TDBGrid
        Left = 0
        Top = 0
        Width = 639
        Height = 307
        DataSource = DS_DbGrid
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnDblClick = DBGrid1DblClick
        Columns = <
          item
            Expanded = False
            FieldName = 'SN'
            Title.Alignment = taCenter
            Title.Caption = 'SN.'
            Width = 32
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CATN_SOURCETESTCODE'
            Title.Alignment = taCenter
            Title.Caption = 'SOURCE TESTCODE'
            Width = 104
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'testname'
            Title.Alignment = taCenter
            Title.Caption = 'TEST NAME'
            Width = 193
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'Test'
            Title.Alignment = taCenter
            Title.Caption = 'TEST'
            Width = 182
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CATN_TESTCODE'
            Title.Alignment = taCenter
            Title.Caption = 'TESTCODE'
            Width = 118
            Visible = True
          end>
      end
    end
    object View: TTabSheet
      Caption = 'View'
      ImageIndex = 1
      object Panel2: TPanel
        Left = -9
        Top = -12
        Width = 578
        Height = 280
        TabOrder = 0
        object Label1: TLabel
          Left = 132
          Top = 142
          Width = 66
          Height = 16
          Caption = 'Test Code :'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
        end
        object Label2: TLabel
          Left = 82
          Top = 75
          Width = 115
          Height = 16
          Caption = 'Source  Test Code :'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
        end
        object Label3: TLabel
          Left = 127
          Top = 44
          Width = 70
          Height = 16
          Caption = 'Test Name :'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
        end
        object Label4: TLabel
          Left = 165
          Top = 108
          Width = 32
          Height = 16
          Caption = 'Test :'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
        end
        object Edit_TestCode: TEdit
          Left = 201
          Top = 142
          Width = 176
          Height = 21
          CharCase = ecUpperCase
          TabOrder = 0
          OnKeyPress = Edit_TestCodeKeyPress
        end
        object DBLCB_TestName: TDBLookupComboBox
          Left = 202
          Top = 42
          Width = 174
          Height = 21
          DropDownWidth = 250
          KeyField = 'TESTNAMEID'
          ListField = 'TESTNAME'
          ListSource = DS_Testname
          TabOrder = 1
          OnClick = DBLCB_TestNameClick
          OnKeyDown = DBLCB_TestNameKeyDown
          OnKeyPress = DBLCB_TestNameKeyPress
        end
        object DBLCB_CalcCode: TDBLookupComboBox
          Left = 202
          Top = 73
          Width = 174
          Height = 21
          DropDownWidth = 250
          KeyField = 'CALCULATIONID'
          ListField = 'SOURCETESTCODE'
          ListSource = DS_Calcode
          TabOrder = 2
          OnKeyDown = DBLCB_CalcCodeKeyDown
          OnKeyPress = DBLCB_CalcCodeKeyPress
        end
        object DBLCB_Test: TDBLookupComboBox
          Left = 202
          Top = 106
          Width = 174
          Height = 21
          DropDownWidth = 250
          KeyField = 'Testid'
          ListField = 'Test'
          ListSource = DS_Test
          TabOrder = 3
          OnKeyDown = DBLCB_TestKeyDown
          OnKeyPress = DBLCB_TestKeyPress
        end
      end
    end
  end
  object Edit_TestName: TEdit
    Left = 154
    Top = 31
    Width = 196
    Height = 21
    CharCase = ecUpperCase
    TabOrder = 2
    OnChange = Edit_TestNameChange
  end
  object Edit_Test: TEdit
    Left = 349
    Top = 31
    Width = 184
    Height = 21
    TabOrder = 3
    OnChange = Edit_TestChange
  end
  object OraQuery_Calccode: TOraQuery
    SQL.Strings = (
      
        'select cana_CALCULATIONID as CALCULATIONID,cana_SOURCETESTCODE a' +
        's SOURCETESTCODE from lb_cana_calculationname where CANA_ISACTIV' +
        'E='#39'Y'#39)
    Left = 400
    Top = 264
  end
  object DS_Calcode: TDataSource
    DataSet = OraQuery_Calccode
    Left = 72
    Top = 245
  end
  object OraQuery_Testname: TOraQuery
    Session = DM_Hospital.DB
    SQL.Strings = (
      
        'select tena_testnameid testnameid, tena_testname testname from h' +
        's_tena_testname where tena_ISACTIVE='#39'Y'#39' order by tena_testname')
    Left = 320
    Top = 272
  end
  object DS_Testname: TDataSource
    DataSet = OraQuery_Testname
    Left = 152
    Top = 256
  end
  object DS_Test: TDataSource
    DataSet = OraQuery_Test
    Left = 136
    Top = 288
  end
  object OraQuery_Test: TOraQuery
    Session = DM_Hospital.DB
    SQL.Strings = (
      'select Testid,Test from test')
    Left = 264
    Top = 264
  end
  object OraQuery_DbGrid: TOraQuery
    Session = DM_Hospital.DB
    SQL.Strings = (
      
        'select rownum as sn,A.* from (select a.*,(select CANA_CALCULATIO' +
        'NID from lb_cana_calculationname where CANA_SOURCETESTCODE=a.CAT' +
        'N_SOURCETESTCODE)SOURCETESTCODE,(select tena_testname from hs_te' +
        'na_testname where tena_testnameid=a.catn_testnameid) as testname' +
        ',(select test from test where testid=a.catn_testid)as test from ' +
        'lb_catn_calculationtestname a order by CATN_SOURCETESTCODE)A')
    Left = 504
    Top = 264
  end
  object DS_DbGrid: TDataSource
    DataSet = OraQuery_DbGrid
    Left = 472
    Top = 224
  end
end
