...
type
  TFormInpMain = class(TForm)
    ...
    BtnCancel: TBitBtn;
    BtnSave: TBitBtn;
    BitBtnDischargeBill: TBitBtn;
    BitBtnBillBreakup: TBitBtn;
    BitBtn_ReferralDoc: TBitBtn;
    BitBtnUndo: TBitBtn;
    BitBtnCrBilling: TBitBtn;
    BitBtnDeposit: TBitBtn;
    BitBtnBedExchange: TBitBtn;
    Panel1: TPanel;
    PageControl1: TPageControl;
    TabSheet1: TTabSheet;
    RadioButton_OPBilling: TRadioButton;
    RadioButton_OPRefundBilling: TRadioButton;
    RB_OPAdvanceBilling: TRadioButton;
    RadioButton_IPBilling: TRadioButton;
    CheckBoxOPRef: TCheckBox;
    CheckBoxIPRefund: TCheckBox;
    Chk_photo: TCheckBox;
    ...
    LabelIpNo: TLabel;
    Label22: TLabel;
    CheckBoxIPTransfer: TCheckBox;
    CheckBoxIPDischarge: TCheckBox;
    Label11: TLabel;
    LabelDepTotal: TLabel;
    Label12: TLabel;
    LabelCB: TLabel;
    Edit_PreviousBillNo: TLabeledEdit;
    Label24: TLabel;
    ...
    ChkBoxAllDis: TCheckBox;
    ChkBoxOldbill: TCheckBox;
    ...
    GroupBoxOldTest: TPanel;
    ButtonCash: TSpeedButton;
    BitBtnBillCancel: TSpeedButton;
    BitbtnCommon: TSpeedButton;
    DBGridOldBill: TDBGrid;
    Edit_TestCancelRemarks: TLabeledEdit;
    ...
    DBGridListOfBill: TDBGrid;
    DBGrid_DocInvolved: TDBGrid;
    DBGridOldTest: TDBGrid;
    CB_NonRefundBillFromRefund: TCheckBox;
    ...
    procedure Edit_PreviousBillNoExit(Sender: TObject);
    procedure Edit_PreviousBillNoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure CB_AddedChrgForIPClick(Sender: TObject);
    ...
    procedure ChkBoxOldbillClick(Sender: TObject);
...
procedure TFormInpMain.CB_AddedChrgForIPClick(Sender: TObject);
begin
...
procedure TFormInpMain.ChkBoxOldbillClick(Sender: TObject);
begin
     BitbtnCommon.Enabled:=True;
     ...
     Begin
          ...
          IF RadioButton_IPBilling.Checked=True Then
          DBGridOldBill.Columns[4].Visible:=True
          Else
          DBGridOldBill.Columns[4].Visible:=False;

          IF ChkBoxAllDis.Checked Then
          ChkBoxAllDis.Checked:=False;

          IF Not ButtonCash.Visible Then
          IF RadioButton_IPBilling.Checked=False Then
          ButtonCash.Visible:=False;

          ButtonCash.Caption:='&Cash';

          BitbtnCommon.Visible:=True;
          BitbtnCommon.Caption:='&Reprint Bill';

          GroupBoxOldTest.Visible:=True;
          GroupBoxOldTest.Caption:='Old Bill';
          DBGridOldTest.Visible:=False;
          DBGridOldBill.Visible:=True;


          With QueryBillMaster Do
          Begin
                Close;
                IF (RadioButton_OPBilling.Checked=True) or (RB_OPAdvanceBilling.Checked=True) or (CB_NonRefundBillFromRefund.Checked=True) Then //
                Begin
                     sql[1]:=' BillNo,BillDate,(Select ExtraCharge From BillMaster where BillNo=BillDetail.BillNo) as ExtraCharge,'+
                             ' (Select UserName From UserMain where UserId=BillDetail.ModifyBy) UserName,';
                     sql[5]:=' (BillType=''B'' or BillType=''O'') and CancelStatus=''N'' and PatientId='+IntToStr(pi_PatientId);
                     sql[6]:=' ';
                     sql[7]:=' Group By BillNo,BillDate,BillType,ModifyBy,PatientId,SchemeID Order By BillDate,BillNo';

                End
                Else IF RadioButton_IPBilling.Checked=True Then
                Begin
                     sql[1]:=' CrBillNo as BillNo,CrModifyBy,CrModifyDate as BillDate,(Select ExtraCharge From BillMaster where BillNo=BillDetail.CrBillNo) as ExtraCharge,'+
                             ' (Select UserName From UserMain where UserId=BillDetail.CrModifyBy) UserName,';
                     sql[5]:=' (BillType=''IP'' or BillType=''A'') and Service<>''BED'' and CancelStatus=''N'' and InPatientId='+IntToStr(pi_InPatientId);
                     sql[6]:=' ';
                     sql[7]:=' Group By CrBillNo,CrModifyBy,CrModifyDate,BillType,ModifyBy,PatientId,SchemeID Order By CrModifyDate,CrBillNo';
                End
                Else     {     Refund Billing     }
                Begin
                     sql[1]:=' BillNo,BillDate,(Select ExtraCharge From BillMaster where BillNo=BillDetail.BillNo) as ExtraCharge,'+
                             ' (Select UserName From UserMain where UserId=BillDetail.ModifyBy) UserName,';
                     sql[5]:=' Service<>''BED''';
                     sql[6]:=' and (PayType=''REFUND'') and PatientId='+IntToStr(pi_PatientId);
                     sql[7]:=' Group By BillNo,BillDate,BillType,ModifyBy,PatientId,SchemeID Order By BillDate,BillNo';
                End;
                //sql.saveToFile('C:\Billing.Txt');
                Open;
          End;
          ButtonCash.Visible:=False;
...
procedure TFormInpMain.Edit_PreviousBillNoExit(Sender: TObject);
begin
      DBGridListOfBill.Visible:=False;
   IF Trim(Edit_PreviousBillNo.Text)<>'' Then
   Begin
      With Query_Process do
      Begin
         Close;
         sql.Clear;
         sql.add(' Select Distinct RateType From BillDetail where ');
         IF CheckBoxIPRefund.Checked=True Then
         sql.add(' CRBillNo='+#39+Edit_PreviousBillNo.Text+#39+' and InpatientId='+EditPsearch.Text)
         Else
         sql.add(' BillNo='+#39+Edit_PreviousBillNo.Text+#39+'and PatientId='+EditPsearch.Text);
         Open;
      End;
      IF Query_Process.FieldByName('RateType').AsString='GEN' Then
      CMBBillType.ItemIndex:=0
      Else IF Query_Process.FieldByName('RateType').AsString='PVT' Then
      CMBBillType.ItemIndex:=1
      Else IF Query_Process.FieldByName('RateType').AsString='SPL' Then
      CMBBillType.ItemIndex:=2
      Else IF Query_Process.FieldByName('RateType').AsString='FRG' Then
      CMBBillType.ItemIndex:=2; //3
   End;
end;

procedure TFormInpMain.Edit_PreviousBillNoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     IF (key=VK_Down) and (QueryListOfBill.Active)  Then
     Begin
          DBGridListOfBill.Visible:=True;
          QueryListOfBill.Next;
          Edit_PreviousBillNo.Text:=QueryListOfBill.FieldByName('BillNo').AsString;
          DBLC_Doctor.KeyValue:=QueryListOfBill.FieldByName('RefDocCode').AsString;
     End;

     IF (key=VK_Up) and (QueryListOfBill.Active)  Then
     Begin
          DBGridListOfBill.Visible:=True;
          QueryListOfBill.Prior;
          Edit_PreviousBillNo.Text:=QueryListOfBill.FieldByName('BillNo').AsString;
          DBLC_Doctor.KeyValue:=QueryListOfBill.FieldByName('RefDocCode').AsString;
     End;
...
procedure TFormInpMain.Edit_PreviousBillNoKeyPress(Sender: TObject; var Key: Char);
var ls_FromDate,ls_ToDate:String;
begin
     IF LabelNo.Caption='None' then Exit;

     IF Trim(Edit_PreviousBillNo.Text)='' Then
     Begin
          IF (Trim(EditPsearch.Text)<>'') and (CheckBoxOPRef.Checked=True) and (Key=#13) Then
          Begin
               DBGridListOfBill.Visible:=True;
               ls_ToDate:=ServerDate.TodaysDate;
               DEX_Control.text:=ls_ToDate;
               DEX_Control.ADDateAsDate:=DEX_Control.ADDateAsDate-60;
               ls_FromDate:=DEX_Control.Text;
               With QueryListOfBill do
               Begin
                    Close;
                    sql.Clear;
                    sql.add(' Select Distinct BillNo,BillDate,RefDocCode,ModifyBy From BillDetail where');
                    sql.add(' BillType=''B'' and (PayType=''CASH'' or PayType=''CREDIT'')');
                    sql.add(' and PatientId='+IntToStr(pi_PatientId));
                    sql.add(' And Service<>''OPCRD''');
                    sql.add(' Order By BillDate Desc,BillNO Desc');
                    Open;
               End;
          End
          Else IF (Trim(EditPsearch.Text)<>'') and (CheckBoxIPRefund.Checked=True) and (Key=#13) Then
          Begin
               DBGridListOfBill.Visible:=True;
               ls_ToDate:=ServerDate.TodaysDate;
               DEX_Control.text:=ls_ToDate;
               DEX_Control.ADDateAsDate:=DEX_Control.ADDateAsDate-200;
               ls_FromDate:=DEX_Control.Text;
               With QueryListOfBill do
               Begin
                    Close;
                    sql.Clear;
                    sql.add(' Select Distinct CRBillNo as BillNo,CrModifyDate as BillDate,DocCode,RefDocCode,ModifyBy From BillDetail where BillType=''IP'' ');
                    sql.add(' and (PayType=''CASH'' or PayType=''CREDIT'') and InPatientId='+EditPsearch.Text);
                    sql.add(' and Trim(DisBillDate) is not null Order By CrModifyDate Desc,CRBillNo Desc');
                    Open;
               End;
          End;
     End
     ...
     Begin
          ...
          Begin
               ...
               { Referral Doc. }
               With Query_GetRefDoc do
               Begin
                    Close;
                    sql.Clear;
                    sql.add(' Select Distinct RefDocCode From BillDetail where BillNo='+#39+Edit_PreviousBillNo.Text+#39);
                    Open;

                    IF Query_GetRefDoc.RecordCount > 0 Then
                    DBLC_Doctor.KeyValue:=Query_GetRefDoc.FieldByName('RefDocCode').AsString
                    Else
                    DBLC_Doctor.KeyValue:=NULL;
               End;
