...
begin
     ...
     begin
          ...
          SQL.Clear;
          SQL.Add('Select PL.*,to_number(substr(sampleno,2,999))orderno from vw_patienttestlist PL');
          SQL.Add('Where  PL.TESTDATE = ' + #39 + gs_From + #39);

          if gi_compileValue=3 then
              SQL.Add('AND SAMPLENO BETWEEN ' + quotedstr(gs_fromDailyNo) + ' AND ' + quotedstr(gs_ToDailyNo))
          else
              SQL.Add('AND SUBSTR(SAMPLENO,2) BETWEEN ' + copy(gs_fromDailyNo, 11, 10) + ' AND ' + copy(gs_ToDailyNo, 11, 10));

          if gi_TestNameId > 0 then
               SQL.Add(' And PL.TestNameID=' + IntToStr(gi_TestNameId));
          sql.Add('and depid in ('+gs_UserDepId+')');
          SQL.Add('Order by sampleno,patientid,OrderNo,deporder,testnamedisplayorder,testdisplayorder');
