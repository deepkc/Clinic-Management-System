object Form_DoctorWiseChargeSetup: TForm_DoctorWiseChargeSetup
  Left = 0
  Top = 0
  Caption = 'Doctor Wise Charge Setup'
  ClientHeight = 499
  ClientWidth = 974
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnl_DepWiseCharge: TPanel
    Left = 0
    Top = 0
    Width = 974
    Height = 499
    Align = alClient
    TabOrder = 0
    object Label_Department: TLabel
      Left = 114
      Top = 48
      Width = 39
      Height = 13
      Alignment = taRightJustify
      Caption = 'Doctor :'
    end
    object Label_Vatamt: TLabel
      Left = 410
      Top = 189
      Width = 38
      Height = 13
      Caption = 'Vat Amt'
      Color = clMedGray
      ParentColor = False
    end
    object Label_TotalAmt: TLabel
      Left = 474
      Top = 189
      Width = 46
      Height = 13
      Caption = 'Total Amt'
      Color = clMedGray
      ParentColor = False
    end
    object Label_NewRegCharge: TLabel
      Left = 61
      Top = 207
      Width = 92
      Height = 13
      Caption = 'New Reg. Charge :'
    end
    object Label21: TLabel
      Left = 327
      Top = 207
      Width = 70
      Height = 13
      Caption = '<-(%) Vat Rs ->'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label23: TLabel
      Left = 327
      Top = 230
      Width = 70
      Height = 13
      Caption = '<-(%) Vat Rs ->'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label_FollowupCharge: TLabel
      Left = 66
      Top = 230
      Width = 87
      Height = 13
      Caption = 'Followup Charge :'
    end
    object Label_RegChargeOPD: TLabel
      Left = 23
      Top = 298
      Width = 129
      Height = 13
      Caption = 'Reg.  (OPD Card) Charge :'
    end
    object Label27: TLabel
      Left = 327
      Top = 298
      Width = 70
      Height = 13
      Caption = '<-(%) Vat Rs ->'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label29: TLabel
      Left = 328
      Top = 321
      Width = 70
      Height = 13
      Caption = '<-(%) Vat Rs ->'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label_InpatientRegCharge: TLabel
      Left = 38
      Top = 321
      Width = 115
      Height = 13
      Caption = 'Inpatient Reg. Charge :'
    end
    object Label_FollowupDays: TLabel
      Left = 45
      Top = 500
      Width = 107
      Height = 13
      Caption = 'FREE Follow up Days :'
    end
    object Label25: TLabel
      Left = 85
      Top = 72
      Width = 68
      Height = 13
      Alignment = taRightJustify
      Caption = 'Patient Type :'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label1: TLabel
      Left = 75
      Top = 95
      Width = 78
      Height = 13
      Alignment = taRightJustify
      Caption = 'Currency Type :'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label2: TLabel
      Left = 327
      Top = 253
      Width = 70
      Height = 13
      Caption = '<-(%) Vat Rs ->'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 1
      Top = 253
      Width = 152
      Height = 13
      Caption = 'Intermediate Followup Charge :'
    end
    object Label4: TLabel
      Left = 327
      Top = 276
      Width = 70
      Height = 13
      Caption = '<-(%) Vat Rs ->'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label5: TLabel
      Left = 23
      Top = 276
      Width = 129
      Height = 13
      Caption = 'Referral Followup Charge :'
    end
    object Dblcb_Doctor: TDBLookupComboBox
      Left = 157
      Top = 45
      Width = 284
      Height = 21
      KeyField = 'DOCID'
      ListField = 'DOCNAME'
      ListSource = DS_Doctor
      TabOrder = 1
      OnClick = Dblcb_DoctorClick
    end
    object Edit_NewRegCharge: TEdit
      Left = 157
      Top = 203
      Width = 121
      Height = 21
      TabOrder = 7
      OnExit = Edit_NewRegChargeExit
    end
    object Edit_NewRegTaxPer: TEdit
      Left = 289
      Top = 203
      Width = 29
      Height = 21
      CharCase = ecUpperCase
      ReadOnly = True
      TabOrder = 8
      Text = '0'
    end
    object Edit_NewVatAmt: TEdit
      Left = 400
      Top = 203
      Width = 67
      Height = 21
      TabStop = False
      Enabled = False
      ReadOnly = True
      TabOrder = 9
    end
    object Edit_NewTotalAmount: TEdit
      Left = 472
      Top = 203
      Width = 58
      Height = 21
      Enabled = False
      ReadOnly = True
      TabOrder = 10
    end
    object Edit_FollowupTotalAmount: TEdit
      Left = 472
      Top = 226
      Width = 58
      Height = 21
      Enabled = False
      ReadOnly = True
      TabOrder = 14
    end
    object Edit_FollowupVatAmt: TEdit
      Left = 400
      Top = 226
      Width = 67
      Height = 21
      TabStop = False
      Enabled = False
      ReadOnly = True
      TabOrder = 13
    end
    object Edit_FollowupTaxPer: TEdit
      Left = 289
      Top = 226
      Width = 29
      Height = 21
      CharCase = ecUpperCase
      ReadOnly = True
      TabOrder = 12
      Text = '0'
    end
    object Edit_FollowUpCharge: TEdit
      Left = 157
      Top = 226
      Width = 121
      Height = 21
      TabOrder = 11
      OnExit = Edit_FollowUpChargeExit
    end
    object Edit_RegOPDCardCharge: TEdit
      Left = 157
      Top = 294
      Width = 121
      Height = 21
      TabOrder = 15
      OnExit = Edit_RegOPDCardChargeExit
    end
    object Edit_RegOPDCardTaxPer: TEdit
      Left = 289
      Top = 294
      Width = 29
      Height = 21
      CharCase = ecUpperCase
      ReadOnly = True
      TabOrder = 16
      Text = '0'
    end
    object Edit_RegOPDCardVatAmt: TEdit
      Left = 400
      Top = 294
      Width = 67
      Height = 21
      TabStop = False
      Enabled = False
      ReadOnly = True
      TabOrder = 17
    end
    object Edit_RegOPDCardTotalAmount: TEdit
      Left = 472
      Top = 294
      Width = 58
      Height = 21
      Enabled = False
      ReadOnly = True
      TabOrder = 18
    end
    object Edit_InpatientRegChargeTotalAmount: TEdit
      Left = 472
      Top = 317
      Width = 58
      Height = 21
      Enabled = False
      ReadOnly = True
      TabOrder = 22
    end
    object Edit_InpatientRegVatAmt: TEdit
      Left = 400
      Top = 317
      Width = 67
      Height = 21
      TabStop = False
      Enabled = False
      ReadOnly = True
      TabOrder = 21
    end
    object Edit_InpatientTaxPer: TEdit
      Left = 289
      Top = 317
      Width = 29
      Height = 21
      CharCase = ecUpperCase
      ReadOnly = True
      TabOrder = 20
      Text = '0'
    end
    object Edit_InpatientRegCharge: TEdit
      Left = 157
      Top = 317
      Width = 121
      Height = 21
      TabOrder = 19
      OnExit = Edit_InpatientRegChargeExit
    end
    object Panel2: TPanel
      Left = 1
      Top = 1
      Width = 972
      Height = 39
      Align = alTop
      TabOrder = 0
      object Bb_Save: TBitBtn
        Left = 783
        Top = 1
        Width = 94
        Height = 37
        Margins.Right = 0
        Align = alRight
        Caption = '&Save'
        DoubleBuffered = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentDoubleBuffered = False
        TabOrder = 0
        OnClick = Bb_SaveClick
      end
      object BB_Close: TBitBtn
        Left = 877
        Top = 1
        Width = 94
        Height = 37
        Margins.Left = 0
        Margins.Top = 6
        Margins.Right = 0
        Margins.Bottom = 6
        Align = alRight
        Caption = '&Close'
        DoubleBuffered = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFEDEDF77C7DC96162C58383CB}
        ParentDoubleBuffered = False
        TabOrder = 1
        OnClick = BB_CloseClick
      end
    end
    object ComboBox_Currency: TComboBox
      Left = 157
      Top = 91
      Width = 121
      Height = 21
      Style = csDropDownList
      TabOrder = 3
      Items.Strings = (
        'NRS'
        'USD')
    end
    object Edit_FollowupDays: TEdit
      Left = 157
      Top = 499
      Width = 122
      Height = 21
      TabOrder = 24
    end
    object CB_IncludingServiceTaxCharges: TCheckBox
      Left = 157
      Top = 179
      Width = 165
      Height = 17
      Caption = 'Including Service Tax Charges'
      TabOrder = 5
    end
    object DBLCB_PatientType: TDBLookupComboBox
      Left = 157
      Top = 68
      Width = 121
      Height = 21
      KeyField = 'PATY_PATIENTTYPECODE'
      ListField = 'PATY_PATIENTTYPE'
      ListFieldIndex = 1
      ListSource = DS_PatientType
      TabOrder = 2
    end
    object DBGridDept: TDBGrid
      Left = 5
      Top = 339
      Width = 967
      Height = 159
      DataSource = DS_DocWiseCharge
      TabOrder = 23
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      OnDrawColumnCell = DBGridDeptDrawColumnCell
      OnDblClick = DBGridDeptDblClick
      Columns = <
        item
          Expanded = False
          FieldName = 'DOCCODE'
          Title.Alignment = taCenter
          Title.Caption = 'Doc. Code'
          Width = 65
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DOCNAME'
          Title.Alignment = taCenter
          Title.Caption = 'Doc. Name'
          Width = 178
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DEPNAME'
          Title.Alignment = taCenter
          Title.Caption = 'Dep. Name'
          Width = 159
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PATIENTTYPE'
          Title.Alignment = taCenter
          Title.Caption = 'Patient Type'
          Width = 74
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FREEFOLLOWUPDAYS'
          Title.Alignment = taCenter
          Title.Caption = 'Free Followup Days'
          Width = 113
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NEWVISITCHARGE'
          Title.Alignment = taCenter
          Title.Caption = 'New Visit Charge'
          Width = 87
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NEWVISITTAX'
          Title.Alignment = taCenter
          Title.Caption = 'Svr. Tax'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NEWVISITCHARGETOTAL'
          Title.Alignment = taCenter
          Title.Caption = 'New Visit Charge Tot.'
          Width = 120
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FOLLOWUPCHARGE'
          Title.Alignment = taCenter
          Title.Caption = 'Followup Charge'
          Width = 86
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FOLLOWUPTAX'
          Title.Alignment = taCenter
          Title.Caption = 'Svr. Tax'
          Width = 80
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FOLLOWUPCHARGETOTAL'
          Title.Alignment = taCenter
          Title.Caption = 'Follow Up Total'
          Width = 86
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REGOPDCARDCHARGE'
          Title.Alignment = taCenter
          Title.Caption = 'OPD Card Charge'
          Width = 93
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REGOPDCARDTAX'
          Title.Alignment = taCenter
          Title.Caption = 'Svr. Tax'
          Width = 65
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REGOPDCARDCHARGETOTAL'
          Title.Alignment = taCenter
          Title.Caption = 'OPD Card Total'
          Width = 107
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ADMISSIONCHARGE'
          Title.Alignment = taCenter
          Title.Caption = 'Inpt. Reg Charge'
          Width = 95
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ADMISSIONTAX'
          Title.Alignment = taCenter
          Title.Caption = 'Svr. Tax'
          Width = 57
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ADMISSIONCHARGETOTAL'
          Title.Alignment = taCenter
          Title.Caption = 'Inpatient Reg. Total'
          Width = 108
          Visible = True
        end
        item
          Expanded = False
          Visible = True
        end
        item
          Expanded = False
          Visible = True
        end
        item
          Expanded = False
          Visible = True
        end
        item
          Expanded = False
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CURRENCYTYPE'
          Title.Alignment = taCenter
          Title.Caption = 'Currency Type'
          Width = 87
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ISCHARGEWITHTAX'
          Title.Alignment = taCenter
          Title.Caption = 'Is Charge With Tax'
          Width = 106
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'USERNAME'
          Title.Alignment = taCenter
          Title.Caption = 'User Name'
          Width = 88
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DATAPOSTDATE'
          Title.Alignment = taCenter
          Title.Caption = 'Modify Date'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DATAPOSTTIME'
          Title.Alignment = taCenter
          Title.Caption = 'Modify Time'
          Width = 73
          Visible = True
        end>
    end
    object CB_IsChargeRoundOffGEN: TCheckBox
      Left = 157
      Top = 161
      Width = 202
      Height = 17
      Caption = 'Is Charge Round Off (Normal)'
      Color = clBtnShadow
      ParentColor = False
      TabOrder = 6
      Visible = False
    end
    object le_FreeFollowupDays: TLabeledEdit
      Left = 157
      Top = 113
      Width = 121
      Height = 21
      EditLabel.Width = 101
      EditLabel.Height = 13
      EditLabel.Caption = 'Free Followup Days :'
      LabelPosition = lpLeft
      NumbersOnly = True
      TabOrder = 4
      Text = '0'
      OnExit = le_FreeFollowupDaysExit
    end
    object CB_IsChargeActive: TCheckBox
      Left = 328
      Top = 72
      Width = 113
      Height = 17
      Caption = 'Is Charge Active'
      Checked = True
      State = cbChecked
      TabOrder = 25
    end
    object LE_IntermediateFollowupDays: TLabeledEdit
      Left = 157
      Top = 136
      Width = 121
      Height = 21
      Color = clMenu
      EditLabel.Width = 141
      EditLabel.Height = 13
      EditLabel.Caption = 'Intermediate Followup Days :'
      LabelPosition = lpLeft
      NumbersOnly = True
      TabOrder = 26
      Text = '0'
      OnExit = le_FreeFollowupDaysExit
    end
    object Edit_InterFollowupTotalAmount: TEdit
      Left = 472
      Top = 249
      Width = 58
      Height = 21
      Enabled = False
      ReadOnly = True
      TabOrder = 27
    end
    object Edit_InterFollowupVatAmt: TEdit
      Left = 400
      Top = 249
      Width = 67
      Height = 21
      TabStop = False
      Enabled = False
      ReadOnly = True
      TabOrder = 28
    end
    object Edit_InterFollowupTaxPer: TEdit
      Left = 289
      Top = 249
      Width = 29
      Height = 21
      CharCase = ecUpperCase
      ReadOnly = True
      TabOrder = 29
      Text = '0'
    end
    object Edit_InterFollowUpCharge: TEdit
      Left = 157
      Top = 249
      Width = 121
      Height = 21
      TabOrder = 30
      OnExit = Edit_FollowUpChargeExit
    end
    object Edit_DocRefFollowupTotAmt: TEdit
      Left = 472
      Top = 272
      Width = 58
      Height = 21
      Enabled = False
      ReadOnly = True
      TabOrder = 31
    end
    object Edit_DocRefFollowupVatAmt: TEdit
      Left = 400
      Top = 272
      Width = 67
      Height = 21
      TabStop = False
      Enabled = False
      ReadOnly = True
      TabOrder = 32
    end
    object Edit_DocRefFollowupTaxPer: TEdit
      Left = 289
      Top = 272
      Width = 29
      Height = 21
      CharCase = ecUpperCase
      ReadOnly = True
      TabOrder = 33
      Text = '0'
    end
    object Edit_DocReferalCharge: TEdit
      Left = 157
      Top = 272
      Width = 121
      Height = 21
      TabOrder = 34
      OnExit = Edit_DocReferalChargeExit
    end
  end
  object DS_Doctor: TDataSource
    DataSet = Query_Doctor
    Left = 760
    Top = 56
  end
  object Query_Doctor: TOraQuery
    SQL.Strings = (
      
        'Select DOCT_DOCID DOCID,DOCT_DOCCODE DOCCODE,DOCT_DESIG DESIG,DO' +
        'CT_DOCNAME DOCNAME,DOCT_ISACTIVE IsActive,'
      
        'DOCT_DEPID DEPID,DOCT_DEPCODE DEPCODE,(Select DEPT_DEPNAME From ' +
        'HS_DEPT_DEPARTMENT where DEPT_DEPCODE=DOCT_DEPCODE) as DEPNAME '
      'From HS_DOCT_DOCTOR '
      'Order By DOCT_DOCNAME')
    Left = 720
    Top = 32
  end
  object DS_DocWiseCharge: TDataSource
    DataSet = qry_DocWiseCharge
    Left = 824
    Top = 96
  end
  object Query_PatientType: TOraQuery
    SQL.Strings = (
      'Select * From HS_PATY_PatientType'
      'Order by PATY_DisplayOrder')
    Left = 552
    Top = 16
  end
  object Query_save: TOraQuery
    Left = 704
    Top = 176
  end
  object qry_DocWiseCharge: TOraQuery
    SQL.Strings = (
      
        'Select * From VW_HS_SETUP_DOCWISECHARGE Order by DOCNAME,PATIENT' +
        'TYPECODE')
    Left = 784
    Top = 168
  end
  object Query_Process: TOraQuery
    SQL.Strings = (
      '')
    Left = 632
    Top = 96
  end
  object DS_PatientType: TDataSource
    DataSet = Query_PatientType
    Left = 584
    Top = 16
  end
end
