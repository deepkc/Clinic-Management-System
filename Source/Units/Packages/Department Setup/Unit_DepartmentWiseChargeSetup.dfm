object Form_DepartmentWiseChargeSetup: TForm_DepartmentWiseChargeSetup
  Left = 0
  Top = 0
  Caption = 'Department Wise Charge Setup'
  ClientHeight = 482
  ClientWidth = 974
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnl_DepWiseCharge: TPanel
    Left = 0
    Top = 0
    Width = 974
    Height = 482
    Align = alClient
    TabOrder = 0
    object Label_Department: TLabel
      Left = 89
      Top = 48
      Width = 64
      Height = 13
      Caption = 'Department :'
    end
    object Label_Vatamt: TLabel
      Left = 410
      Top = 181
      Width = 38
      Height = 13
      Caption = 'Vat Amt'
      Color = clMedGray
      ParentColor = False
    end
    object Label_TotalAmt: TLabel
      Left = 474
      Top = 181
      Width = 46
      Height = 13
      Caption = 'Total Amt'
      Color = clMedGray
      ParentColor = False
    end
    object Label_NewRegCharge: TLabel
      Left = 61
      Top = 203
      Width = 92
      Height = 13
      Caption = 'New Reg. Charge :'
    end
    object Label21: TLabel
      Left = 327
      Top = 203
      Width = 70
      Height = 13
      Caption = '<-(%) Vat Rs ->'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label23: TLabel
      Left = 327
      Top = 226
      Width = 70
      Height = 13
      Caption = '<-(%) Vat Rs ->'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label_FollowupCharge: TLabel
      Left = 66
      Top = 226
      Width = 87
      Height = 13
      Caption = 'Followup Charge :'
    end
    object Label_RegChargeOPD: TLabel
      Left = 26
      Top = 279
      Width = 129
      Height = 13
      Caption = 'Reg.  (OPD Card) Charge :'
    end
    object Label27: TLabel
      Left = 327
      Top = 279
      Width = 70
      Height = 13
      Caption = '<-(%) Vat Rs ->'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label29: TLabel
      Left = 328
      Top = 302
      Width = 70
      Height = 13
      Caption = '<-(%) Vat Rs ->'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label_InpatientRegCharge: TLabel
      Left = 38
      Top = 302
      Width = 115
      Height = 13
      Caption = 'Inpatient Reg. Charge :'
    end
    object Label_FollowupDays: TLabel
      Left = 45
      Top = 500
      Width = 107
      Height = 13
      Caption = 'FREE Follow up Days :'
    end
    object Label25: TLabel
      Left = 85
      Top = 72
      Width = 68
      Height = 13
      Alignment = taRightJustify
      Caption = 'Patient Type :'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label1: TLabel
      Left = 75
      Top = 95
      Width = 78
      Height = 13
      Alignment = taRightJustify
      Caption = 'Currency Type :'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label2: TLabel
      Left = 327
      Top = 249
      Width = 70
      Height = 13
      Caption = '<-(%) Vat Rs ->'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 1
      Top = 249
      Width = 125
      Height = 13
      Caption = 'Referal Followup Charge :'
    end
    object Dblcb_Department: TDBLookupComboBox
      Left = 156
      Top = 45
      Width = 284
      Height = 21
      KeyField = 'DEPT_DEPID'
      ListField = 'DEPT_DEPNAME'
      ListSource = DS_Department
      TabOrder = 1
      OnClick = Dblcb_DepartmentClick
    end
    object Edit_NewRegCharge: TEdit
      Left = 159
      Top = 199
      Width = 121
      Height = 21
      TabOrder = 8
      OnExit = Edit_NewRegChargeExit
    end
    object Edit_NewRegTaxPer: TEdit
      Left = 289
      Top = 199
      Width = 29
      Height = 21
      CharCase = ecUpperCase
      ReadOnly = True
      TabOrder = 9
      Text = '0'
    end
    object Edit_NewVatAmt: TEdit
      Left = 400
      Top = 199
      Width = 67
      Height = 21
      TabStop = False
      Enabled = False
      ReadOnly = True
      TabOrder = 10
    end
    object Edit_NewTotalAmount: TEdit
      Left = 472
      Top = 199
      Width = 58
      Height = 21
      Enabled = False
      ReadOnly = True
      TabOrder = 11
    end
    object Edit_FollowupTotalAmount: TEdit
      Left = 472
      Top = 222
      Width = 58
      Height = 21
      Enabled = False
      ReadOnly = True
      TabOrder = 15
    end
    object Edit_FollowupVatAmt: TEdit
      Left = 400
      Top = 222
      Width = 67
      Height = 21
      TabStop = False
      Enabled = False
      ReadOnly = True
      TabOrder = 14
    end
    object Edit_FollowupTaxPer: TEdit
      Left = 289
      Top = 222
      Width = 29
      Height = 21
      CharCase = ecUpperCase
      ReadOnly = True
      TabOrder = 13
      Text = '0'
    end
    object Edit_FollowUpCharge: TEdit
      Left = 157
      Top = 222
      Width = 121
      Height = 21
      TabOrder = 12
      OnExit = Edit_FollowUpChargeExit
    end
    object Edit_RegOPDCardCharge: TEdit
      Left = 157
      Top = 275
      Width = 121
      Height = 21
      TabOrder = 20
      OnExit = Edit_RegOPDCardChargeExit
    end
    object Edit_RegOPDCardTaxPer: TEdit
      Left = 289
      Top = 275
      Width = 29
      Height = 21
      CharCase = ecUpperCase
      ReadOnly = True
      TabOrder = 21
      Text = '0'
    end
    object Edit_RegOPDCardVatAmt: TEdit
      Left = 400
      Top = 275
      Width = 67
      Height = 21
      TabStop = False
      Enabled = False
      ReadOnly = True
      TabOrder = 22
    end
    object Edit_RegOPDCardTotalAmount: TEdit
      Left = 472
      Top = 275
      Width = 58
      Height = 21
      Enabled = False
      ReadOnly = True
      TabOrder = 23
    end
    object Edit_InpatientRegChargeTotalAmount: TEdit
      Left = 472
      Top = 298
      Width = 58
      Height = 21
      Enabled = False
      ReadOnly = True
      TabOrder = 27
    end
    object Edit_InpatientRegVatAmt: TEdit
      Left = 400
      Top = 298
      Width = 67
      Height = 21
      TabStop = False
      Enabled = False
      ReadOnly = True
      TabOrder = 26
    end
    object Edit_InpatientTaxPer: TEdit
      Left = 289
      Top = 298
      Width = 29
      Height = 21
      CharCase = ecUpperCase
      ReadOnly = True
      TabOrder = 25
      Text = '0'
    end
    object Edit_InpatientRegCharge: TEdit
      Left = 157
      Top = 298
      Width = 121
      Height = 21
      TabOrder = 24
      OnExit = Edit_InpatientRegChargeExit
    end
    object Panel2: TPanel
      Left = 1
      Top = 1
      Width = 972
      Height = 39
      Align = alTop
      TabOrder = 0
      object Bb_Save: TBitBtn
        Left = 783
        Top = 1
        Width = 94
        Height = 37
        Margins.Right = 0
        Align = alRight
        Caption = '&Save (F12)'
        DoubleBuffered = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentDoubleBuffered = False
        TabOrder = 0
        OnClick = Bb_SaveClick
      end
      object BB_Close: TBitBtn
        Left = 877
        Top = 1
        Width = 94
        Height = 37
        Margins.Left = 0
        Margins.Top = 6
        Margins.Right = 0
        Margins.Bottom = 6
        Align = alRight
        Caption = '&Close (Esc.)'
        DoubleBuffered = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFEDEDF77C7DC96162C58383CB}
        ParentDoubleBuffered = False
        TabOrder = 1
        OnClick = BB_CloseClick
      end
    end
    object ComboBox_Currency: TComboBox
      Left = 157
      Top = 91
      Width = 121
      Height = 21
      Style = csDropDownList
      TabOrder = 3
      Items.Strings = (
        'NRS'
        'USD')
    end
    object Edit_FollowupDays: TEdit
      Left = 157
      Top = 499
      Width = 122
      Height = 21
      TabOrder = 29
    end
    object CB_IncludingServiceTaxCharges: TCheckBox
      Left = 157
      Top = 178
      Width = 169
      Height = 17
      Caption = 'Including Service Tax Charges'
      TabOrder = 7
    end
    object DBLCB_PatientType: TDBLookupComboBox
      Left = 157
      Top = 68
      Width = 121
      Height = 21
      KeyField = 'PATY_PATIENTTYPECODE'
      ListField = 'PATY_PATIENTTYPE'
      ListFieldIndex = 1
      ListSource = DS_PatientType
      TabOrder = 2
    end
    object DBGridDept: TDBGrid
      Left = 1
      Top = 322
      Width = 967
      Height = 152
      DataSource = DS_DepWiseCharge
      TabOrder = 28
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      OnDblClick = DBGridDeptDblClick
      Columns = <
        item
          Expanded = False
          FieldName = 'DEPCODE'
          Title.Alignment = taCenter
          Title.Caption = 'Dep.Code'
          Width = 65
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DEPNAME'
          Title.Alignment = taCenter
          Title.Caption = 'Dep. Name'
          Width = 159
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PATIENTTYPE'
          Title.Alignment = taCenter
          Title.Caption = 'Patient Type'
          Width = 74
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FREEFOLLOWUPDAYS'
          Title.Alignment = taCenter
          Title.Caption = 'Free Followup Days'
          Width = 113
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NEWVISITCHARGE'
          Title.Alignment = taCenter
          Title.Caption = 'New Visit Charge'
          Width = 87
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NEWVISITTAX'
          Title.Alignment = taCenter
          Title.Caption = 'Svr. Tax'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NEWVISITCHARGETOTAL'
          Title.Alignment = taCenter
          Title.Caption = 'New Visit Charge Tot.'
          Width = 120
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FOLLOWUPCHARGE'
          Title.Alignment = taCenter
          Title.Caption = 'Followup Charge'
          Width = 86
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FOLLOWUPTAX'
          Title.Alignment = taCenter
          Title.Caption = 'Svr. Tax'
          Width = 80
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FOLLOWUPCHARGETOTAL'
          Title.Alignment = taCenter
          Title.Caption = 'Follow Up Total'
          Width = 86
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REGOPDCARDCHARGE'
          Title.Alignment = taCenter
          Title.Caption = 'OPD Card Charge'
          Width = 93
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REGOPDCARDTAX'
          Title.Alignment = taCenter
          Title.Caption = 'Svr. Tax'
          Width = 65
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REGOPDCARDCHARGETOTAL'
          Title.Alignment = taCenter
          Title.Caption = 'OPD Card Total'
          Width = 107
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ADMISSIONCHARGE'
          Title.Alignment = taCenter
          Title.Caption = 'Inpt. Reg Charge'
          Width = 95
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ADMISSIONTAX'
          Title.Alignment = taCenter
          Title.Caption = 'Svr. Tax'
          Width = 57
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ADMISSIONCHARGETOTAL'
          Title.Alignment = taCenter
          Title.Caption = 'Inpatient Reg. Total'
          Width = 108
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'INTERFOLLOWUPDAYS'
          Title.Alignment = taCenter
          Title.Caption = 'Inter. Followup Days'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'INTERFOLLOWUPCHARGE'
          Title.Alignment = taCenter
          Title.Caption = 'Referal Followup Charge'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'INTERFOLLOWUPTAX'
          Title.Alignment = taCenter
          Title.Caption = 'Referal Followup Tax'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'INTERFOLLOWUPCHARGETOTAL'
          Title.Caption = 'Referal Followup Total'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CURRENCYTYPE'
          Title.Alignment = taCenter
          Title.Caption = 'Currency Type'
          Width = 87
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ISCHARGEWITHTAX'
          Title.Alignment = taCenter
          Title.Caption = 'Is Charge With Tax'
          Width = 106
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'USERNAME'
          Title.Alignment = taCenter
          Title.Caption = 'User Name'
          Width = 88
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DATAPOSTDATE'
          Title.Alignment = taCenter
          Title.Caption = 'Modify Date'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DATAPOSTTIME'
          Title.Alignment = taCenter
          Title.Caption = 'Modify Time'
          Width = 73
          Visible = True
        end>
    end
    object CB_IsChargeRoundOffGEN: TCheckBox
      Left = 157
      Top = 159
      Width = 184
      Height = 17
      Caption = 'Is Charge Round Off (Normal)'
      Color = clBtnShadow
      ParentColor = False
      TabOrder = 6
      Visible = False
    end
    object le_FreeFollowupDays: TLabeledEdit
      Left = 157
      Top = 113
      Width = 121
      Height = 21
      EditLabel.Width = 101
      EditLabel.Height = 13
      EditLabel.Caption = 'Free Followup Days :'
      LabelPosition = lpLeft
      NumbersOnly = True
      TabOrder = 4
      Text = '0'
      OnExit = le_FreeFollowupDaysExit
    end
    object LE_IntermediateFollowupDays: TLabeledEdit
      Left = 157
      Top = 136
      Width = 121
      Height = 21
      Color = clMenu
      EditLabel.Width = 141
      EditLabel.Height = 13
      EditLabel.Caption = 'Intermediate Followup Days :'
      LabelPosition = lpLeft
      NumbersOnly = True
      TabOrder = 5
      Text = '0'
      OnExit = le_FreeFollowupDaysExit
    end
    object Edit_InterFollowupTotalAmount: TEdit
      Left = 472
      Top = 245
      Width = 58
      Height = 21
      Enabled = False
      ReadOnly = True
      TabOrder = 19
    end
    object Edit_InterFollowupVatAmt: TEdit
      Left = 400
      Top = 245
      Width = 67
      Height = 21
      TabStop = False
      Enabled = False
      ReadOnly = True
      TabOrder = 18
    end
    object Edit_InterFollowupTaxPer: TEdit
      Left = 289
      Top = 245
      Width = 29
      Height = 21
      CharCase = ecUpperCase
      ReadOnly = True
      TabOrder = 17
      Text = '0'
    end
    object Edit_InterFollowUpCharge: TEdit
      Left = 157
      Top = 245
      Width = 121
      Height = 21
      TabOrder = 16
      OnExit = Edit_FollowUpChargeExit
    end
  end
  object DS_Department: TDataSource
    DataSet = Query_Department
    Left = 512
    Top = 104
  end
  object Query_Department: TOraQuery
    SQL.Strings = (
      'Select Dept_DepId,DEPT_DepCode,Dept_DepName,Dept_DepOrder'
      'From HS_Dept_DEPARTMENT'
      'where Dept_DepType='#39'C'#39
      'Order by Dept_DepName ')
    Left = 480
    Top = 96
  end
  object DS_DepWiseCharge: TDataSource
    DataSet = qry_DepWiseCharge
    Left = 704
    Top = 8
  end
  object Query_PatientType: TOraQuery
    SQL.Strings = (
      'Select * From HS_PATY_PatientType'
      'Order by PATY_DisplayOrder')
    Left = 552
    Top = 16
  end
  object Query_save: TOraQuery
    Left = 696
    Top = 104
  end
  object qry_DepWiseCharge: TOraQuery
    SQL.Strings = (
      
        'Select * From VW_HS_SETUP_DEPTWISECHARGE Order by DepName,PATIEN' +
        'TTYPECODE')
    Left = 672
    Top = 8
  end
  object Query_Process: TOraQuery
    SQL.Strings = (
      '')
    Left = 632
    Top = 96
  end
  object DS_PatientType: TDataSource
    DataSet = Query_PatientType
    Left = 584
    Top = 16
  end
end
