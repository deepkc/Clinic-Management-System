unit Unit_DepartmentWiseChargeSetup;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, fxn,   DM,
  Dialogs, ExtCtrls, Grids, DBGrids, StdCtrls, DBCtrls, Buttons, DB, DBTables, DBAccess, Ora, OraSmart, MemDS, OraError, Unit_Master_Hos;

type
  TForm_DepartmentWiseChargeSetup = class(TForm)
  ...
  public
            pb_IsNew: boolean;
            pf_NEWREGCHARGE,pf_FOLLOWUPCHARGE,pf_REFERFOLLOWUPCHARGE,pf_RegOPDCardCharge,pf_ADMISSONCHARGE : Double;
            pf_NEWREGTAX,pf_FOLLOWUPTAX, pf_REFERFOLLOWUPTAX, pf_RegOPDCardTAX,pf_ADMISSONTAX : Double;

            pi_DEPWISECHARGEID, pi_DepID : Integer;

            procedure GetDepartmentWiseCharge;

    { Public declarations }
  end;

var
  Form_DepartmentWiseChargeSetup: TForm_DepartmentWiseChargeSetup;
...
procedure TForm_DepartmentWiseChargeSetup.BB_CloseClick(Sender: TObject);
begin
     Close;
end;

procedure TForm_DepartmentWiseChargeSetup.Bb_SaveClick(Sender: TObject);
begin
     if (Dblcb_Department.Text = '') then
     begin
          MsgBox(1006, 0, '', '', '');
          exit;
     end;
     ...
     IF StrToInt(LE_IntermediateFollowupDays.Text) > 400 Then
     begin
          messagedlg('Intermediate Followup Days Cross 400 Days.',mtWarning,[mbok],0);
          LE_IntermediateFollowupDays.SetFocus;
          Exit;
     end;
     ...
     Begin
          IF IsCheckDuplicateDepWiseCharge(Dblcb_Department.KeyValue,DBLCB_PatientType.KeyValue)=True Then
          begin
               messagedlg('Already Exist Department Wise Charge of this patient type.',mtWarning,[mbok],0);
               DBLCB_PatientType.SetFocus;
               Exit;
          end;
     End;
...
procedure TForm_DepartmentWiseChargeSetup.Bb_CancelClick(Sender: TObject);
begin
     close;
end;

procedure TForm_DepartmentWiseChargeSetup.Calculate_Tax_And_Amount;
begin
     ...
     Begin
          ...
          Begin
               ...
               pf_REFERFOLLOWUPCHARGE:=GetNoOfDecimalPartOfFloatNum(StrToFloat(Edit_InterFollowupCharge.Text)-StrToFloat(Edit_InterFollowupVatAmt.Text),3);
               IF CB_IncludingServiceTaxCharges.Checked=False Then
          ...
          Begin
               ...
               pf_REFERFOLLOWUPCHARGE:=StrToFloat(Edit_InterFollowupCharge.Text)-StrToFloat(Edit_InterFollowupVatAmt.Text);
               IF CB_IncludingServiceTaxCharges.Checked=False Then
               Edit_InterFollowupTotalAmount.Text:=FloatToStr(StrToFloat(Edit_InterFollowupCharge.Text)+StrToFloat(Edit_InterFollowupVatAmt.Text))
               Else
               Edit_InterFollowupTotalAmount.Text:=Edit_InterFollowupCharge.Text;
     ...
     Begin
          ...
          Begin
               ...
               pf_REFERFOLLOWUPCHARGE:=StrToFloat(Edit_InterFollowupCharge.Text);
               IF CB_IncludingServiceTaxCharges.Checked=False Then
               Edit_InterFollowupTotalAmount.Text:=FloatToStr(StrToFloat(Edit_InterFollowupCharge.Text)+StrToFloat(Edit_InterFollowupVatAmt.Text))
               Else
               Edit_InterFollowupTotalAmount.Text:=Edit_InterFollowupCharge.Text;
...
procedure TForm_DepartmentWiseChargeSetup.DBGridDeptDblClick(Sender: TObject);
begin
     IF qry_DepWiseCharge.FieldByName('DEPWISECHARGEID').AsInteger<=0 Then
     begin
          MessageDlg('No Record to Modify.',mtWarning,[mbOK],0);
          Exit;
     end;

     pi_DEPWISECHARGEID:=qry_DepWiseCharge.FieldByName('DEPWISECHARGEID').AsInteger;

     pb_IsNew:=False;
     Dblcb_Department.Enabled:=True;
     Dblcb_Department.KeyValue:=qry_DepWiseCharge.FieldByName('DepID').AsInteger;
     Dblcb_Department.Color:=clMenu;
     Dblcb_Department.Enabled:=False;



     DBLCB_PatientType.Enabled:=True;
     DBLCB_PatientType.KeyValue:=qry_DepWiseCharge.FieldByName('PatientTypeCode').AsString;
     DBLCB_PatientType.Color:=clMenu;
     //DBLCB_PatientType.Enabled:=False;


     if qry_DepWiseCharge.FieldByName('CURRENCYTYPE').AsString='NRS' then
     ComboBox_Currency.ItemIndex:=0
     else
     ComboBox_Currency.ItemIndex:=1;
     //ComboBox_Currency.Text:=qry_DepWiseCharge.FieldByName('CURRENCYTYPE').AsString;
     ComboBox_Currency.SetFocus;

     le_FreeFollowupDays.Text:=IntToStr(qry_DepWiseCharge.FieldByName('FREEFOLLOWUPDAYS').AsInteger);
     LE_IntermediateFollowupDays.Text:=IntToStr(qry_DepWiseCharge.FieldByName('INTERFOLLOWUPDAYS').AsInteger);

     if qry_DepWiseCharge.FieldByName('ISCHARGEWITHTAX').AsString='Y' then
     ...
     Begin
          ...
          Edit_NewRegCharge.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('NewVisitCharge').ASFloat + qry_DepWiseCharge.FieldByName('NewVisitTax').AsFloat);
          Edit_FollowUpCharge.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('FOLLOWUPCHARGE').ASFloat + qry_DepWiseCharge.FieldByName('FOLLOWUPTAX').AsFloat );
          Edit_INTERFollowUpCharge.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('REFERFOLLOWUPCHARGE').ASFloat + qry_DepWiseCharge.FieldByName('REFERFOLLOWUPTAX').AsFloat );
          Edit_RegOPDCardCharge.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('REGOPDCARDCHARGE').ASFloat + qry_DepWiseCharge.FieldByName('REGOPDCARDTAX').AsFloat);
          Edit_InpatientRegCharge.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('ADMISSIONCHARGE').ASFloat + qry_DepWiseCharge.FieldByName('ADMISSIONTAX').AsFloat);
     ...
     Begin
          ...
          Edit_NewRegCharge.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('NewVisitCharge').ASFloat);
          Edit_FollowUpCharge.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('FOLLOWUPCHARGE').ASFloat);
          Edit_INTERFollowUpCharge.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('REFERFOLLOWUPCHARGE').ASFloat);
          Edit_RegOPDCardCharge.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('REGOPDCARDCHARGE').ASFloat);
          Edit_InpatientRegCharge.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('ADMISSIONCHARGE').ASFloat);
     End;

     pf_NEWREGCHARGE:=qry_DepWiseCharge.FieldByName('NewVisitCharge').ASFloat;
     Edit_NewTotalAmount.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('NewVisitCharge').AsFloat+qry_DepWiseCharge.FieldByName('NewVisitTax').AsFloat);
     Edit_NewVatAmt.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('NewVisitTax').AsFloat);

     pf_FOLLOWUPCHARGE:=qry_DepWiseCharge.FieldByName('FOLLOWUPCHARGE').ASFloat;
     Edit_FollowupTotalAmount.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('FOLLOWUPCHARGE').AsFloat+qry_DepWiseCharge.FieldByName('FOLLOWUPTAX').AsFloat);
     Edit_FollowupVatAmt.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('FOLLOWUPTAX').AsFloat);


     pf_REFERFOLLOWUPCHARGE:=qry_DepWiseCharge.FieldByName('REFERFOLLOWUPCHARGE').ASFloat;
     Edit_INTERFollowupTotalAmount.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('REFERFOLLOWUPCHARGE').AsFloat+qry_DepWiseCharge.FieldByName('REFERFOLLOWUPTAX').AsFloat);
     Edit_INTERFollowupVatAmt.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('REFERFOLLOWUPTAX').AsFloat);

     pf_RegOPDCardCharge:=qry_DepWiseCharge.FieldByName('REGOPDCARDCHARGE').ASFloat;
     Edit_RegOPDCardTotalAmount.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('REGOPDCARDCHARGE').AsFloat+qry_DepWiseCharge.FieldByName('REGOPDCARDTAX').AsFloat);
     Edit_RegOPDCardVatAmt.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('REGOPDCARDTAX').AsFloat);

     pf_ADMISSONCHARGE:=qry_DepWiseCharge.FieldByName('ADMISSIONCHARGE').ASFloat;
     Edit_InpatientRegChargeTotalAmount.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('ADMISSIONCHARGE').AsFloat+qry_DepWiseCharge.FieldByName('ADMISSIONTAX').AsFloat);
     Edit_InpatientRegVatAmt.Text:=FloatToStr(qry_DepWiseCharge.FieldByName('ADMISSIONTAX').AsFloat);
end;

procedure TForm_DepartmentWiseChargeSetup.Dblcb_DepartmentClick(
  Sender: TObject);
begin
     if Dblcb_Department.Text<>'' then
     Begin
          pi_DepID:=Dblcb_Department.KeyValue;
          GetDepartmentWiseCharge;
     End;
end;

procedure TForm_DepartmentWiseChargeSetup.Edit_ChargeKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TForm_DepartmentWiseChargeSetup.Edit_FollowUpChargeExit(
  Sender: TObject);
begin
     Set_Zero_InBlank;
     Calculate_Tax_And_Amount;
...
procedure TForm_DepartmentWiseChargeSetup.Edit_InpatientRegChargeExit(
  Sender: TObject);
begin
     Set_Zero_InBlank;
     Calculate_Tax_And_Amount;
...
procedure TForm_DepartmentWiseChargeSetup.Edit_NewRegChargeExit(Sender: TObject);
begin
     Set_Zero_InBlank;
     Calculate_Tax_And_Amount;
end;

procedure TForm_DepartmentWiseChargeSetup.Edit_RegOPDCardChargeExit(Sender: TObject);
begin
     Set_Zero_InBlank;
     Calculate_Tax_And_Amount;
end;

procedure TForm_DepartmentWiseChargeSetup.FormShow(Sender: TObject);
begin
     NewEntryMode;
end;

procedure TForm_DepartmentWiseChargeSetup.SaveData;
var
     ls_PATIENTTYPECODE,ls_ISCHARGEWITHSVRTAX,ls_CURRENCYTYPE: string;
     li_DepId, li_FREEFOLLOWUPDAYS, li_INTERFOLLOWUPDAYS: INTEGER;
begin
     ...
     pf_REFERFOLLOWUPTAX:=StrToFloat(Edit_InterFollowupVatAmt.Text);
     pf_RegOPDCardTAX:=StrToFloat(Edit_RegOPDCardVatAmt.Text);
     pf_ADMISSONTAX:=StrToFloat(Edit_InpatientRegVatAmt.Text);


     if pb_IsNew then
     SaveDepartmentWiseCharge(ls_PATIENTTYPECODE,ls_CURRENCYTYPE,ls_ISCHARGEWITHSVRTAX,pf_NEWREGCHARGE, pf_NEWREGTAX,
          pf_FOLLOWUPCHARGE,pf_FOLLOWUPTAX, pf_REFERFOLLOWUPCHARGE,pf_REFERFOLLOWUPTAX,pf_RegOPDCardCharge,pf_RegOPDCardTAX,pf_ADMISSONCHARGE,pf_ADMISSONTAX,
          li_DepId, li_FREEFOLLOWUPDAYS, li_INTERFOLLOWUPDAYS)
     else
     UpdateDepartmentWiseCharge(ls_PATIENTTYPECODE,ls_CURRENCYTYPE,ls_ISCHARGEWITHSVRTAX,pf_NEWREGCHARGE, pf_NEWREGTAX,
                    pf_FOLLOWUPCHARGE,pf_FOLLOWUPTAX,pf_REFERFOLLOWUPCHARGE,pf_REFERFOLLOWUPTAX,pf_RegOPDCardCharge,pf_RegOPDCardTAX,pf_ADMISSONCHARGE,pf_ADMISSONTAX,
                    li_FREEFOLLOWUPDAYS,li_INTERFOLLOWUPDAYS, pi_DEPWISECHARGEID);
     ShowDoneMessage;
end;

procedure TForm_DepartmentWiseChargeSetup.Set_SvrTax_Percent;
begin
...
procedure TForm_DepartmentWiseChargeSetup.Set_Zero_InBlank;
begin
...
procedure TForm_DepartmentWiseChargeSetup.GetDepartmentWiseCharge;
begin
     With qry_DepWiseCharge do
     begin
          Close;
          Session := DM_Hospital.DB;
          SQL.Clear;
          SQL.Add(' Select * From VW_HS_SETUP_DEPTWISECHARGE');
          SQL.Add(' Where DEPID='+IntToStr(pi_DepID));
          SQL.Add(' Order by DepName,PATIENTTYPECODE');
          Open;
     end;
end;

procedure TForm_DepartmentWiseChargeSetup.le_FreeFollowupDaysExit(
  Sender: TObject);
begin
...
Procedure TForm_DepartmentWiseChargeSetup.NewEntryMode;
Begin
     pb_IsNew:=True;
     Query_Department.Close;
     Query_Department.Open;
     Dblcb_Department.KeyValue:=pi_DepID;
     Dblcb_Department.Enabled:=True;
     Dblcb_Department.Color:=clWhite;
     ...
     begin
          ...
          LE_IntermediateFollowupDays.Enabled:=True;
          LE_IntermediateFollowupDays.Color:=clWhite;

          Label3.Enabled:=True;
          Edit_InterFollowUpCharge.Enabled:=True;
          Edit_InterFollowUpCharge.Color:=clWindow;

          Edit_InterFollowupTaxPer.Enabled:=True;
          Edit_InterFollowupTaxPer.Color:=clWindow;

          Edit_InterFollowupVatAmt.Enabled:=True;
          Edit_InterFollowupVatAmt.Color:=clWindow;

          Edit_InterFollowupTotalAmount.Enabled:=True;
          Edit_InterFollowupTotalAmount.Color:=clWindow;
     end;
