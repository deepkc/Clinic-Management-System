    ...
    begin
          ...
          begin
               ...
               begin
                    ...
                    //SQL.Clear;
                    SQL[8]:='And BillDate<=' + #39 + gs_to + #39;
                    Open;
               end;
             ...
             begin
                 With QueryDeposit Do
                    Begin
                         ...
                         Session:=DM_Hospital.DB;
                         Sql.Clear;
                         Sql.Add('select PatientId,Patientname,Billno,Sum(ReceiptAmount)as Dr,ReceiptDate BillDAte,RECEIPTTIME BillTime,(Select UserName From UserMain where userId=cp.RECEIPTBY)BillByUser');
                         Sql.Add('From creditpaymentmaster Cp ');
                         Sql.Add('where RECEIPTDATE>=' + #39 + gs_From + #39 + ' And RECEIPTDATE<=' + #39 + gs_to + #39);
                         Sql.Add('group by PatientId,ReceiptDate,Billno,RECEIPTTIME,PatientName,RECEIPTBY');
