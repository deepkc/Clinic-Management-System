unit Unit_PatientTestRectification;
...
uses
  ...
  DateEditXControl_TLB, ExtCtrls, Buttons, Grids, DBGrids,ServerDate, SMDBGrid,Unit_Billing;

type
  TForm_PatientTestRectification = class(TForm)
    Panel1: TPanel;
    ...
    ds_tempbill: TDataSource;
    query_tempbilling: TTable;
  ...
  pf_TestPrice, pf_Dis, pf_SvrTax, pf_Qty, pf_TotalPrice, pf_disper,
      ...
      pf_DOLLARTOTALAMOUNT, pf_DOLLARDISCOUNT, pf_DOLLARSERVICECHARGE,
      pf_PrevRefundTotal:double;
      pi_TotItems,pi_TotalItems,pi_billdetailid,pi_servicebilldetailid
      ,pi_gridselectedtestnameid
      ,pi_gridselectedtestprogressstatus,pi_patienttestid:integer;
      pi_gridselectedtestnamecode,pi_gridselectedtestname,pi_testnameid,ps_servicebillno,ps_billno :string;
  ...
  public
      ...
      Function IsBillExistR(BillNo: String): Boolean;

  end;

var
  Form_PatientTestRectification: TForm_PatientTestRectification;

implementation

{$R *.DFM}

procedure TForm_PatientTestRectification.BitBtnCancelClick(Sender: TObject);
begin
   Close;
end;

procedure TForm_PatientTestRectification.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TForm_PatientTestRectification.EditHospitalNoKeyPress(Sender: TObject;
  var Key: Char);
begin
     IF key=#8 Then Exit;
...
procedure TForm_PatientTestRectification.Edit_PreveBillNoKeyDown(
...
procedure TForm_PatientTestRectification.Edit_PreveBillNoKeyPress(
...
begin
          ...
          begin
               ...
               begin
                    ...
                    Begin
                         ...
                         SQL.Add(' From HS_sebd_serviceBillDetail BD where sebd_PatientId=' + IntToStr
                                   (pi_PatientID) + ' and sebd_BillType=''B'' and SEBD_BILLDETAILID=0');
                         SQL.Add(
               ...
               Begin
                    ...
                    Begin
                         ...
                         SQL.Add(' From HS_BIDE_BillDetail BD where BIDE_PatientId=' + IntToStr
                                   (pi_PatientID) + ' and BIDE_BillType=''B''');
                         SQL.Add(
               ...
               Begin
                    ...
                    Begin
                         ...
                         SQL.Add(' From HS_BIDE_BillDetail BD where BIDE_PatientId=' + IntToStr(pi_PatientID) +
                         ...
                         SQL.Add(' From HS_SEBD_ServiceBillDetail SBD where SEBD_PatientId=' + IntToStr(pi_PatientID) +
     ...
     begin
          ...
          begin
               ...
               pi_testnameid:='';
               pi_billdetailid:=0;
               pi_servicebilldetailid:=0;
               pi_gridselectedtestnameid:=0;
               pi_gridselectedtestprogressstatus:=0;
               DBLCB_Scheme.KeyValue:=-1;
               pi_patienttestid:=0;
               ps_servicebillno:='';
               if IsBillExistR(Edit_PreveBillNo.Text) = False then
               begin
                    MessageDlg(' No Items Exist With This Bill No. " ' +
                              Edit_PreveBillNo.Text + ' "', mtWarning, [MbOk], 0);
                    Edit_PreveBillNo.Text:=ps_billno;
               ...
               ps_billno:=Edit_PreveBillNo.Text;
               DBGrid_Billing.OnCellClick(Column);
          end;
     end;
end;

Function TForm_PatientTestRectification.IsBillExistR(BillNo: String): Boolean;
Var
     Qry: TOraQuery;
begin
     ...
     begin
          ...
          begin
               ...
               sql.Add
                 (' where SEBD_BillNo=' + #39 + BillNo +#39);
               SQL.Add('AND SEBD_PATIENTID='+INTTOSTR(gi_PatientId));
          end;
          open;
     End;
...
procedure TForm_PatientTestRectification.LoadForBillRefund(BillNo: string);
Var
  li_RemQty: Double;
...
Begin
     ...
     Begin
          ...
          Session := DM_Hospital.DB;
          SQL.Clear;
          if  (RB_ServiceBilling.Checked=false) and (RB_OPBILL.Checked = True) or (RB_ServiceBilling.Checked=false) AND (Query_RefundBillList.FieldByName('BillType').AsString = 'FINALBILL') then
          Begin
               ...
               SQL.Add(' BD.BIDE_PATIENTID PATIENTID,BD.BIDE_INPATIENTID INPATIENTID,BD.BIDE_DEPID DEPID,BD.BIDE_DOCCODE DOCCODE,');
               ...
               sql.Add('(select testprogressstatus from patienttest pt where patientid=bide_patientid and billdetailid=bide_billdetailid)testprogressstatus,');
               sql.Add('(select patienttestid from patienttest pt where patientid=bide_patientid and billdetailid=bide_billdetailid)patienttestid,');
               SQL.Add('(SELECT UNDOSTATUS FROM SAMPLECOLLECTION WHERE PATIENTTESTID=(select patienttestid from patienttest pt where patientid=bide_patientid and billdetailid=bide_billdetailid))UNDOSTATUS,');
               ...
               SQL.Add(' , (SELECT SEBD_BILLNO FROM HS_SEBD_SERVICEBILLDETAIL WHERE SEBD_SERVICEBILLDETAILID=BD.BIDE_SERVICEBILLDETAILID)SERVICEBILLNO');
...
//               sql.Add('(select testprogressstatus from patienttest pt where patientid=bide_patientid and billdetailid=bide_billdetailid)testprogressstatus,');
          ...
          End;
          //sql.saveToFile('C:\RefundDetail1.Txt');
          Open;
          First;
          ...
          Begin
               ...
               begin
                    ...
                    FieldByName('UNDOSTATUS').AsInteger :=Query_RefundBillItem.FieldByName('UNDOSTATUS').AsInteger;
                    FieldByName('BillNo').AsString := Query_RefundBillItem.FieldByName('BillNo').AsString;
                    FieldByName('ServiceBillNo').AsString := Query_RefundBillItem.FieldByName('ServiceBillNo').AsString;
                    ...
                    FieldByName('CancelStatus').AsString :=Query_RefundBillItem.FieldByName('CancelStatus').AsString;
               ...
               if pi_testnameid='' then
                    pi_testnameid:=Query_RefundBillItem.FieldByName('TestNameId').AsString
               else
                    pi_testnameid:=pi_testnameid+','+Query_RefundBillItem.FieldByName('TestNameId').AsString;
     ...
     begin
          close;
          sql[2]:='and tena_testnameid not in ('+pi_testnameid+')';
          open;
     end;
     ...
     Begin
          IF trim(Edit_PreveBillNo.Text) <> '' Then
          Begin
               Filter := 'CancelStatus =''N'' and testprogressstatus<=3 and refundbillno is null';
               Filtered := true;
          End
          Else
               Filtered := false;
...
procedure TForm_PatientTestRectification.CalculateLabels;
Var
...
begin
     ...
     begin
          ...
          edit1.Text:=floattostr(pf_nettotal);
          edit2.Text:=floattostr(pf_grandtotal);
          edit3.Text:=floattostr(pf_svrtaxtotal);
...
procedure TForm_PatientTestRectification.CreateTableFinalBill;
begin
     ...
     begin
          ...
          FieldDefs.Add('ServiceBillNo', ftString, 15);
          ...
          FieldDefs.Add('UndoStatus', ftString, 1);
...
procedure TForm_PatientTestRectification.EditHosNoKeyPress(Sender: TObject;
  var Key: Char);
begin
     IF key=#8 Then Exit;
...
procedure TForm_PatientTestRectification.DBGridBillListCellClick(
  Column: TColumn);
begin
...
procedure TForm_PatientTestRectification.DBGrid_BillingCellClick(
  Column: TColumn);
  var qry:tquery;
begin
     pi_billdetailid:=table_billing.FieldByName('billdetailid').AsInteger;
     pi_servicebilldetailid:=table_billing.FieldByName('servicebilldetailid').AsInteger;
     pi_depid:=table_billing.FieldByName('depid').AsInteger;
     pi_patienttestid:=table_billing.FieldByName('patienttestid').AsInteger;
     ps_servicebillno:=table_billing.FieldByName('servicebillno').AsString;
     pi_gridselectedtestnameid:=table_billing.FieldByName('testnameid').AsInteger;
     pi_gridselectedtestnamecode:=table_billing.FieldByName('testnamecode').AsString;
     pi_gridselectedtestname:=table_billing.FieldByName('testname').AsString;
     pi_gridselectedtestprogressstatus:=table_billing.FieldByName('testprogressstatus').AsInteger;
     Qry := TQuery.Create(Nil);
     with qry do
     begin
          close;
          databasename:=gs_temppath;
          sql.Clear;
          sql.Add('update billing.db set status=''Y'' ');
          sql.Add('where billdetailid='+inttostr(pi_billdetailid));
          ExecSQL;

          close;
          sql.Clear;
          sql.Add('update billing.db set status='''' ');
          sql.Add('where billdetailid <>'+inttostr(pi_billdetailid));
          ExecSQL;
     end;
     table_billing.Close;
     table_billing.Open;
     Table_Billing.Locate('BillDetailID', pi_billdetailid, []);

end;

procedure TForm_PatientTestRectification.DBGrid_BillingDrawColumnCell(
  Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
     if table_billing.FieldByName('status').AsString='Y' then
     begin
...
procedure TForm_PatientTestRectification.DBGrid_PrevBillNoInfoDblClick(
  Sender: TObject);
  var key :char;
begin
     Edit_PreveBillNo.Text := Query_RefundBillList.FieldByName('BillNo').AsString;
...
procedure TForm_PatientTestRectification.DBLCB_SchemeKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
     if Key=VK_DELETE then
     DBLCB_Scheme.KeyValue:=-1;
end;

procedure TForm_PatientTestRectification.BtnSaveClick(Sender: TObject);
Var ls_LogRemarks,ls_testnamecode: string;
var li_schemeid,li_communityid,maxidGlobalLog,li_testnameid
,li_depid,li_maxid:integer;

 ls_totalamount,ls_testprice,ls_vatamount :double;
k :char;
...
//     if pi_billdetailid=0 then
//     begin
//          showmessage('Please select test you want to change!!!');
//          DBGrid_Billing.SetFocus;
//          exit;
//     end;
...
//     if pi_gridselectedtestprogressstatus>3 then
//     begin
//          MessageDlg('Findings are entered. Can not change!!!', mtWarning, [mbok], 0);
//          exit;
//     end;

     //showmessage(inttostr(pi_gridselectedtestnameid));

     Query_RefundBillItem.First;
     while not Query_RefundBillItem.eof do
     begin
          if DBLCB_Scheme.KeyValue= Query_RefundBillItem.FieldByName('testnameid').AsInteger then
          begin
               MessageDlg('Test can not be change. It is already in the list!!!', mtWarning, [mbok], 0);
               exit;
          end;
          Query_RefundBillItem.Next;
     end;
     with query_testdetail do
     begin
          close;
          session := dm_hospital.DB;
          sql.clear;
          sql.Add('select (select tena_testnamecode from hs_tena_testname where tena_testnameid=a.tenp_testnameid)testnamecode,');
          sql.Add('a.* from HS_TENP_TESTNAMEPRICE a where a.tenp_patienttypecode=''GEN'' and ');
          sql.Add('a.tenp_testnameid='+inttostr(DBLCB_Scheme.KeyValue));
          //sql.SaveToFile('c:\testnameid.txt');
          open;
          li_depid:=query_testdetail.FieldByName('TENP_DEPID').AsInteger;
          li_testnameid:=query_testdetail.FieldByName('tenp_testnameid').AsInteger;
          ls_testnamecode:=query_testdetail.FieldByName('testnamecode').AsString;
          ls_testprice:=query_testdetail.FieldByName('TENP_TESTPRICE').AsFloat;
          ls_vatamount:=query_testdetail.FieldByName('tenp_vatamt').AsFloat;
          ls_totalamount:=ls_testprice+ls_vatamount;
     end;
     if ls_testprice<=0 then// cannot change to test which have no testprice
     begin
          MessageDlg('You can not change to this test!!!', mtWarning, [mbok], 0);
          exit;
     end;
     if MessageDlg('Are you sure you want to change selected test ?',mtConfirmation,[mbYes,mbNo],0)=mrYes then
     begin
          try
               li_maxid:=getmaxid('hs_trlo_testrectificationlog','trlo_logid');
               ...
               begin
                    ...
                    session :=dm_hospital.DB;
                    sql.Clear;
                    sql.Add('insert into hs_trlo_testrectificationlog values(');
                    sql.Add(inttostr(li_maxid)+',');
                    sql.Add(inttostr(pi_billdetailid)+',');
                    sql.Add(inttostr(pi_servicebilldetailid)+',');
                    sql.Add(inttostr(pi_PatientId)+',');
                    sql.Add(inttostr(pi_patienttestid)+',');
                    sql.Add(quotedstr(Edit_PreveBillNo.Text)+',');
                    sql.Add(quotedstr(ps_servicebillno)+',');
                    sql.Add(inttostr(li_depid)+',');
                    sql.Add(inttostr(li_testnameid)+',');
                    sql.Add(quotedstr(ls_testnamecode)+',');
                    sql.Add(quotedstr(DBLCB_Scheme.Text)+',');
                    sql.Add(inttostr(pi_depid)+',');
                    sql.Add(inttostr(pi_gridselectedtestnameid)+',');
                    sql.Add(quotedstr(pi_gridselectedtestnamecode)+',');
                    sql.Add(quotedstr(pi_gridselectedtestname)+',');
                    sql.Add(quotedstr(gs_macid)+',');
                    sql.Add(quotedstr(todaysdate)+',');
                    sql.Add(quotedstr(todaystime)+',');
               ...
               begin
                    ...
                    DatabaseName := gs_temppath;
                    sql.Clear;
                    sql.Add('update billing.db set depid='+inttostr(li_depid));
                    sql.Add(', testnameid='+inttostr(li_testnameid));
                    sql.Add(', testnamecode='+quotedstr(ls_testnamecode));
                    sql.Add(', testname='+quotedstr(DBLCB_Scheme.Text));
                    sql.Add(', testprice='+floattostr(ls_testprice));
                    sql.Add(', prevtestprice='+floattostr(ls_testprice));
                    sql.Add(', totalprice='+floattostr(ls_testprice));
                    sql.Add(', svrtax='+floattostr(ls_vatamount));
                    sql.Add(', nettotal='+floattostr(ls_totalamount));
                    sql.Add(', status='''' ');
                    sql.Add('where billdetailid='+inttostr(pi_billdetailid));
                    ExecSQL;
                    //sql.SaveToFile('c:\saveeeeeee.txt');
               end;
               ...
               begin
                    sql.Clear;
                    sql.Add('update hs_sebd_servicebilldetail set sebd_depid='+inttostr(li_depid));
                    sql.Add(', SEBD_SERVICETYPE='+quotedstr(DBLCB_Scheme.Text));
                    sql.Add(', SEBD_SERVICE='+quotedstr(ls_testnamecode));
                    sql.Add(', SEBD_AMOUNT='+floattostr(ls_testprice));
                    sql.Add(', SEBD_VATAMT='+floattostr(ls_vatamount));
                    sql.Add(', SEBD_TOTALAMOUNT='+floattostr(ls_totalamount));
                    sql.Add('where SEBD_BILLDETAILID='+inttostr(pi_billdetailid));
                    //sql.SaveToFile('c:\saveeeeeee.txt');
                    ExecSQL;
                    ...
                    Session := DM_Hospital.DB;
                    sql.Clear;
                    sql.Add('update hs_sebm_servicebillmaster set SEBM_GROSSTOTAL='+edit2.Text);
                    //sql.Add(', SEBM_VATTOTAL='+floattostr(ls_vatamount));
                    sql.Add(', SEBM_nettotal='+edit1.Text);
                    sql.Add('where SEBM_billno='+quotedstr(ps_servicebillno));
                    //sql.SaveToFile('c:\saveeeeeee.txt');
                    ExecSQL;

                    sql.Clear;
                    sql.Add('update hs_bide_billdetail set bide_depid='+inttostr(li_depid));
                    sql.Add(', BIDE_SERVICETYPE='+quotedstr(DBLCB_Scheme.Text));
                    sql.Add(', BIDE_SERVICE='+quotedstr(ls_testnamecode));
                    sql.Add(', BIDE_AMOUNT='+floattostr(ls_testprice));
                    sql.Add(', BIDE_VATAMT='+floattostr(ls_vatamount));
                    sql.Add(', BIDE_TOTALAMOUNT='+floattostr(ls_totalamount));
                    sql.Add('where BIDE_BILLDETAILID='+inttostr(pi_billdetailid));
                    //sql.SaveToFile('c:\saveeeeeee.txt');
                    ExecSQL;
                    ...
                    Session := DM_Hospital.DB;
                    sql.Clear;
                    sql.Add('update hs_bima_billmaster set bima_grosstotal='+edit2.Text);
                    //sql.Add(', bima_VATTOTAL='+floattostr(ls_vatamount));
                    sql.Add(', bima_nettotal='+edit1.Text);
                    sql.Add('where bima_billno='+quotedstr(Edit_PreveBillNo.Text));
                    //sql.SaveToFile('c:\saveeeeeee.txt');
                    ExecSQL;

                    sql.Clear;
                    sql.Add('update patienttest set depid='+inttostr(li_depid));
                    sql.Add(', testnameid='+inttostr(li_testnameid));
                    sql.Add(', testnamecode='+quotedstr(ls_testnamecode));
                    sql.Add(', testamount='+floattostr(ls_testprice));
                    sql.Add('where billdetailid='+inttostr(pi_billdetailid));
                    //sql.SaveToFile('c:\saveeeeeee.txt');
                    ExecSQL;
...
procedure TForm_PatientTestRectification.CB_RemoveSchemeClick(Sender: TObject);
begin
...
procedure TForm_PatientTestRectification.EditHosNoExit(Sender: TObject);
begin
     IF (Trim(EditHosNo.Text)<>'') and (EditHosNo.Text<>'0') Then
     //SBSearchClick(Sender);
end;

Procedure TForm_PatientTestRectification.GetPatientInfo;
Begin
...
procedure TForm_PatientTestRectification.RB_DirectBillingClick(
  Sender: TObject);
begin
     if RB_DirectBilling.Checked=True then
...
procedure TForm_PatientTestRectification.RB_ServiceBillingClick(
  Sender: TObject);
begin
     if RB_ServiceBilling.Checked=True then
...
procedure TForm_PatientTestRectification.SBSearchClick(Sender: TObject);
begin
...
procedure TForm_PatientTestRectification.FormShow(Sender: TObject);
begin
     Query_Scheme.Close;
     Query_Scheme.Open;
     DBLCB_Scheme.KeyValue:=-1;
