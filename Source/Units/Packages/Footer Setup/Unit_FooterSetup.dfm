object Form_FooterSetup: TForm_FooterSetup
  Left = 0
  Top = 0
  Caption = 'Footer Setup'
  ClientHeight = 429
  ClientWidth = 833
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 833
    Height = 33
    Align = alTop
    BevelOuter = bvNone
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object BB_Save: TBitBtn
      Left = 677
      Top = 0
      Width = 78
      Height = 33
      Margins.Left = 0
      Margins.Top = 0
      Margins.Right = 0
      Margins.Bottom = 0
      Align = alRight
      Caption = '&Save'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 0
      OnClick = BB_SaveClick
    end
    object BB_Close: TBitBtn
      Left = 755
      Top = 0
      Width = 78
      Height = 33
      Margins.Left = 0
      Margins.Top = 6
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = '&Close'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFEDEDF77C7DC96162C58383CB}
      ParentDoubleBuffered = False
      TabOrder = 1
      OnClick = BB_CloseClick
    end
    object BB_New: TBitBtn
      Left = 599
      Top = 0
      Width = 78
      Height = 33
      Margins.Left = 0
      Margins.Top = 0
      Margins.Right = 0
      Margins.Bottom = 0
      Align = alRight
      Caption = '&New'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 2
      OnClick = BB_NewClick
    end
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 33
    Width = 833
    Height = 396
    ActivePage = TabSheet2
    Align = alClient
    TabOrder = 1
    OnChange = PageControl1Change
    object TabSheet1: TTabSheet
      Caption = 'Footer'
      DesignSize = (
        825
        368)
      object Label2: TLabel
        Left = 5
        Top = 40
        Width = 114
        Height = 21
        Alignment = taCenter
        AutoSize = False
        Caption = 'Footer'
        Color = 2922059
        Font.Charset = ANSI_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        Transparent = False
        Layout = tlCenter
      end
      object SpeedButton1: TSpeedButton
        Left = 732
        Top = 3
        Width = 49
        Height = 51
        Flat = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
      end
      object SpeedButton2: TSpeedButton
        Left = 781
        Top = 3
        Width = 49
        Height = 51
        Flat = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
      end
      object Label6: TLabel
        Left = 56
        Top = 18
        Width = 61
        Height = 13
        Alignment = taRightJustify
        Caption = 'Test Name  :'
      end
      object spbtnSendToExcel: TSpeedButton
        Left = 547
        Top = 9
        Width = 42
        Height = 47
        Flat = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
        Visible = False
        OnClick = spbtnSendToExcelClick
      end
      object SpeedButton3: TSpeedButton
        Left = 487
        Top = 9
        Width = 49
        Height = 47
        Flat = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
      end
      object Edit_Search: TEdit
        Left = 123
        Top = 41
        Width = 231
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 1
        OnChange = Edit_SearchChange
      end
      object Dblcb_TestName: TDBLookupComboBox
        Left = 123
        Top = 14
        Width = 231
        Height = 21
        KeyField = 'TestNameID'
        ListField = 'TestName'
        ListSource = DS_TestName
        TabOrder = 0
        OnClick = Dblcb_TestNameClick
        OnKeyDown = Dblcb_TestNameKeyDown
      end
      object SMDBGrid1: TSMDBGrid
        Left = 3
        Top = 66
        Width = 819
        Height = 301
        Anchors = [akLeft, akTop, akRight, akBottom]
        DataSource = Ds_List
        Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        TabOrder = 2
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnDblClick = SMDBGrid1DblClick
        Flat = False
        BandsFont.Charset = DEFAULT_CHARSET
        BandsFont.Color = clWindowText
        BandsFont.Height = -11
        BandsFont.Name = 'Tahoma'
        BandsFont.Style = []
        Groupings = <>
        GridStyle.Style = gsPriceList
        GridStyle.OddColor = cl3DLight
        GridStyle.EvenColor = clWindow
        TitleHeight.PixelCount = 24
        FooterColor = clBtnFace
        ExOptions = [eoENTERlikeTAB, eoKeepSelection, eoStandardPopup, eoBLOBEditor, eoShowFilterBar, eoAnyKeyFilter, eoFilterAutoApply]
        RegistryKey = 'Software\Scalabium'
        RegistrySection = 'SMDBGrid'
        WidthOfIndicator = 11
        DefaultRowHeight = 17
        ScrollBars = ssHorizontal
        Columns = <
          item
            Expanded = False
            FieldName = 'FooterCode'
            Title.Caption = 'Footer Code'
            Width = 100
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'Footer_Grid'
            Title.Caption = 'Footer'
            Width = 458
            Visible = True
          end>
      end
    end
    object TabSheet2: TTabSheet
      Caption = 'New Footer'
      ImageIndex = 1
      object Label7: TLabel
        Left = 47
        Top = 15
        Width = 6
        Height = 13
        Alignment = taRightJustify
        Caption = '*'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label3: TLabel
        Left = 76
        Top = 72
        Width = 36
        Height = 13
        Caption = 'Footer:'
      end
      object Label4: TLabel
        Left = 38
        Top = 45
        Width = 6
        Height = 13
        Alignment = taRightJustify
        Caption = '*'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label1: TLabel
        Left = 53
        Top = 16
        Width = 61
        Height = 13
        Alignment = taRightJustify
        Caption = 'Test Name  :'
      end
      object Label5: TLabel
        Left = 69
        Top = 71
        Width = 6
        Height = 13
        Alignment = taRightJustify
        Caption = '*'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Le_FooterCode: TLabeledEdit
        Left = 117
        Top = 42
        Width = 145
        Height = 21
        CharCase = ecUpperCase
        EditLabel.Width = 67
        EditLabel.Height = 13
        EditLabel.Caption = 'Footer Code :'
        LabelPosition = lpLeft
        TabOrder = 2
      end
      object CB_IsActive: TCheckBox
        Left = 270
        Top = 17
        Width = 64
        Height = 17
        Caption = 'Is Active'
        Checked = True
        State = cbChecked
        TabOrder = 0
      end
      object Dblcb_TestNameNew: TDBLookupComboBox
        Left = 119
        Top = 15
        Width = 145
        Height = 21
        DropDownWidth = 250
        KeyField = 'TestNameID'
        ListField = 'TestName'
        ListSource = DS_TestName
        TabOrder = 1
        OnKeyDown = Dblcb_TestNameNewKeyDown
      end
      object RichEdit_Footer: TRichEdit
        Left = 117
        Top = 69
        Width = 706
        Height = 298
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
      end
    end
  end
  object Query_list: TOraQuery
    SQL.Strings = (
      
        'Select F.*,Replace(CAST(F.Footertext AS VARCHAR2(50)),chr(13),ch' +
        'r(9))Footer_GRID from Footer F'
      'where 99=99 '
      'AND 99=99'
      'Order by Footercode'
      ''
      '')
    Left = 248
    Top = 184
  end
  object Ds_List: TDataSource
    DataSet = Query_list
    Left = 200
    Top = 216
  end
  object Query_TestName: TOraQuery
    SQL.Strings = (
      
        'select tena_TestNameID TestNameID,TENA_TestNameCode TestNameCode' +
        ',TENA_TestName TestName from HS_TENA_TestName'
      'order by TENA_TestName')
    Left = 411
    Top = 217
  end
  object DS_TestName: TDataSource
    DataSet = Query_TestName
    Left = 376
    Top = 216
  end
  object Qry: TOraQuery
    SQL.Strings = (
      
        'Select F.*,Replace(CAST(F.Footertext AS VARCHAR2(50)),chr(13),ch' +
        'r(9))Footer_GRID from Footer F'
      'where 99=99 '
      'AND 99=99'
      'Order by Footercode'
      ''
      '')
    Left = 496
    Top = 216
  end
  object OraQuery_Process: TOraQuery
    Left = 384
    Top = 120
  end
end
