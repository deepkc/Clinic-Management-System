...
var ps_fromdate,ps_todate:String;
begin
    ...
    // BitBtn_ToDate.Caption := 'BS';
     ...
     begin
          ...
          begin
               ...
               sql.Clear;
               sql.Add(' select A.* from (SELECT ROWNUM AS SN,');
               sql.Add(' (select TRIM(REPLACE(REPLACE(REPLACE(replace(REDO_docname,''DR''),''PROF''),''.''),''(MRS)'')) from HS_REDO_referaldoctor where REDO_docid=CRD.REFDOCid) ReferalDoc,');
               sql.Add(' (select HOSE_hospitalname from HS_HOSE_hospitalsetup where HOSE_hospitalcode=CRD.REFHOSCODE) ReferalHos,(select PAMA_PATIENTNAME from hs_pama_patientmain where pama_patientid=CRD.patientid)patientname,');
               sql.Add(' CRD.* from VW_CREDITRECEIPTDETAIL CRD');
               sql.Add(' Where ReceiptDate>='+QuotedStr(gs_From)+' And ReceiptDate<='+QuotedStr(gs_To)+')A');
               if DBLCB_Hospital.KeyValue<>null then
               sql.Add(' where REFHOSCODE='+QuotedStr(DBLCB_Hospital.KeyValue));
               if DBLCB_RefDocCode.KeyValue<>null then
               sql.Add(' And REFDOCID='+QuotedStr(DBLCB_RefDocCode.KeyValue));
               //sql.savetofile('c:\abc454.txt');
               Open;
          end;
          if pb_ToXl =False  then
          qr_creditCollection.Preview
          else
          SendToExcels_query(Query_Receipt,nil,'New Department Wise Collection',ps_fromdate+'-'+ps_todate,'',
...
procedure TForm_ReceiptDetailReport.DBLCB_HospitalClick(Sender: TObject);
begin
     with QueryRefDoc do
     begin
          Close;
