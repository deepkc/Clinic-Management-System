unit Unit_CashServiceSalesReport;
...
type
  TForm_CashServiceSalesReport = class(TForm)
    Panel2: TPanel;
    Label1: TLabel;
    Label2: TLabel;
...
var
  Form_CashServiceSalesReport: TForm_CashServiceSalesReport;
...
procedure TForm_CashServiceSalesReport.BitBtn1Click(Sender: TObject);
begin
     pb_ToXl:=True;
     BitBtnPreviewClick(Sender);
end;

procedure TForm_CashServiceSalesReport.BitBtnCancelClick(Sender: TObject);
begin
     Close;
end;

procedure TForm_CashServiceSalesReport.BitBtnPreviewClick(Sender: TObject);
...
begin
     ...
     begin
          ...
          begin
               ...
               Session:= DM_Hospital.DB;
               sql.Clear;
               sql.Add(' Select * from VW_SERVICECREDITSALE CCS ') ;
               sql.Add(' Where billdate >='+QuotedStr(DateEditX_FromDate.text)+' and billdate<='+QuotedStr(DateEditX_ToDate.text)+'');
               if DBLCB_RefDocCode.KeyValue<>NULL then
               begin
                    SQL.Add(' AND REFDOCID='+intToStr(DBLCB_RefDocCode.KeyValue));
          ...
          {SendToExcelsMultiQuery_UserColnDetail(Query_CashCredit,Query_CashRefund,Query_ServiceCredit,Query_ServiceRefund,nil,
                                   nil,nil,nil,nil,nil,Nil,
                                   'Cash Service Sale Report (Summary)','','','','','','','','','','',DateEditX_FromDate.text+'-'+DateEditX_ToDate.text,
                                   'Generated Date/Time/By :'+TodaysDate+'/'+TodaysTime+'/'+gs_UserName,gs_HospitalName,gs_HospitalAddress,10,5); }
...
procedure TForm_CashServiceSalesReport.BitBtn_FromDateClick(Sender: TObject);
begin
...
procedure TForm_CashServiceSalesReport.BitBtn_ToDateClick(Sender: TObject);
begin
...
procedure TForm_CashServiceSalesReport.DBLCB_HospitalClick(Sender: TObject);
begin
     with QueryRefDoc do
     begin
          Close;
...
procedure TForm_CashServiceSalesReport.DBLCB_HospitalKeyDown(Sender: TObject;
...
procedure TForm_CashServiceSalesReport.DBLCB_RefDocCodeKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
     if Key = VK_DELETE then
     begin
...
procedure TForm_CashServiceSalesReport.FormKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
      if Key = 27 then
          close;
...
procedure TForm_CashServiceSalesReport.FormShow(Sender: TObject);
begin
