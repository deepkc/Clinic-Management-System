object Form_DepartmentWiseSummary: TForm_DepartmentWiseSummary
  Left = 566
  Top = 259
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = 'Department Wise Summary'
  ClientHeight = 240
  ClientWidth = 376
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poMainFormCenter
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 0
    Top = 31
    Width = 376
    Height = 190
    Align = alClient
    TabOrder = 1
    object Label1: TLabel
      Left = 47
      Top = 152
      Width = 52
      Height = 13
      Alignment = taRightJustify
      Caption = 'From Date:'
    end
    object Label2: TLabel
      Left = 206
      Top = 152
      Width = 16
      Height = 13
      Caption = 'To:'
    end
    object Bevel1: TBevel
      Left = 0
      Top = 0
      Width = 368
      Height = 22
    end
    object Label3: TLabel
      Left = 38
      Top = 91
      Width = 61
      Height = 13
      Alignment = taRightJustify
      Caption = 'Department :'
    end
    object DateEditX_FromDate: TDateEditX
      Left = 106
      Top = 148
      Width = 93
      Height = 21
      TabOrder = 6
      ControlData = {
        545046300A5444617465456469745200044C656674026A03546F700394000577
        69647468025D0B4461746541442E5965617203D6070C4461746541442E4D6F6E
        746802010A4461746541442E44617902180B4461746556532E59656172030E08
        0C4461746556532E4D6F6E7468020A0A4461746556532E446179020B0000}
    end
    object DateEditX_ToDate: TDateEditX
      Left = 229
      Top = 148
      Width = 93
      Height = 21
      TabOrder = 7
      ControlData = {
        545046300A5444617465456469745200044C65667403E50003546F7003940005
        7769647468025D0B4461746541442E5965617203D6070C4461746541442E4D6F
        6E746802010A4461746541442E44617902180B4461746556532E59656172030E
        080C4461746556532E4D6F6E7468020A0A4461746556532E446179020B0000}
    end
    object BitBtn_FromDate: TBitBtn
      Left = 173
      Top = 149
      Width = 24
      Height = 18
      Caption = 'BS'
      DoubleBuffered = True
      ParentDoubleBuffered = False
      TabOrder = 9
      TabStop = False
      OnClick = BitBtn_FromDateClick
    end
    object BitBtn_ToDate: TBitBtn
      Left = 297
      Top = 148
      Width = 24
      Height = 18
      Caption = 'BS'
      DoubleBuffered = True
      ParentDoubleBuffered = False
      TabOrder = 10
      TabStop = False
      OnClick = BitBtn_ToDateClick
    end
    object RadioButton_OPCollection: TRadioButton
      Left = 139
      Top = 50
      Width = 111
      Height = 17
      Caption = '&OP Collection (F3)'
      Checked = True
      Enabled = False
      TabOrder = 0
      TabStop = True
      Visible = False
      OnClick = RadioButton_OPCollectionClick
    end
    object RadioButton_IPCollection: TRadioButton
      Left = 139
      Top = 74
      Width = 106
      Height = 17
      Caption = '&IP Collection (F4)'
      Enabled = False
      TabOrder = 1
      Visible = False
      OnClick = RadioButton_IPCollectionClick
    end
    object CheckBoxLandSacpe: TCheckBox
      Left = 106
      Top = 254
      Width = 97
      Height = 17
      Caption = 'Print &Landscape'
      Checked = True
      Enabled = False
      State = cbChecked
      TabOrder = 8
      Visible = False
    end
    object RadioButton_IPOPCollection: TRadioButton
      Left = 139
      Top = 27
      Width = 127
      Height = 17
      Caption = 'OP+IP Collection(F5)'
      Enabled = False
      TabOrder = 2
      Visible = False
      OnClick = RadioButton_IPOPCollectionClick
    end
    object DBLC_Department: TDBLookupComboBox
      Left = 106
      Top = 87
      Width = 215
      Height = 21
      KeyField = 'DEPID'
      ListField = 'DEPNAME'
      ListSource = DataSource_Department
      TabOrder = 5
      OnKeyDown = DBLC_DepartmentKeyDown
    end
    object CheckBox_TaxAnalysis: TCheckBox
      Left = 3
      Top = 76
      Width = 80
      Height = 14
      Caption = 'Tax Analysis'
      TabOrder = 3
      Visible = False
      OnClick = CheckBox_TaxAnalysisClick
    end
    object RadioGroup_Tax: TRadioGroup
      Left = -35
      Top = 6
      Width = 136
      Height = 70
      Enabled = False
      Items.Strings = (
        'Taxable'
        'Non-Taxable'
        'Taxable+Non-Taxable')
      TabOrder = 4
      Visible = False
    end
    object CB_ReferralDeptBasis: TCheckBox
      Left = 4
      Top = 93
      Width = 146
      Height = 17
      Caption = 'Referral Department Basis'
      TabOrder = 11
      Visible = False
    end
    object CB_DirectPrint: TCheckBox
      Left = 229
      Top = 259
      Width = 97
      Height = 17
      Caption = 'Direct Print'
      TabOrder = 12
      Visible = False
    end
    object CB_DayWiseRevenueReport: TCheckBox
      Left = 81
      Top = 6
      Width = 183
      Height = 17
      Caption = 'Day Wise Revenue Report'
      TabOrder = 13
      Visible = False
    end
    object CBMergeDep: TCheckBox
      Left = 107
      Top = 224
      Width = 121
      Height = 17
      Caption = 'Merge Department'
      Checked = True
      State = cbChecked
      TabOrder = 14
      Visible = False
    end
    object RG_IP: TRadioGroup
      Left = 270
      Top = 5
      Width = 136
      Height = 70
      Items.Strings = (
        'Final Bill Basis'
        'Discharge In Deposit'
        'Both')
      TabOrder = 15
      Visible = False
    end
    object RG_Category: TRadioGroup
      Left = 38
      Top = 187
      Width = 292
      Height = 31
      Columns = 3
      ItemIndex = 0
      Items.Strings = (
        'Both'
        'BNB'
        'BLK')
      TabOrder = 16
      Visible = False
    end
    object CheckListBox_Department: TCheckListBox
      Left = 121
      Top = 100
      Width = 172
      Height = 15
      OnClickCheck = CheckListBox_DepartmentClickCheck
      Enabled = False
      ItemHeight = 13
      TabOrder = 17
      Visible = False
    end
    object CB_BNBSpl: TCheckBox
      Left = 8
      Top = 113
      Width = 59
      Height = 17
      Caption = 'BNB Spl'
      Enabled = False
      TabOrder = 18
      Visible = False
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 376
    Height = 31
    Align = alTop
    TabOrder = 0
    object BitBtnPreview: TBitBtn
      Left = 0
      Top = 2
      Width = 118
      Height = 28
      Caption = 'Preview'
      DoubleBuffered = True
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 0
      OnClick = BitBtnPreviewClick
    end
    object BitBtn2: TBitBtn
      Left = 255
      Top = 2
      Width = 115
      Height = 28
      Caption = '&Cancel (Esc)'
      DoubleBuffered = True
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 2
      OnClick = BitBtn2Click
    end
    object BitBtnExcel: TBitBtn
      Left = 117
      Top = 2
      Width = 138
      Height = 28
      Caption = 'Send  To &Excel (F2)'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFF0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0}
      ParentDoubleBuffered = False
      TabOrder = 1
      OnClick = BitBtnExcelClick
    end
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 221
    Width = 376
    Height = 19
    Panels = <
      item
        Width = 50
      end
      item
        Width = 50
      end>
  end
  object QueryDetail: TOraQuery
    SQL.Strings = (
      '')
    Left = 40
    Top = 312
  end
  object QueryProcess: TOraQuery
    SQL.Strings = (
      'update billmaster'
      'set freeby='#39'OTHER'#39
      'where freeby is NULL'
      'and dis > 0')
    Left = 64
    Top = 296
  end
  object QuerySubProcess: TOraQuery
    Left = 264
    Top = 117
  end
  object Query_Department: TOraQuery
    SQL.Strings = (
      'Select Distinct DEPID,DepName From VW_HS_DEPT_WISE_REVENUE_BNB1'
      'Order by DepName')
    Left = 307
    Top = 216
  end
  object DataSource_Department: TDataSource
    DataSet = Query_Department
    Left = 338
    Top = 207
  end
  object QueryNetRealizedRev: TOraQuery
    Left = 324
    Top = 129
  end
  object Query_Total: TOraQuery
    Left = 56
    Top = 287
  end
  object Query_Checklist: TOraQuery
    SQL.Strings = (
      
        'select * from hs_paca_patientcategory order by PACA_PATIENTCATEG' +
        'ORYID')
    Left = 32
    Top = 48
  end
  object Table_DepartmentWiseSummary: TTable
    Left = 448
    Top = 168
  end
  object QueryTemp1: TQuery
    Left = 392
    Top = 264
  end
  object QueryTemp: TQuery
    Left = 352
    Top = 80
  end
  object Query_Void: TQuery
    Left = 440
    Top = 40
  end
end
