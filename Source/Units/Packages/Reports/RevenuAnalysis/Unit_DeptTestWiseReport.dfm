object Form_DeptTestWiseReport: TForm_DeptTestWiseReport
  Left = 388
  Top = 188
  Caption = 'Department/Test Wise Report'
  ClientHeight = 388
  ClientWidth = 363
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 363
    Height = 30
    Align = alTop
    TabOrder = 0
    object BitBtnPreview: TBitBtn
      Left = 1
      Top = 1
      Width = 108
      Height = 28
      Align = alRight
      Caption = '&Preview (F1)'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      TabOrder = 0
      OnClick = BitBtnPreviewClick
    end
    object BitBtnExcel: TBitBtn
      Left = 109
      Top = 1
      Width = 138
      Height = 28
      Align = alRight
      Caption = 'Send To Excel (F2)'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        }
      ParentDoubleBuffered = False
      ParentFont = False
      TabOrder = 1
      OnClick = BitBtnExcelClick
    end
    object BitBtnCancel: TBitBtn
      Left = 247
      Top = 1
      Width = 115
      Height = 28
      Align = alRight
      Caption = '&Cancel (Esc)'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      TabOrder = 2
      OnClick = BitBtnCancelClick
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 30
    Width = 363
    Height = 339
    Align = alClient
    TabOrder = 1
    object Bevel1: TBevel
      Left = 0
      Top = 0
      Width = 372
      Height = 20
    end
    object Label1: TLabel
      Left = 42
      Top = 280
      Width = 28
      Height = 13
      Caption = 'From:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label2: TLabel
      Left = 193
      Top = 280
      Width = 16
      Height = 13
      Caption = 'To:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label_Identification: TLabel
      Left = 8
      Top = 226
      Width = 60
      Height = 13
      Alignment = taRightJustify
      Caption = 'Department '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 17
      Top = 252
      Width = 51
      Height = 13
      Alignment = taRightJustify
      Caption = 'Test Name'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object DateEditX_FromDate: TDateEditX
      Left = 77
      Top = 276
      Width = 93
      Height = 21
      TabOrder = 5
      ControlData = {
        
        0000}
    end
    object DateEditX_ToDate: TDateEditX
      Left = 212
      Top = 276
      Width = 93
      Height = 21
      TabOrder = 6
      ControlData = {
        
        080000}
    end
    object BitBtn_FromDate: TBitBtn
      Left = 146
      Top = 277
      Width = 24
      Height = 18
      Caption = 'BS'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentDoubleBuffered = False
      ParentFont = False
      TabOrder = 7
      TabStop = False
      OnClick = BitBtn_FromDateClick
    end
    object BitBtn_ToDate: TBitBtn
      Left = 279
      Top = 277
      Width = 24
      Height = 18
      Caption = 'BS'
      DoubleBuffered = True
      ParentDoubleBuffered = False
      TabOrder = 8
      TabStop = False
      OnClick = BitBtn_ToDateClick
    end
    object CMBDepartment: TDBLookupComboBox
      Left = 77
      Top = 222
      Width = 270
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      KeyField = 'DEPID'
      ListField = 'DEPNAME'
      ListSource = DataSource_Department
      ParentFont = False
      TabOrder = 3
      OnClick = CMBDepartmentClick
      OnKeyDown = CMBDepartmentKeyDown
    end
    object DBLCBTestName: TDBLookupComboBox
      Left = 77
      Top = 248
      Width = 270
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      KeyField = 'TESTNAMECODE'
      ListField = 'TestNameCode;TESTNAME'
      ListFieldIndex = 1
      ListSource = DataSource_TestName
      ParentFont = False
      TabOrder = 4
      OnKeyDown = DBLCBTestNameKeyDown
    end
    object GroupBox1: TGroupBox
      Left = 25
      Top = 111
      Width = 139
      Height = 75
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      object RB_DepartmentWise: TRadioButton
        Left = 6
        Top = 19
        Width = 130
        Height = 17
        Caption = '&Department Wise (F3)'
        Checked = True
        TabOrder = 0
        TabStop = True
        OnClick = RB_DepartmentWiseClick
      end
      object RB_TestWise: TRadioButton
        Left = 6
        Top = 44
        Width = 106
        Height = 17
        Caption = '&Test Wise (F4)'
        TabOrder = 1
        OnClick = RB_TestWiseClick
      end
    end
    object GroupBox2: TGroupBox
      Left = 170
      Top = 111
      Width = 92
      Height = 75
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      Visible = False
      object RBOPBill: TRadioButton
        Left = 6
        Top = 54
        Width = 53
        Height = 17
        Caption = 'OP Bill'
        TabOrder = 0
      end
      object RBIPBill: TRadioButton
        Left = 6
        Top = 35
        Width = 50
        Height = 17
        Caption = 'IP Bill'
        TabOrder = 1
      end
      object RB_OPIP: TRadioButton
        Left = 6
        Top = 17
        Width = 76
        Height = 17
        Caption = 'OP + IP Bill'
        Checked = True
        TabOrder = 2
        TabStop = True
      end
    end
    object CheckBox_WithQty: TCheckBox
      Left = 80
      Top = 198
      Width = 98
      Height = 17
      Caption = 'With &Quantity'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
    end
    object GB_PayType: TGroupBox
      Left = 265
      Top = 111
      Width = 98
      Height = 75
      Caption = 'Pay Type'
      Color = clMenu
      ParentColor = False
      TabOrder = 9
      object RBCash: TRadioButton
        Left = 10
        Top = 36
        Width = 53
        Height = 17
        Caption = 'Cash '
        TabOrder = 0
      end
      object RBCredit: TRadioButton
        Left = 10
        Top = 53
        Width = 50
        Height = 17
        Caption = 'Credit'
        TabOrder = 1
      end
      object RB_PayTypeBoth: TRadioButton
        Left = 10
        Top = 18
        Width = 46
        Height = 17
        Caption = 'Both'
        Checked = True
        TabOrder = 2
        TabStop = True
      end
    end
    object CB_OPDReg: TCheckBox
      Left = 25
      Top = 81
      Width = 200
      Height = 17
      Caption = 'Including &OPD Registration Revenue'
      TabOrder = 10
      Visible = False
    end
    object Cb_Time: TCheckBox
      Left = 54
      Top = 311
      Width = 17
      Height = 17
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 11
      Visible = False
      OnClick = Cb_TimeClick
    end
    object GroupBox3: TGroupBox
      Left = 77
      Top = 297
      Width = 228
      Height = 37
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 12
      Visible = False
      object Label4: TLabel
        Left = 10
        Top = 15
        Width = 28
        Height = 13
        Caption = 'From:'
      end
      object Label5: TLabel
        Left = 105
        Top = 15
        Width = 16
        Height = 13
        Caption = 'To:'
      end
      object Edit_FTime: TMaskEdit
        Left = 46
        Top = 11
        Width = 51
        Height = 21
        EditMask = '!90:00;1;_'
        MaxLength = 5
        TabOrder = 0
        Text = '  :  '
      end
      object Edit_TTime: TMaskEdit
        Left = 134
        Top = 11
        Width = 43
        Height = 21
        EditMask = '!90:00;1;_'
        MaxLength = 5
        TabOrder = 1
        Text = '  :  '
      end
    end
    object Cb_ReferalDep: TCheckBox
      Left = 174
      Top = 198
      Width = 146
      Height = 17
      Caption = 'Referal Department'
      TabOrder = 13
      OnClick = Cb_ReferalDepClick
    end
    object RG_Category: TRadioGroup
      Left = 275
      Top = 74
      Width = 292
      Height = 31
      Columns = 3
      ItemIndex = 0
      Items.Strings = (
        'Both'
        'BNB'
        'BLK')
      TabOrder = 14
      Visible = False
    end
    object CheckListBox_Department: TCheckListBox
      Left = 80
      Top = 6
      Width = 209
      Height = 75
      OnClickCheck = CheckListBox_DepartmentClickCheck
      ItemHeight = 13
      TabOrder = 15
    end
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 369
    Width = 363
    Height = 19
    Panels = <
      item
        Width = 50
      end
      item
        Width = 50
      end>
  end
  object Query_SubProcess: TOraQuery
    SQL.Strings = (
      'update billmaster'
      'set freeby='#39'OTHER'#39
      'where freeby is NULL'
      'and dis > 0')
    Left = 65535
    Top = 89
  end
  object Query_Process: TOraQuery
    SQL.Strings = (
      'update billmaster'
      'set freeby='#39'OTHER'#39
      'where freeby is NULL'
      'and dis > 0')
    Left = 65531
    Top = 145
  end
  object Query_Department: TOraQuery
    SQL.Strings = (
      
        'Select D.Dept_DepId DepId,D.Dept_DepCode  DepCode,D.Dept_DepName' +
        ' DepName From Hs_Dept_Department D'
      'where 1=1'
      'Order By DepName')
    Left = 250
    Top = 158
  end
  object DataSource_Department: TDataSource
    DataSet = Query_Department
    Left = 294
    Top = 183
  end
  object Query_TestName: TOraQuery
    SQL.Strings = (
      
        'Select Tena_TestNameCode TestNameCode,Tena_TestName TestName Fro' +
        'm Hs_Tena_TestName '
      'Where 39=39'
      'Order By TestNameCode')
    Left = 249
    Top = 214
  end
  object DataSource_TestName: TDataSource
    DataSet = Query_TestName
    Left = 304
    Top = 230
  end
  object Query_Master1: TOraQuery
    SQL.Strings = (
      
        'Select Distinct TestNameCode,TestName,PayType  From TestWiseDept' +
        'Wise.db where TestName<>'#39' '#39
      ' ')
    Left = 216
    Top = 38
  end
  object DS_Master: TDataSource
    DataSet = Query_Master1
    Left = 312
    Top = 46
  end
  object Query_Detail2: TOraQuery
    SQL.Strings = (
      
        'Select TestNameCode,TestName,PayType,BillNo,BillDate,UserName,Su' +
        'm(Amount) as Amount,Sum(Discount) as Discount,'
      
        'Sum(SubTotal) as SubTotal,Sum(VatAmt) as VatAmt,Sum(NetAmount) a' +
        's NetAmount From'
      
        'TestWiseBill.db where TestName=:TestName and PayType=:PayType an' +
        'd TestName<>'#39' '#39' Group BY TestNameCode,TestName,PayType,BillNo,Bi' +
        'llDate,UserName'
      'Order By TestNameCode,TestName,PayType,BillNo')
    MasterSource = DS_Master
    Left = 312
    Top = 30
    ParamData = <
      item
        DataType = ftString
        Name = 'TestName'
      end
      item
        DataType = ftString
        Name = 'PayType'
      end>
  end
  object Query_Void: TOraQuery
    SQL.Strings = (
      'update billmaster'
      'set freeby='#39'OTHER'#39
      'where freeby is NULL'
      'and dis > 0')
    Left = 65535
    Top = 137
  end
  object Query1: TOraQuery
    Left = 44
    Top = 144
  end
  object Query_Checklist: TOraQuery
    SQL.Strings = (
      
        'select * from hs_paca_patientcategory order by PACA_PATIENTCATEG' +
        'ORYID')
    Left = 32
    Top = 40
  end
  object Table_TestWiseBill: TTable
    Left = 224
    Top = 48
  end
  object Table_TestWiseDeptWise: TTable
    Left = 192
    Top = 72
  end
end
