object Form_ReportFooterSetup: TForm_ReportFooterSetup
  Left = 0
  Top = 0
  AlphaBlend = True
  Caption = 'Footer Setup'
  ClientHeight = 466
  ClientWidth = 636
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 636
    Height = 33
    Align = alTop
    BevelOuter = bvNone
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object BB_Save: TBitBtn
      Left = 480
      Top = 0
      Width = 78
      Height = 33
      Margins.Left = 0
      Margins.Top = 0
      Margins.Right = 0
      Margins.Bottom = 0
      Align = alRight
      Caption = '&Save'
      DoubleBuffered = True
      Enabled = False
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 0
      OnClick = BB_SaveClick
    end
    object BB_Close: TBitBtn
      Left = 558
      Top = 0
      Width = 78
      Height = 33
      Margins.Left = 0
      Margins.Top = 6
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = '&Close'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFEDEDF77C7DC96162C58383CB}
      ParentDoubleBuffered = False
      TabOrder = 1
      OnClick = BB_CloseClick
    end
    object BB_New: TBitBtn
      Left = 402
      Top = 0
      Width = 78
      Height = 33
      Margins.Left = 0
      Margins.Top = 0
      Margins.Right = 0
      Margins.Bottom = 0
      Align = alRight
      Caption = '&New'
      DoubleBuffered = True
      Glyph.Data = {
        
        C596F5C596F5C596F5C596EEEEEEF4F4F4FAFAFAFEFEFEFFFFFF}
      ParentDoubleBuffered = False
      TabOrder = 2
      OnClick = BB_NewClick
    end
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 33
    Width = 636
    Height = 433
    ActivePage = TabSheet2
    Align = alClient
    TabOrder = 1
    OnChange = PageControl1Change
    object TabSheet1: TTabSheet
      Caption = 'List'
      DesignSize = (
        628
        405)
      object Label2: TLabel
        Left = 5
        Top = 38
        Width = 115
        Height = 21
        Alignment = taCenter
        AutoSize = False
        Caption = 'Doctor'
        Color = 2922059
        Font.Charset = ANSI_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        Transparent = False
        Layout = tlCenter
      end
      object SpeedButton1: TSpeedButton
        Left = 524
        Top = 3
        Width = 49
        Height = 51
        Anchors = [akTop, akRight]
        Flat = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
        Visible = False
        ExplicitLeft = 401
      end
      object SpeedButton2: TSpeedButton
        Left = 573
        Top = 3
        Width = 49
        Height = 51
        Anchors = [akTop, akRight]
        Flat = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
        OnClick = SpeedButton2Click
        ExplicitLeft = 450
      end
      object Edit_Search: TEdit
        Left = 126
        Top = 38
        Width = 128
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 0
        OnChange = Edit_SearchChange
      end
      object DBGrid1: TDBGrid
        Left = 3
        Top = 65
        Width = 619
        Height = 337
        DataSource = Ds_List
        TabOrder = 1
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnDblClick = DBGrid1DblClick
        Columns = <
          item
            Expanded = False
            FieldName = 'DOCCODE'
            Width = 65
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DOCNAME'
            Width = 170
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'QUALIFICATION'
            Width = 128
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'SPECIALIZATION'
            Width = 149
            Visible = True
          end>
      end
    end
    object TabSheet2: TTabSheet
      Caption = 'New'
      ImageIndex = 1
      object Label7: TLabel
        Left = 37
        Top = 266
        Width = 6
        Height = 13
        Alignment = taRightJustify
        Caption = '*'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Visible = False
      end
      object Label8: TLabel
        Left = 36
        Top = 290
        Width = 6
        Height = 13
        Alignment = taRightJustify
        Caption = '*'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Visible = False
      end
      object Label1: TLabel
        Left = 52
        Top = 144
        Width = 30
        Height = 13
        Alignment = taRightJustify
        Caption = 'Align :'
      end
      object lbl1: TLabel
        Left = 18
        Top = 171
        Width = 64
        Height = 13
        Alignment = taRightJustify
        Caption = 'Department :'
        Visible = False
      end
      object lbl2: TLabel
        Left = 43
        Top = 44
        Width = 39
        Height = 13
        Alignment = taRightJustify
        Caption = 'Doctor :'
      end
      object le_Doccode: TLabeledEdit
        Left = 117
        Top = 262
        Width = 145
        Height = 21
        CharCase = ecUpperCase
        EditLabel.Width = 67
        EditLabel.Height = 13
        EditLabel.Caption = 'Doctor Code :'
        LabelPosition = lpLeft
        TabOrder = 0
        Visible = False
      end
      object le_Docname: TLabeledEdit
        Left = 117
        Top = 289
        Width = 145
        Height = 21
        CharCase = ecUpperCase
        EditLabel.Width = 69
        EditLabel.Height = 13
        EditLabel.Caption = 'Doctor Name :'
        LabelPosition = lpLeft
        TabOrder = 1
        Visible = False
      end
      object le_qualification: TLabeledEdit
        Left = 85
        Top = 65
        Width = 244
        Height = 21
        EditLabel.Width = 66
        EditLabel.Height = 13
        EditLabel.Caption = 'Qualification :'
        LabelPosition = lpLeft
        TabOrder = 2
      end
      object le_sepcialization: TLabeledEdit
        Left = 85
        Top = 89
        Width = 244
        Height = 21
        EditLabel.Width = 71
        EditLabel.Height = 13
        EditLabel.Caption = 'Specialization :'
        LabelPosition = lpLeft
        TabOrder = 3
      end
      object cb_isactive: TCheckBox
        Left = 336
        Top = 40
        Width = 97
        Height = 17
        Caption = 'Is Active'
        TabOrder = 4
      end
      object Cb_Align: TComboBox
        Left = 85
        Top = 141
        Width = 244
        Height = 22
        Style = csOwnerDrawFixed
        TabOrder = 5
        Items.Strings = (
          'Left'
          'Center'
          'Right'
          'Left1')
      end
      object Le_Status: TLabeledEdit
        Left = 85
        Top = 115
        Width = 244
        Height = 21
        EditLabel.Width = 38
        EditLabel.Height = 13
        EditLabel.Caption = 'Status :'
        LabelPosition = lpLeft
        TabOrder = 6
      end
      object cbb1: TComboBox
        Left = 85
        Top = 169
        Width = 244
        Height = 22
        Style = csOwnerDrawFixed
        TabOrder = 7
        Visible = False
        Items.Strings = (
          'BIOCHEMISTRY'
          'MICROBIOLOGY'
          'HAEMATOLOGY'
          'SEROLOGY'
          'PARASITOLOGY')
      end
      object Dblcb_Doctor: TDBLookupComboBox
        Left = 85
        Top = 40
        Width = 244
        Height = 21
        KeyField = 'DocID'
        ListField = 'DocName'
        ListSource = DS_Doctor
        TabOrder = 8
        OnKeyDown = Dblcb_DoctorKeyDown
      end
    end
  end
  object Query_Department: TOraQuery
    SQL.Strings = (
      'Select * from hs_doct_Doctor'
      'Order by Doct_DocName')
    FetchAll = True
    Left = 424
    Top = 88
  end
  object Ds_Department: TDataSource
    DataSet = Query_Department
    Left = 488
    Top = 144
  end
  object Query_list: TOraQuery
    SQL.Strings = (
      'Select * from Report_Footer'
      'Order by DocName')
    Left = 280
    Top = 176
  end
  object Ds_List: TDataSource
    DataSet = Query_list
    Left = 408
    Top = 248
  end
  object Query_Doc: TOraQuery
    SQL.Strings = (
      
        'Select DOCT_DOCID,DOCT_DESIG||DOCT_DOCNAME as DocName,doct_docco' +
        'de doccode from hs_doct_Doctor'
      'Order by DOCT_DOCNAME')
    Left = 424
    Top = 176
  end
  object DS_Doctor: TDataSource
    DataSet = Query_Doc
    Left = 496
    Top = 248
  end
end
