object Form_ManualRepFooterSetup: TForm_ManualRepFooterSetup
  Left = 0
  Top = 0
  AutoSize = True
  Caption = 'Manual Report Footer Setup'
  ClientHeight = 340
  ClientWidth = 613
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnl_top: TPanel
    Left = 0
    Top = 0
    Width = 613
    Height = 35
    Align = alTop
    TabOrder = 0
    object lbl2: TLabel
      Left = 19
      Top = 9
      Width = 200
      Height = 13
      Caption = '* Please press [Delete] to remove footer.'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object btn_Save: TBitBtn
      Left = 456
      Top = 1
      Width = 78
      Height = 33
      Margins.Left = 0
      Margins.Top = 0
      Margins.Right = 0
      Margins.Bottom = 0
      Align = alRight
      Caption = '&Save'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 2
      OnClick = btn_SaveClick
    end
    object btnBB_Close: TBitBtn
      Left = 534
      Top = 1
      Width = 78
      Height = 33
      Margins.Left = 0
      Margins.Top = 6
      Margins.Right = 0
      Margins.Bottom = 6
      Align = alRight
      Caption = '&Close'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFEDEDF77C7DC96162C58383CB}
      ParentDoubleBuffered = False
      TabOrder = 0
      OnClick = btnBB_CloseClick
    end
    object btn_New: TBitBtn
      Left = 378
      Top = 1
      Width = 78
      Height = 33
      Margins.Left = 0
      Margins.Top = 0
      Margins.Right = 0
      Margins.Bottom = 0
      Align = alRight
      Caption = '&New'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 1
      OnClick = btn_NewClick
    end
  end
  object pnl_body: TPanel
    Left = 0
    Top = 35
    Width = 613
    Height = 305
    Align = alClient
    TabOrder = 1
    object pgc_Main: TPageControl
      Left = 1
      Top = 1
      Width = 611
      Height = 303
      ActivePage = ts_New
      Align = alClient
      TabOrder = 0
      OnChange = pgc_MainChange
      object ts_List: TTabSheet
        Caption = 'List'
        object dbgrd_List: TDBGrid
          Left = 0
          Top = 0
          Width = 603
          Height = 275
          Align = alClient
          DataSource = ds_list
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'Tahoma'
          TitleFont.Style = []
          OnDblClick = dbgrd_ListDblClick
          OnKeyDown = dbgrd_ListKeyDown
          Columns = <
            item
              Expanded = False
              FieldName = 'sno'
              Title.Caption = 'S. No.'
              Width = 42
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'depname'
              Title.Caption = 'Department'
              Width = 126
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'MARF_Footerleft'
              Title.Caption = 'Footer Left'
              Width = 140
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'MARF_FooterCent'
              Title.Caption = 'Footer Center'
              Width = 151
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'MARF_FooterRight'
              Title.Caption = 'Footer Right'
              Width = 152
              Visible = True
            end>
        end
      end
      object ts_New: TTabSheet
        Caption = 'New'
        ImageIndex = 1
        object lbl1: TLabel
          Left = 24
          Top = 32
          Width = 64
          Height = 13
          Caption = 'Department :'
        end
        object shp1: TShape
          Left = 3
          Top = 64
          Width = 589
          Height = 208
          Brush.Style = bsClear
          Shape = stRoundRect
        end
        object lbledt_footerleft: TLabeledEdit
          Left = 24
          Top = 139
          Width = 157
          Height = 21
          EditLabel.Width = 84
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Left Name'
          TabOrder = 5
        end
        object dblkcbb_department: TDBLookupComboBox
          Left = 96
          Top = 28
          Width = 162
          Height = 21
          KeyField = 'dept_depid'
          ListField = 'dept_depname'
          ListSource = ds_dep
          TabOrder = 0
          OnKeyDown = dblkcbb_departmentKeyDown
        end
        object chk_isactive: TCheckBox
          Left = 280
          Top = 32
          Width = 97
          Height = 17
          Caption = 'Is Active'
          TabOrder = 1
        end
        object lbledt_footercenter: TLabeledEdit
          Left = 224
          Top = 139
          Width = 153
          Height = 21
          EditLabel.Width = 98
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Center Name'
          TabOrder = 6
        end
        object lbledt_footerRight: TLabeledEdit
          Left = 424
          Top = 139
          Width = 153
          Height = 21
          EditLabel.Width = 90
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Right Name'
          TabOrder = 7
        end
        object lbledt_FLStatus: TLabeledEdit
          Left = 24
          Top = 99
          Width = 153
          Height = 21
          EditLabel.Width = 88
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Status Left'
          TabOrder = 2
        end
        object lbledt_FCstatus: TLabeledEdit
          Left = 224
          Top = 99
          Width = 153
          Height = 21
          EditLabel.Width = 102
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Status Center'
          TabOrder = 3
        end
        object lbledt_FRStatus: TLabeledEdit
          Left = 424
          Top = 99
          Width = 153
          Height = 21
          EditLabel.Width = 94
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Status Right'
          TabOrder = 4
        end
        object lbledt_FLQualification: TLabeledEdit
          Left = 24
          Top = 182
          Width = 153
          Height = 21
          EditLabel.Width = 116
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Left Qualification'
          TabOrder = 8
        end
        object lbledt_FCQualifiation: TLabeledEdit
          Left = 224
          Top = 187
          Width = 153
          Height = 21
          EditLabel.Width = 130
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Center Qualification'
          TabOrder = 9
        end
        object lbledt_FRQualification: TLabeledEdit
          Left = 424
          Top = 187
          Width = 153
          Height = 21
          EditLabel.Width = 122
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Right Qualification'
          TabOrder = 10
        end
        object lbledt_FLSpecialization: TLabeledEdit
          Left = 24
          Top = 224
          Width = 153
          Height = 21
          EditLabel.Width = 121
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Left Specialization'
          TabOrder = 11
        end
        object lbledt_FCSpecialization: TLabeledEdit
          Left = 224
          Top = 229
          Width = 153
          Height = 21
          EditLabel.Width = 135
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Center Specialization'
          TabOrder = 12
        end
        object lbledt_FRSpecialization: TLabeledEdit
          Left = 424
          Top = 229
          Width = 153
          Height = 21
          EditLabel.Width = 127
          EditLabel.Height = 13
          EditLabel.Caption = 'Footer Right Specialization'
          TabOrder = 13
        end
        object cbb_left: TComboBox
          Left = 24
          Top = 139
          Width = 157
          Height = 22
          Style = csOwnerDrawFixed
          TabOrder = 14
          OnClick = cbb_leftClick
          OnKeyDown = cbb_leftKeyDown
        end
        object cbb_center: TComboBox
          Left = 224
          Top = 139
          Width = 157
          Height = 22
          Style = csOwnerDrawFixed
          TabOrder = 15
          OnClick = cbb_centerClick
          OnKeyDown = cbb_centerKeyDown
        end
        object cbb_right: TComboBox
          Left = 424
          Top = 139
          Width = 157
          Height = 22
          Style = csOwnerDrawFixed
          TabOrder = 16
          OnClick = cbb_rightChange
          OnKeyDown = cbb_rightKeyDown
        end
        object btn_setup: TBitBtn
          Left = 488
          Top = 33
          Width = 75
          Height = 25
          Caption = '&Setup'
          DoubleBuffered = True
          ParentDoubleBuffered = False
          TabOrder = 17
          OnClick = btn_setupClick
        end
      end
    end
  end
  object oraqry_list: TOraQuery
    SQL.Strings = (
      
        'select rownum as sno,a.*,(select dept_depname as depname from hs' +
        '_dept_department where dept_depid=a.MARF_depid)depname'
      'from MARF_ManualReportFooter a order by MARF_depid')
    Left = 16
    Top = 80
  end
  object orqry_dep: TOraQuery
    SQL.Strings = (
      
        'select * from hs_dept_department where dept_deptype in('#39'P'#39','#39'H'#39') ' +
        'order by dept_depname')
    Left = 520
    Top = 48
  end
  object ds_dep: TDataSource
    DataSet = orqry_dep
    Left = 568
    Top = 56
  end
  object ds_list: TDataSource
    DataSet = oraqry_list
    Left = 80
    Top = 80
  end
  object orqry_Name: TOraQuery
    SQL.Strings = (
      
        'select * from UWMF_UserWiseManualFooterDtl order by UWMF_USERNAM' +
        'E')
    Left = 168
    Top = 56
  end
  object ds_name: TDataSource
    DataSet = orqry_Name
    Left = 400
    Top = 48
  end
  object orqry1: TOraQuery
    Session = DM_Hospital.DB
    SQL.Strings = (
      'select * from lab_usermain')
    Left = 224
    Top = 88
  end
  object orqry2: TOraQuery
    Session = DM_Hospital.DB
    SQL.Strings = (
      'select * from lab_usermain')
    Left = 280
    Top = 48
  end
  object ds1: TDataSource
    DataSet = orqry1
    Left = 472
    Top = 72
  end
  object ds2: TDataSource
    DataSet = orqry2
    Left = 352
    Top = 48
  end
  object orqry_userdetail: TOraQuery
    Session = DM_Hospital.DB
    Left = 160
    Top = 128
  end
end
