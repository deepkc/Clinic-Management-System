unit Unit_QrFindingGMCLabReport_kpl;
...
type
     TForm_QrFindingGMCLabReport_kpl = class(TForm)
    Qr_GMClabFinding: TQuickRep;
    ...
    procedure lbl_reporteddate1Print(sender: TObject; var Value: string);
     ...
     public
          useridleft,useridright,useridCenter,userwiseid:String;
          { Public declarations }
     end;

var
Form_QrFindingGMCLabReport_kpl: TForm_QrFindingGMCLabReport_kpl;
...
procedure TForm_QrFindingGMCLabReport_kpl.AddSignature;
VAR
...
procedure TForm_QrFindingGMCLabReport_kpl.ChildBand1AfterPrint(
...
procedure TForm_QrFindingGMCLabReport_kpl.ChildBand1BeforePrint(
...
procedure TForm_QrFindingGMCLabReport_kpl.CreateFindingsTable;
begin
...
procedure TForm_QrFindingGMCLabReport_kpl.FormCreate(Sender: TObject);
var
RepComment:string;
begin
    // QRImage1.Enabled:=True;
     ...
     if gi_compileValue in [14,23] then
     begin
    ...
    // lbl_referaldoc.Caption :=GetRefDoc(Gs_BillNo);   //GetRefHosDoctFromPatientTest(gi_PatientTestID,Gs_BillNo);
     ...
     begin
          ...
          begin
               ...
               if gi_compileValue in [1,14,21,22,23,24,26,27]  then
               begin
               ...
               if gi_compileValue in [1,14,21,22,23,24,26,27,28]  then
               begin
              ...
              begin
                   if gi_compileValue in [14,21,23,26,27] then
                   begin
...
procedure TForm_QrFindingGMCLabReport_kpl.FormDestroy(Sender: TObject);
begin
     Table_Findings.Free;
     Table_Footer.Free;
...
procedure TForm_QrFindingGMCLabReport_kpl.lbl_PrintdatePrint(sender: TObject;
  var Value: string);
begin
     value:=TodaysDateVS+' '+TodaysTime;
end;

procedure TForm_QrFindingGMCLabReport_kpl.lbl_referaldocPrint(Sender: TObject; var Value: string);
begin
     if Value = '' then
     begin
...
procedure TForm_QrFindingGMCLabReport_kpl.lbl_reporteddate1Print(sender: TObject;
  var Value: string);
begin
//     lbl_ReportedDate1.Caption:=GetFindingPostDateTime
end;

procedure TForm_QrFindingGMCLabReport_kpl.PageHeaderBand1AfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
     ItemsCount := 0;
end;

procedure TForm_QrFindingGMCLabReport_kpl.PrintBarcode;
Var
...
procedure TForm_QrFindingGMCLabReport_kpl.QRDBText1Print(Sender: TObject; var Value: string);
begin
...
procedure TForm_QrFindingGMCLabReport_kpl.QRSubDetail1AfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
     // if ItemsCount > 32 then
     // QrFinding.NewPage;
end;

procedure TForm_QrFindingGMCLabReport_kpl.QRSubDetail1BeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
...
procedure TForm_QrFindingGMCLabReport_kpl.QRSubDetail3AfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
...
procedure TForm_QrFindingGMCLabReport_kpl.QRSysData1Print(sender: TObject;
  var Value: string);
begin
...
procedure TForm_QrFindingGMCLabReport_kpl.SubDetail_SampleSourceBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
     if gb_HideSource then
          PrintBand := false;
end;

procedure TForm_QrFindingGMCLabReport_kpl.Table_FooterAfterScroll(DataSet: TDataSet);
begin
