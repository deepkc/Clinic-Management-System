...
uses
     ...
     Unit_Master, DB, DBTables, DBAccess, Ora, OraSmart, MemDS, OraError,DBgridexporttoexcel;

type
     TForm_SampleRegister = class(TForm)
          Panel3: TPanel;
          BtnCancel: TBitBtn;
          Panel1: TPanel;
...
var
     Form_SampleRegister: TForm_SampleRegister;

implementation

{$R *.dfm}

procedure TForm_SampleRegister.BB_RefreshClick(Sender: TObject);
begin
     With Query_List do
     begin
          Close;
          ...
          sql.Add('select patientid,patientname,billdate,billtime,billno,testname,sampleno,testprogressstatus from');
          sql.Add('(select patientid,patientname,testdate as billdate,testtime as billtime,billno,testname,'''' as sampleno,TESTPROGRESSSTATUS');
          sql.Add('from vw_sample_to_be_collected');
          sql.Add('where testdate between '+quotedstr(DEX_From.Text)+' and '+quotedstr(DEX_To.Text));
          sql.Add('and depid in ('+gs_UserDepId+')');
          if gi_compileValue=12 then
          SQL.Add('and deptype='+QuotedStr(gs_DepType));
          sql.add(' union');
          sql.Add('select patientid,patientname,patienttestdate as billdate,patienttesttime as billtime,billno,testname,sampleno,TESTPROGRESSSTATUS');
          sql.Add('from vw_sample_collected');
          sql.Add(' where patienttestdate between '+quotedstr(DEX_From.Text)+' and '+quotedstr(DEX_To.Text));
          sql.Add('and depid in ('+gs_UserDepId+')');
          if gi_compileValue=12 then
          SQL.Add('and deptype='+QuotedStr(gs_DepType));
          sql.Add(')');
          sql.add('order by to_Number(substr(SampleNo,9))');
         // sql.SaveToFile('c:\servicedetail.txt');
          Open;
     end;
...
procedure TForm_SampleRegister.BtnCancelClick(Sender: TObject);
begin
     Close;
end;

procedure TForm_SampleRegister.DBGrid_ServiceChargeDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
...
procedure TForm_SampleRegister.FormCreate(Sender: TObject);
begin
...
procedure TForm_SampleRegister.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
     if Key = 27 then
          BtnCancelClick(Sender);
end;

procedure TForm_SampleRegister.FormKeyPress(Sender: TObject; var Key: Char);
begin
     Key:=UpCase(Key)
end;

procedure TForm_SampleRegister.Rg_StatusClick(Sender: TObject);
begin
     with Query_List do
     begin
          Filtered:=False;
...
procedure TForm_SampleRegister.SPB_FromClick(Sender: TObject);
begin
     ChangeDateSystem(DEX_From, SPB_From);
end;

procedure TForm_SampleRegister.SPB_ToClick(Sender: TObject);
begin
     ChangeDateSystem(DEX_To, SPB_To);
end;

procedure TForm_SampleRegister.SpeedButton1Click(Sender: TObject);
Var
     Qry: TOraQuery;
begin
...
procedure TForm_SampleRegister.SpeedButton2Click(Sender: TObject);
begin
...
procedure TForm_SampleRegister.SpeedButton3Click(Sender: TObject);
begin
     if MessageDlg('Are you sure to export?',mtConfirmation,mbYesNo,0)=mrYes then
     begin
     //     SendToExcels(nil,DBGrid_ServiceCharge,'Sample Register',TodaysDate,'',gs_HospitalName,gs_HospitalAddress,0);
       ExportDBGrid(Form_SampleRegister,DBGrid_ServiceCharge,true,'Sample Register',TodaysDate);
     end;
end;
