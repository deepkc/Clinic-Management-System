object Form_Users: TForm_Users
  Left = 0
  Top = 0
  Caption = 'Users'
  ClientHeight = 507
  ClientWidth = 786
  Color = 16764108
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poOwnerFormCenter
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PageControl_Users: TPageControl
    Left = 0
    Top = 40
    Width = 786
    Height = 467
    ActivePage = TabSheet_List
    Align = alClient
    TabOrder = 0
    OnChange = PageControl_UsersChange
    object TabSheet_List: TTabSheet
      Caption = 'List'
      object DBGrid1: TDBGrid
        Left = 0
        Top = 27
        Width = 775
        Height = 387
        DataSource = DS_List
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnDblClick = DBGrid1DblClick
        OnTitleClick = DBGrid1TitleClick
        Columns = <
          item
            Expanded = False
            FieldName = 'USERID'
            Title.Caption = 'SN.'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'USERNAME'
            Title.Caption = 'User'
            Width = 100
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FULLNAME'
            Title.Caption = 'Full Name'
            Width = 140
            Visible = True
          end>
      end
      object Edit_SUserId: TEdit
        Left = 13
        Top = 3
        Width = 51
        Height = 21
        NumbersOnly = True
        TabOrder = 1
        OnChange = Edit_SUserIdChange
      end
      object Edit_SUserName: TEdit
        Left = 65
        Top = 3
        Width = 100
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 2
        OnChange = Edit_SUserNameChange
      end
      object Edit_SExpDate: TEdit
        Left = 310
        Top = 3
        Width = 100
        Height = 21
        TabOrder = 3
        OnChange = Edit_SExpDateChange
        OnKeyPress = Edit_SExpDateKeyPress
      end
      object Edit_SFullName: TEdit
        Left = 166
        Top = 3
        Width = 141
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 4
        OnChange = Edit_SFullNameChange
      end
      object StatusBar1: TStatusBar
        Left = 0
        Top = 420
        Width = 778
        Height = 19
        Panels = <
          item
            Width = 200
          end
          item
            Width = 50
          end>
      end
      object DBLCB_StaffDept: TDBLookupComboBox
        Left = 411
        Top = 3
        Width = 133
        Height = 21
        KeyField = 'STAFFDEPID'
        ListField = 'STAFFDEPname'
        ListSource = Ds_StaffDep
        TabOrder = 6
        Visible = False
        OnClick = DBLCB_StaffDeptClick
        OnKeyDown = DBLCB_StaffDeptKeyDown
      end
    end
    object TabSheet_New: TTabSheet
      Caption = 'New'
      ImageIndex = 1
      object Label1: TLabel
        Left = 119
        Top = 23
        Width = 59
        Height = 13
        Caption = 'User Name :'
      end
      object Label2: TLabel
        Left = 125
        Top = 47
        Width = 53
        Height = 13
        Caption = 'Password :'
      end
      object Label3: TLabel
        Left = 85
        Top = 71
        Width = 93
        Height = 13
        Caption = 'Confirm Password :'
      end
      object Label4: TLabel
        Left = 126
        Top = 294
        Width = 52
        Height = 13
        Caption = 'Exp-Date :'
      end
      object Label5: TLabel
        Left = 125
        Top = 214
        Width = 53
        Height = 13
        Caption = 'Full Name :'
      end
      object ADVS_ExpDate: TSpeedButton
        Left = 257
        Top = 292
        Width = 23
        Height = 21
        Caption = 'BS'
        OnClick = ADVS_ExpDateClick
      end
      object Label6: TLabel
        Left = 423
        Top = 188
        Width = 91
        Height = 13
        Caption = 'Staff Department :'
        Visible = False
      end
      object Label7: TLabel
        Left = 119
        Top = 166
        Width = 59
        Height = 13
        Caption = 'User Name :'
      end
      object SpeedButton4: TSpeedButton
        Left = 677
        Top = 187
        Width = 20
        Height = 20
        Flat = True
        Glyph.Data = {
          
          F000}
        Visible = False
        OnClick = SpeedButton4Click
      end
      object Label8: TLabel
        Left = 448
        Top = 214
        Width = 66
        Height = 13
        Caption = 'Group Name :'
        Visible = False
      end
      object Image1: TImage
        Left = 191
        Top = 350
        Width = 167
        Height = 63
        AutoSize = True
      end
      object Label9: TLabel
        Left = 125
        Top = 323
        Width = 53
        Height = 13
        Caption = 'Signature :'
      end
      object sp_picOpen: TSpeedButton
        Left = 184
        Top = 319
        Width = 46
        Height = 25
        Glyph.Data = {
          
          ECD8E9ECD8E9ECD8E9EC}
        OnClick = sp_picOpenClick
      end
      object Label10: TLabel
        Left = 87
        Top = 189
        Width = 91
        Height = 13
        Caption = 'Staff Department :'
      end
      object Label11: TLabel
        Left = 115
        Top = 240
        Width = 63
        Height = 13
        Caption = 'Designation :'
      end
      object Label_NMCNO: TLabel
        Left = 133
        Top = 267
        Width = 45
        Height = 13
        Caption = 'NMC No :'
      end
      object Edit_UserName: TEdit
        Left = 191
        Top = 20
        Width = 145
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 0
      end
      object Edit_Password: TEdit
        Left = 191
        Top = 44
        Width = 145
        Height = 21
        CharCase = ecUpperCase
        PasswordChar = '*'
        TabOrder = 1
      end
      object Edit_ConfirmPassword: TEdit
        Left = 191
        Top = 68
        Width = 145
        Height = 21
        CharCase = ecUpperCase
        PasswordChar = '*'
        TabOrder = 2
      end
      object DEX_ExpDate: TDateEditX
        Left = 191
        Top = 292
        Width = 64
        Height = 21
        TabOrder = 7
        ControlData = {
          
          7374656D4F6644617465070C64734973776953616D6261740000}
      end
      object Edit_FullName: TEdit
        Left = 191
        Top = 211
        Width = 145
        Height = 21
        TabOrder = 6
      end
      object GroupBox1: TGroupBox
        Left = 191
        Top = 89
        Width = 145
        Height = 69
        Caption = 'User'
        TabOrder = 3
        object Rb_Doctor: TRadioButton
          Left = 16
          Top = 14
          Width = 65
          Height = 17
          Caption = 'Is Doctor'
          TabOrder = 0
          OnClick = Rb_DoctorClick
        end
        object Rb_Nurse: TRadioButton
          Left = 16
          Top = 32
          Width = 57
          Height = 17
          Caption = 'Is Nurse'
          TabOrder = 1
          Visible = False
          OnClick = Rb_NurseClick
        end
        object Rb_Other: TRadioButton
          Left = 16
          Top = 50
          Width = 49
          Height = 17
          Caption = 'Other'
          TabOrder = 2
          OnClick = Rb_OtherClick
        end
      end
      object Dblcb_DocNurse: TDBLookupComboBox
        Left = 191
        Top = 162
        Width = 145
        Height = 21
        KeyField = 'NURSEID'
        ListField = 'NAME'
        ListSource = Ds_DocNurse
        TabOrder = 4
        OnClick = Dblcb_DocNurseClick
      end
      object Dblcb_StaffDep: TDBLookupComboBox
        Left = 527
        Top = 186
        Width = 145
        Height = 21
        KeyField = 'STAFFDEPID'
        ListField = 'STAFFDEPname'
        ListSource = Ds_StaffDep
        TabOrder = 8
        Visible = False
      end
      object Dblc_Gname: TDBLookupComboBox
        Left = 527
        Top = 210
        Width = 145
        Height = 21
        KeyField = 'GROUPSETUPID'
        ListField = 'GROUPNAME'
        ListSource = Ds_Ganame
        TabOrder = 9
        Visible = False
      end
      object cb_staffdepartment: TComboBox
        Left = 191
        Top = 186
        Width = 145
        Height = 22
        Style = csOwnerDrawFixed
        TabOrder = 5
        Items.Strings = (
          'PATHOLOGY'
          'RADIOLOGY'
          'HISTOLOGY')
      end
      object Edit_Desig: TEdit
        Left = 191
        Top = 239
        Width = 197
        Height = 21
        TabOrder = 10
      end
      object Edit_NMCNO: TEdit
        Left = 191
        Top = 265
        Width = 197
        Height = 21
        TabOrder = 11
      end
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 786
    Height = 40
    Align = alTop
    TabOrder = 1
    DesignSize = (
      786
      40)
    object BB_New: TBitBtn
      Left = 215
      Top = 1
      Width = 106
      Height = 38
      Align = alRight
      Caption = '&New [F2]'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 0
      OnClick = BB_NewClick
    end
    object BB_Previlage: TBitBtn
      Left = 515
      Top = 1
      Width = 90
      Height = 38
      Align = alRight
      Caption = '&Previlege'
      DoubleBuffered = True
      ParentDoubleBuffered = False
      TabOrder = 4
      OnClick = BB_PrevilageClick
    end
    object BB_Save: TBitBtn
      Left = 605
      Top = 1
      Width = 90
      Height = 38
      Align = alRight
      Caption = '&Save [F12]'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 1
      OnClick = BB_SaveClick
    end
    object BB_Close: TBitBtn
      Left = 695
      Top = 1
      Width = 90
      Height = 38
      Align = alRight
      Caption = '&Close [ESC]'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFEDEDF77C7DC96162C58383CB}
      ParentDoubleBuffered = False
      TabOrder = 2
      OnClick = BB_CloseClick
    end
    object BitBtn1: TBitBtn
      Left = 2
      Top = 6
      Width = 172
      Height = 30
      Anchors = [akTop, akRight]
      Caption = 'User Accessible Department'
      DoubleBuffered = True
      ParentDoubleBuffered = False
      TabOrder = 3
      OnClick = BitBtn1Click
    end
    object btn_ReportPrevilage: TBitBtn
      Left = 418
      Top = 1
      Width = 97
      Height = 38
      Align = alRight
      Caption = 'Report Previlage'
      DoubleBuffered = True
      ParentDoubleBuffered = False
      TabOrder = 5
      OnClick = btn_ReportPrevilageClick
    end
    object Btn_MenuPrevilage: TBitBtn
      Left = 321
      Top = 1
      Width = 97
      Height = 38
      Align = alRight
      Caption = '&Menu Previlage'
      DoubleBuffered = True
      ParentDoubleBuffered = False
      TabOrder = 6
      OnClick = btn_ReportPrevilageClick
    end
  end
  object Query_UserEntry: TOraQuery
    Left = 472
    Top = 224
  end
  object Query_List: TOraQuery
    SQL.Strings = (
      
        'select userid,username,upassword,deptype,expdate,fullname,design' +
        'ation,Userdepid,doctorid,nurseid,GroupId,Nmcno,Nhpcno,(select gr' +
        'oupname from groupsetup where groupsetupid=lab_usermain.groupid)' +
        'GroupName'
      'from lab_usermain'
      'where 1=1'
      'and 1=1'
      'Order by UserName')
    Left = 48
    Top = 304
  end
  object DS_List: TDataSource
    DataSet = Query_List
    Left = 56
    Top = 144
  end
  object Ds_StaffDep: TDataSource
    DataSet = Query_StaffDep
    Left = 64
    Top = 208
  end
  object Ds_DocNurse: TDataSource
    DataSet = Query_DocNurse
    Left = 280
    Top = 168
  end
  object Query_DocNurse: TOraQuery
    SQL.Strings = (
      'select NurseId,Name from nurse')
    Left = 388
    Top = 176
  end
  object Query_StaffDep: TOraQuery
    SQL.Strings = (
      'select * from staffdepartment')
    Left = 404
    Top = 264
  end
  object Ds_Ganame: TDataSource
    DataSet = Query_Gname
    Left = 24
    Top = 168
  end
  object Query_Gname: TOraQuery
    SQL.Strings = (
      'select * from GroupSetup')
    Left = 128
    Top = 160
  end
  object OpenPictureDialog1: TOpenPictureDialog
    Left = 568
    Top = 144
  end
  object qryTQuery: TOraQuery
    Options.TemporaryLobUpdate = True
    Options.EnableBCD = True
    Left = 488
    Top = 320
  end
end
